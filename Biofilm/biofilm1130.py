...
def plot_fit(data, fit, title, labels):
    steps = np.arange(len(data))
    colors = ['blue', 'orange', 'green', 'red']
    plt.figure(figsize=(8, 5))
    for i in range(data.shape[1]):
        plt.plot(steps, data[:, i], 'o', color=colors[i], alpha=0.4, label=f'{labels[i]} Data')
        plt.plot(steps, fit[:, i], '-', color=colors[i], linewidth=2, label=f'{labels[i]} Fit')
    plt.title(title)
...
if __name__ == "__main__":
    ...
    # Plot Fig 9
    plot_fit(d_M1, solver.run_M1(est_M1), "Fig 9: M1 Fit (Species 1 & 2)", ["Sp1", "Sp2"])
    ...
    # Plot Fig 11
    plot_fit(d_M2, solver.run_M2(est_M2), "Fig 11: M2 Fit (Species 3 & 4)", ["Sp3", "Sp4"])
    ...
    # Plot Fig 13
    plot_fit(d_M3, solver.run_M3(est_M3, est_M1, est_M2), "Fig 13: M3 Fit (All Species)", ["Sp1", "Sp2", "Sp3", "Sp4"])

    # --- Final Results ---
    print("\n=== ESTIMATION RESULT ===")
    # ... (前略: est_M1, est_M2, est_M3 が求まった後) ...

    # 1. まず全データを結合 (元の順序: M1 -> M2 -> M3)
    ...
    # 2. 並び替えたい新しい順序 (Target Order)
    # a: 11, 12, 13, 14, 22, 23, 24, 33, 34, 44
    # b: 1, 2, 3, 4
    new_order_indices = [
        0,  # a11
        1,  # a12
    ...
    plt.figure(figsize=(14, 6)) # 横幅を少し広げる
    x = np.arange(len(all_true))

    # 色分け: aパラメータ(青系), bパラメータ(緑系)
    colors_true = ['#ff7f0e'] * 10 + ['#2ca02c'] * 4 # Orange for A, Green for B? No, let's stick to True/Est colors
