...
# =============================================================================
# =============================================================================
# 4. NEWTON-RAPHSON BIOFILM SOLVER (Case II)
# =============================================================================
class HierarchicalBiofilmSolver:
    """
    Newton-Raphson based biofilm solver for Case II.
    Based on the provided Mathematica-translated code.
    """
    
    def __init__(self, eps=1e-3, max_newton_iter=100):
        self.eps = eps
        self.max_newton_iter = max_newton_iter
        self.Kp1 = 1e-4  # Penalty parameter
    
    def _compute_Q_vector_2species(self, g_new, g_old, t, dt, A, B, eta, c_star, alpha_star):
        """Compute residual vector Q for 2-species model."""
        phi_new = g_new[0:2]
        phi0_new = g_new[2]
        psi_new = g_new[3:5]
        gamma_new = g_new[5]
        ...
        # Penalty terms
        phi_penalty = (self.Kp1 * (2.0 - 4.0 * phi_new)) / (np.power(phi_new - 1.0, 3) * np.power(phi_new, 3) + 1e-20)
        phi0_penalty = (self.Kp1 * (2.0 - 4.0 * phi0_new)) / (np.power(phi0_new - 1.0, 3) * np.power(phi0_new, 3) + 1e-20)
        psi_penalty = (-2.0 * self.Kp1) / (np.power(psi_new - 1.0, 2) * np.power(psi_new, 3) + 1e-20) - \
                      (2.0 * self.Kp1) / (np.power(psi_new - 1.0, 3) * np.power(psi_new, 2) + 1e-20)
        
        # Q[0:2] - phi equations
        term1_phi = phi_penalty
        term2_phi = (1.0 / eta) * (gamma_new + 
                    (eta + eta * psi_new**2) * phidot + 
                    eta * phi_new * psi_new * psidot)
        term3_phi = (c_t / eta) * psi_new * Interaction
        Q[0:2] = term1_phi + term2_phi - term3_phi
        
        # Q[2] - phi0 equation
        Q[2] = gamma_new + phi0_penalty + phi0dot
        
        # Q[3:5] - psi equations
        term1_psi = psi_penalty
        term2_psi = (B * alpha_t / eta) * psi_new
        term3_psi = phi_new * psi_new * phidot + phi_new**2 * psidot
        term4_psi = (c_t / eta) * phi_new * Interaction
        Q[3:5] = term1_psi + term2_psi + term3_psi - term4_psi
        
        # Q[5] - constraint equation
        Q[5] = np.sum(phi_new) + phi0_new - 1.0
        
        return Q
    
    def _compute_Jacobian_2species(self, g_new, g_old, t, dt, A, B, eta, c_star, alpha_star):
        """Compute Jacobian matrix K for 2-species model."""
        v = g_new
        phi_new = g_new[0:2]
        phi0_new = g_new[2]
        psi_new = g_new[3:5]
        
        phidot = (phi_new - g_old[0:2]) / dt
        psidot = (psi_new - g_old[3:5]) / dt
        ...
        # Penalty derivatives
        phi_p_deriv = (self.Kp1*(-4. + 8.*v[0:2]))/(np.power(v[0:2],3)*np.power(v[0:2]-1.,3) + 1e-20) - \
                      (self.Kp1*(2. - 4.*v[0:2]))*(3./(np.power(v[0:2],4)*np.power(v[0:2]-1.,3) + 1e-20) + 
                                                   3./(np.power(v[0:2],3)*np.power(v[0:2]-1.,4) + 1e-20))
        
        phi0_p_deriv = (self.Kp1*(-4. + 8.*v[2]))/(np.power(v[2],3)*np.power(v[2]-1.,3) + 1e-20) - \
                       (self.Kp1*(2. - 4.*v[2]))*(3./(np.power(v[2],4)*np.power(v[2]-1.,3) + 1e-20) + 
                                                  3./(np.power(v[2],3)*np.power(v[2]-1.,4) + 1e-20))
        
        psi_p_deriv = (4.0 * self.Kp1 * (3.0 - 5.0*v[3:5] + 5.0*v[3:5]**2)) / (np.power(v[3:5], 4) * np.power(v[3:5] - 1.0, 4) + 1e-20)
        
        for i in range(2):
            for j in range(2):
                K[i, j] = (c_t / eta[i]) * psi_new[i] * (-A[i, j] * psi_new[j])
            
            K[i, i] = phi_p_deriv[i] + \
                      (1.0 / eta[i]) * ((eta[i] + eta[i] * psi_new[i]**2) / dt + eta[i] * psi_new[i] * psidot[i]) - \
                      (c_t / eta[i]) * (psi_new[i] * (Interaction[i] + A[i, i] * psi_new[i]))
            
            K[i, 2] = 0.0
            
            for j in range(2):
                K[i, j+3] = (c_t / eta[i]) * psi_new[i] * (-A[i, j] * phi_new[j])
            
            K[i, i+3] = (1.0 / eta[i]) * (2.0 * eta[i] * psi_new[i] * phidot[i] + 
                                          eta[i] * phi_new[i] * psidot[i] + 
                                          eta[i] * phi_new[i] * psi_new[i] / dt) - \
                        (c_t / eta[i]) * ((Interaction[i] + A[i, i] * phi_new[i] * psi_new[i]) + 
                                          psi_new[i] * (A[i, i] * phi_new[i]))
            
            K[i, 5] = 1.0 / eta[i]
        
        K[2, 2] = phi0_p_deriv + 1.0/dt
        K[2, 5] = 1.0
        ...
        for i in range(2):
            ...
            for j in range(2):
                K[k, j] = -(c_t / eta[i]) * (A[i, j] * psi_new[j] * phi_new[i] + 
                                             Interaction[i] * (1.0 if i == j else 0.0))
            
            K[k, i] = (psi_new[i] * phidot[i] + psi_new[i] * phi_new[i] / dt + 2.0 * phi_new[i] * psidot[i]) - \
                      (c_t / eta[i]) * (A[i, i] * psi_new[i] * phi_new[i] + Interaction[i] + phi_new[i] * A[i, i] * psi_new[i])
            
            K[k, 2] = 0.0
            
            for j in range(2):
                K[k, j+3] = -(c_t / eta[i]) * phi_new[i] * (A[i, j] * phi_new[j])
            
            K[k, i+3] = psi_p_deriv[i] + \
                        (B[i] * alpha_t / eta[i]) + \
                        (phi_new[i] * phidot[i] + phi_new[i]**2 / dt) - \
                        (c_t / eta[i]) * phi_new[i] * (A[i, i] * phi_new[i])
    ...
    def run_M1(self, params):
        """
        Run M1 for Case II: Species 1 & 2.
        Table 3: η=[1,1], φ₀=[0.2,0.2], c*=100, α*=100, N=2500, Δt=1e-5
        """
        ...
        n_steps = 2500
        n_data = 20
        save_every = n_steps // n_data
        
        phi_init = np.array([0.2, 0.2])
        phi0_init = 1.0 - np.sum(phi_init)
        psi_init = np.array([0.999, 0.999])
        gamma_init = 0.0
        
        g_prev = np.concatenate([phi_init, [phi0_init], psi_init, [gamma_init]])
        ...
        for step in range(n_steps):
            ...
            for newton_iter in range(self.max_newton_iter):
                Q = self._compute_Q_vector_2species(g_new, g_prev, t, dt, A, B, eta, c_star, alpha_star)
                ...
                K = self._compute_Jacobian_2species(g_new, g_prev, t, dt, A, B, eta, c_star, alpha_star)
                ...
                try:
                    ...
                    g_new[3:5] = np.clip(g_new[3:5], 1e-6, 1.0 - 1e-6)
                except:
                    break
            ...
            if step % save_every == 0 and step > 0:
                phi_bar = g_new[0:2] * g_new[3:5]
                results.append(phi_bar.copy())
        
        return np.array(results) if results else np.zeros((1, 2))
    
    def run_M2(self, params):
        """
        Run M2 for Case II: Species 3 & 4.
        Table 3: η=[1,1], φ₀=[0.2,0.2], c*=100, α*=10, N=5000, Δt=1e-5
        """
        a33, a34, a44, b3, b4 = params
        
        A = np.array([[a33, a34], [a34, a44]])
        B = np.array([b3, b4])
        eta = np.array([1.0, 1.0])
    ...
    def _compute_Q_vector_4species(self, g_new, g_old, t, dt, A, B, eta, c_star, alpha_star):
        """Compute residual vector Q for 4-species model."""
        phi_new = g_new[0:4]
        phi0_new = g_new[4]
        psi_new = g_new[5:9]
        gamma_new = g_new[9]
        ...
        Q[4] = gamma_new + phi0_penalty + phi0dot
    ...
    def _compute_Jacobian_4species(self, g_new, g_old, t, dt, A, B, eta, c_star, alpha_star):
        """Compute Jacobian matrix K for 4-species model."""
        v = g_new
        phi_new = g_new[0:4]
        psi_new = g_new[5:9]
        ...
        phi_p_deriv = (self.Kp1*(-4. + 8.*v[0:4]))/(np.power(v[0:4],3)*np.power(v[0:4]-1.,3) + 1e-20) - \
                      (self.Kp1*(2. - 4.*v[0:4]))*(3./(np.power(v[0:4],4)*np.power(v[0:4]-1.,3) + 1e-20) + 
                                                   3./(np.power(v[0:4],3)*np.power(v[0:4]-1.,4) + 1e-20))
        
        phi0_p_deriv = (self.Kp1*(-4. + 8.*v[4]))/(np.power(v[4],3)*np.power(v[4]-1.,3) + 1e-20) - \
                       (self.Kp1*(2. - 4.*v[4]))*(3./(np.power(v[4],4)*np.power(v[4]-1.,3) + 1e-20) + 
                                                  3./(np.power(v[4],3)*np.power(v[4]-1.,4) + 1e-20))
        
        psi_p_deriv = (4.0 * self.Kp1 * (3.0 - 5.0*v[5:9] + 5.0*v[5:9]**2)) / (np.power(v[5:9], 4) * np.power(v[5:9] - 1.0, 4) + 1e-20)
        ...
        for i in range(4):
            ...
            for j in range(4):
                K[i, j+5] = (c_t / eta[i]) * psi_new[i] * (-A[i, j] * phi_new[j])
            
            K[i, i+5] = (1.0 / eta[i]) * (2.0 * eta[i] * psi_new[i] * phidot[i] + 
                                          eta[i] * phi_new[i] * psidot[i] + 
                                          eta[i] * phi_new[i] * psi_new[i] / dt) - \
                        (c_t / eta[i]) * ((Interaction[i] + A[i, i] * phi_new[i] * psi_new[i]) + 
                                          psi_new[i] * (A[i, i] * phi_new[i]))
        ...
        for i in range(4):
            ...
            for j in range(4):
                K[k, j+5] = -(c_t / eta[i]) * phi_new[i] * (A[i, j] * phi_new[j])
    ...
    def run_M3(self, params_M3, params_M1, params_M2):
        """
        Run M3 for Case II: All 4 species.
        Table 3: η=[1,1,1,1], φ₀=[0.02]*4, c*=25, α*=0, N=750, Δt=1e-4
        """
        ...
        n_steps = 750
        n_data = 20
        save_every = max(1, n_steps // n_data)
        
        phi_init = np.array([0.02, 0.02, 0.02, 0.02])
        phi0_init = 1.0 - np.sum(phi_init)
        psi_init = np.array([0.999, 0.999, 0.999, 0.999])
        gamma_init = 0.0
        ...
        for step in range(n_steps):
            ...
            for newton_iter in range(self.max_newton_iter):
                Q = self._compute_Q_vector_4species(g_new, g_prev, t, dt, A, B, eta, c_star, alpha_star)
                ...
                K = self._compute_Jacobian_4species(g_new, g_prev, t, dt, A, B, eta, c_star, alpha_star)
                ...
                try:
                    ...
                    g_new[5:9] = np.clip(g_new[5:9], 1e-6, 1.0 - 1e-6)
                except:
                    break
            ...
            if step % save_every == 0 and step > 0:
                phi_bar = g_new[0:4] * g_new[5:9]
                results.append(phi_bar.copy())
        
        return np.array(results) if results else np.zeros((1, 4))
    
    def run_M3_val(self, params_M1, params_M2, params_M3):
        """Run M3_val: Validation with time-dependent antibiotics (α*=50 for t>0.5)."""
        a13, a14, a23, a24 = params_M3
        a11, a12, a22, b1, b2 = params_M1
        a33, a34, a44, b3, b4 = params_M2
        ...
        for step in range(n_steps):
            ...
            alpha_star = 50.0 if step > n_steps // 2 else 0.0
...
def likelihood_with_uncertainty_parallel(params, data, solver_func, 
                                         tolerance=1e-4, n_mc_samples=30):
    """
...
class BayesianOptimizer:
    ...
    def objective_M3(self, params, data, m1, m2):
        """Objective for M3"""
        # Direct call instead of lambda to avoid pickle issues
        sim = self.solver.run_M3(params, m1, m2)
        if sim is None:
            return 1e15
        
        # Inline likelihood calculation to avoid lambda
        CoV = 0.005
        mc_outputs = []
        
        for i in range(30):
            noisy_params = params * (1 + np.random.normal(0, CoV, len(params)))
            noisy_sim = self.solver.run_M3(noisy_params, m1, m2)
            if noisy_sim is not None:
                mc_outputs.append(noisy_sim)
    ...
    def optimize(self, obj_func, bounds, args_tuple, name, 
        ...
        Note: M3 uses sequential workers to avoid pickle issues with complex args
        """
        self.reset()
        param_names = [f'p{i}' for i in range(len(bounds))]
        
        # Determine if we can use parallel workers
        # M3 has complex args that can't be pickled, so use sequential
        use_parallel = (name != "M3")
        n_workers = -1 if use_parallel else 1
        update_mode = 'deferred' if use_parallel else 'immediate'
        
        # Stage 1: Global search with differential evolution
        print(f"\n  Stage 1 (Coarse): Global search for {name}...")
        if not use_parallel:
            print(f"    (Using sequential mode for {name} due to complex parameters)")
    ...
    def _generate_posterior_parallel(self, obj_func, map_estimate, bounds, 
        ...
        Falls back to sequential if parallel fails (e.g., pickle issues)
        """
        ...
        # Always use sequential for safety (methods can't be pickled)
        # Parallel MCMC would require refactoring to use global functions
        results = []
        for arg in pool_args:
            result = _run_mcmc_chain_worker(arg)
