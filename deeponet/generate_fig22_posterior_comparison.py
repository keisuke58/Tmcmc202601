...
PARAM_NAMES = [
    r'$\mu_{\mathrm{So}}$', r'$\mu_{\mathrm{An}}$', r'$\mu_{\mathrm{Vd}}$',
    r'$\mu_{\mathrm{Fn}}$', r'$\mu_{\mathrm{Pg}}$',
    r'$a_{12}$', r'$a_{13}$', r'$a_{14}$', r'$a_{21}$', r'$a_{23}$',
    r'$a_{24}$', r'$a_{31}$', r'$a_{32}$', r'$a_{34}$', r'$a_{41}$',
    r'$a_{15}$', r'$a_{25}$', r'$a_{35}^{\mathrm{inv}}$', r'$a_{35}$', r'$a_{45}$',
]
...
def load_samples_and_map(run_dir):
    ...
    if samples_path.exists(): samples = np.load(str(samples_path))
    if logL_path.exists(): logL = np.load(str(logL_path))
    if map_path.exists():
        with open(map_path) as f: data = json.load(f)
...
def compute_overlap(s1, s2, idx):
    ...
    if std1 < 1e-8 or std2 < 1e-8: return 0.0
...
def plot_kde(ax, data, color, label, alpha_fill=0.25):
    if len(data) < 5 or data.std() < 1e-10:
        ax.axvline(data.mean(), color=color, lw=2, label=label); return
    try:
...
def main():
    ...
    if ode_s is None or don_s is None: print('ERROR: DH samples not found'); return
    ...
    for idx in range(20):
        ...
        if ode_m is not None: ax.axvline(ode_m[idx], color=c_ode, lw=1.2, ls='--', alpha=0.7)
        if don_m is not None: ax.axvline(don_m[idx], color=c_don, lw=1.2, ls=':', alpha=0.7)
        ol = overlaps[idx]
        ...
        if idx == 0: ax.legend(fontsize=6, loc='upper right', framealpha=0.7)
    ...
    ax_ol.set_ylim(0, 1.08); ax_ol.grid(axis='y', alpha=0.2)
    for i, ol in enumerate(overlaps):
        if ol < 0.6: ax_ol.text(i, ol+0.03, f'{ol:.2f}', ha='center', va='bottom', fontsize=7, fontweight='bold', color='#C62828')
    hi = sum(1 for o in overlaps if o>0.95)
    md = sum(1 for o in overlaps if 0.5<=o<=0.95)
    lo = sum(1 for o in overlaps if o<0.5)
    ax_ol.text(0.02, 0.95, f'High (>0.95): {hi}/20\nMedium: {md}/20\nLow (<0.5): {lo}/20',
               transform=ax_ol.transAxes, fontsize=7, va='top',
    ...
    ax_sp.set_yscale('log'); ax_sp.set_xticks(x); ax_sp.set_xticklabels(conds, fontsize=10)
    ax_sp.set_ylabel('Wall Time [s]', fontsize=9)
    ax_sp.set_title('(c) TMCMC Computational Cost', fontsize=10, fontweight='bold')
    ax_sp.legend(fontsize=8, loc='upper right'); ax_sp.grid(axis='y', alpha=0.2, which='both')
    for i,(s,d) in enumerate(zip(su,dv)):
        ax_sp.annotate(f'{s:.0f}x', xy=(x[i]+w/2, d), xytext=(0,-15),
                       textcoords='offset points', ha='center', fontsize=10, fontweight='bold', color='#2E7D32')

    fig.suptitle('(a) DeepONet-TMCMC vs ODE-TMCMC Posterior Comparison\n(Dysbiotic HOBIC, dashed/dotted = MAP)',
                 fontsize=12, fontweight='bold', y=0.995)
    for ext in ['png','pdf']:
        out = PAPER_FIG_DIR / f'Fig22_deeponet_posterior_comparison.{ext}'
        fig.savefig(str(out), dpi=300, bbox_inches='tight'); print(f'Saved: {out}')
    plt.close(fig)
    ...
    for c in conds:
        ...
        if om is not None and dm is not None:
            ...
            print(f'{c:<8} ODE={ot[c]:>6.0f}s DON={dt[c]:>6.1f}s Speedup={ot[c]/dt[c]:>5.0f}x MAP_diff={diff:.4f}')
