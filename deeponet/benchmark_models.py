...
from dem_elasticity_3d import ElasticityNetwork, strain_energy_density
...
def run_pinn(key):
    ...
    for i in range(N_EPOCHS):
        ...
        # Random points
        dom_pts = jr.uniform(
            k1, (BATCH_SIZE_PINN, 3), minval=jnp.array([0.0, 0.0, 0.0]), maxval=jnp.array([W, H, D])
        )
        bc_pts = jr.uniform(
            k2,
            (BATCH_SIZE_PINN // 10, 3),
            minval=jnp.array([0.0, H, 0.0]),
            maxval=jnp.array([W, H, D]),
        )
...
def main():
    ...
    print("\n\n=== Benchmark Results ===")
    print(
        f"{'Model':<10} | {'Params':<10} | {'Time/Epoch (s)':<15} | {'Total Time (s)':<15} | {'Final Loss/Energy':<20}"
    )
    print("-" * 80)
    for r in results:
        print(
            f"{r['name']:<10} | {r['params']:<10} | {r['avg_time_per_epoch']:<15.4f} | {r['total_time']:<15.2f} | {r['final_loss']:<20.4e}"
        )
