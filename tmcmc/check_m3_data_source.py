...
if run_dir.exists():
    ...
    try:
        ...
        print(f"  [OK] 実行結果のデータを読み込み成功")
        print(f"  data_M3 shape: {data_M3_actual.shape}")
        print(f"  idx_M3 shape: {idx_M3_actual.shape}")
        print(f"  t_M3 shape: {t_M3_actual.shape}, range: [{t_M3_actual.min():.8f}, {t_M3_actual.max():.8f}]")
...
for i, sp in enumerate(config_M3["active_species"]):
    data_M3_generated[:, i] = phibar_M3[idx_sparse_M3, i] + rng_M3.standard_normal(exp_config_n_data) * exp_config_sigma_obs
...
if data_M3_actual is not None:
    ...
    if np.allclose(data_M3_generated, data_M3_actual, atol=1e-6):
        print(f"  [OK] 生成データと実行結果のデータが一致（M3から生成されている）")
    else:
        print(f"  [WARNING] 生成データと実行結果のデータが一致しない")
    ...
    if np.array_equal(idx_sparse_M3, idx_M3_actual):
        print(f"  [OK] インデックスが一致")
    else:
        print(f"  [WARNING] インデックスが一致しない")
...
print(f"  比率 (M3/M1): {(t_arr_M3.max() - t_arr_M3.min()) / (t_arr_M1.max() - t_arr_M1.min()):.4f}")
...
if max_diff_norm < 0.01:
    print(f"  [OK] 正規化時間はほぼ一致")
else:
    print(f"  [WARNING] 正規化時間に差がある")
