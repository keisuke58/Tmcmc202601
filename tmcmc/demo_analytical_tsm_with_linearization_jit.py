...
class BiofilmTSM_Analytical(BiofilmTSM):
    ...
    def __init__(self, solver, cov_rel=0.005, active_theta_indices=None,
                 use_complex_step=True, use_analytical=False,
                 theta_linearization=None, paper_mode=True):
        """
    ...
    def update_linearization_point(self, theta_new):
        ...
        """
        if theta_new.shape[0] != 14:
            raise ValueError(f"theta_new must have shape (14,), got {theta_new.shape}")
        
        # Store new linearization point
        old_theta = self.theta_linearization.copy() if self.theta_linearization is not None else None
    ...
    def solve_tsm(self, theta):
        ...
        x0 : ndarray
            ...
            Shape: (n_time, 10) for 5 species: [phi1..phi5, psi1..psi5]
        sigma2 : ndarray
        ...
        if is_at_linearization:
            ...
            if self._deterministic_solution_cached is not None:
                ...
                if hasattr(self.solver, 'solve_deterministic'):
                    ...
                    raise AttributeError("Solver must have either solve_deterministic or run_deterministic method")
        ...
        x1 = self._x1_cached  # Shape: (n_time, 10, n_active)
        
        # 2) 線形補正: x(θ) ≈ x(θ₀) + ∂x/∂θ|_{θ₀} · (θ - θ₀)
        delta_theta = theta - theta_center
        x0 = x0_center.copy()  # Shape: (n_time, 10)
        
        # Apply linear correction for each active parameter
        # x1[:, :, k] is (n_time, 10) - sensitivity of all states to parameter k
        ...
        # Shape: (n_time, 10) = sum over active parameters of (x1^2 * var_act)
        # x1 shape: (n_time, 10, n_active)
    ...
    def _compute_and_cache_x1(self, theta_center):
        ...
        # x0_center has one entry per time point (n_time, 10)
        ...
        x1_full = np.zeros((n_time, 10, len(self.active_idx)), dtype=np.float64)
        x1_full[1:, :, :] = x1_core
        x1_full[0, :, :] = x1_core[0, :, :]
    ...
    def verify_analytical_derivatives(self, theta=None, eps=1e-20):
        ...
        dG_analytical = AnalyticalDerivatives.compute_dG_dtheta_array(
            g_new, g_old, t, dt, theta, c, alpha,
            A, b_diag, 
            self.solver.Eta_vec, self.solver.Eta_phi_vec,
            self.active_idx
        )
    ...
    def _compute_G(self, g_new, g_old, dt, c, alpha, A, b_diag):
        ...
        if np.iscomplexobj(A):
            ...
            return compute_G_residual(
                g_new, g_old, dt, c, alpha, A, b_diag,
                self.solver.Eta_vec, self.solver.Eta_phi_vec, self.solver.Kp1
            )
    
    def _compute_G_python(self, g, g_old, dt, c, alpha, A, b_diag):
        """Fallback Python implementation for complex step."""
        N = 5
        G = np.zeros(10, dtype=g.dtype)
