...
idx_sparse_ref = select_sparse_data_indices(
    len(t_arr_M3_ref), exp_config.n_data, t_arr=t_arr_M3_ref
)
...
for i, sp in enumerate(config_M3["active_species"]):
    data_M3_ref[:, i] = (
        phibar_M3_ref[idx_sparse_ref, i]
        + rng_M3.standard_normal(exp_config.n_data) * exp_config.sigma_obs
    )
