...
for i, sp in enumerate(config["active_species"]):
    ...
    print(
        f"  Residual variance: {np.var(residuals_sp):.8f} (theoretical: {exp_config_sigma_obs**2})"
    )
    print(f"  Residual range: [{np.min(residuals_sp):.6f}, {np.max(residuals_sp):.6f}]")
    print(
        f"  Theoretical range (±3σ): [{-3*exp_config_sigma_obs:.6f}, {3*exp_config_sigma_obs:.6f}]"
    )
