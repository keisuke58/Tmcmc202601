...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--models", type=str, default=None, help="Comma-separated list of models to run (e.g. M1 or M1,M3)")

    # Pass-through overrides for case2 (optional)
    p.add_argument("--sigma-obs", type=float, default=None)
    p.add_argument("--no-noise", action="store_true", default=False, help="Generate data without noise")
    p.add_argument("--cov-rel", type=float, default=None)
    p.add_argument("--rho", type=float, default=None)
    p.add_argument("--aleatory-samples", type=int, default=None, help="Reporting-only: paper Nsamples for double-loop cost conversion")
    ...
    p.add_argument("--n-jobs", type=int, default=None, help="Number of parallel jobs for particle evaluation")
    p.add_argument("--use-threads", action="store_true", default=False, help="Use threads instead of processes")
...
def main() -> int:
    ...
    run_id = args.run_id or (datetime.now().strftime("%Y%m%d_%H%M%S") + f"_{args.mode}_seed{args.seed}")
    ...
    if args.linearization_enable_rom_threshold is not None:
        cmd += ["--linearization-enable-rom-threshold", str(args.linearization_enable_rom_threshold)]
