...
REPORT_STATUS_RE = re.compile(
    r"^- \*\*status\*\*: \*\*(?P<status>PASS|WARN|FAIL)\*\*", re.MULTILINE
)
...
def _classify(run_dir: Path, runs_root: Path) -> RunAudit:
    ...
    last_rc, last_end_ts, last_cmd, last_err, tb = (
        _parse_subprocess_log(subprocess_log)
        if has_subprocess
        else (None, "missing", "missing", "missing", False)
    )
...
def _move_failed(
    rows: list[RunAudit], runs_root: Path, failed_root: Path, apply: bool
) -> tuple[int, int]:
    moved = 0
    skipped = 0
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Include already-isolated _failed runs in the audit",
    )
    p.add_argument(
        "--md-out",
        type=str,
        default=None,
        help="Write markdown report to this path (otherwise prints)",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="Move FAIL runs under --failed-root",
    )
    p.add_argument(
        "--move-status",
        type=str,
    ...
    p.add_argument(
        "--failed-root",
        type=str,
        default="tmcmc/_runs/_failed",
        help="Destination root for failed run isolation",
    )
    p.add_argument(
        "--apply", action="store_true", default=False, help="Actually move (otherwise dry-run)"
    )
    ...
    if args.move_failed:
        ...
        for st in sorted(statuses):
            if st != "FAIL":
                ...
                tmp_rows = [
                    RunAudit(**{**r.__dict__, "status": "FAIL"}) if r.status == st else r
                    for r in rows
                ]
                m, s = _move_failed(
                    tmp_rows, runs_root=runs_root, failed_root=failed_root, apply=bool(args.apply)
                )
            else:
                m, s = _move_failed(
                    rows, runs_root=runs_root, failed_root=failed_root, apply=bool(args.apply)
                )
            moved += m
            skipped += s
        print(
