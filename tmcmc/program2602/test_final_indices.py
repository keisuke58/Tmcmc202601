...
if max_diff < 0.001:  # Allow small tolerance for integer index rounding
    ...
    print("    (Difference < 0.001, which is acceptable for integer indices)")
else:
...
print(
    f"Last index: {indices[-1]} (normalized: {t_normalized_positions[-1]:.6f}, expected: {expected_normalized_times[-1]:.6f})"
)
