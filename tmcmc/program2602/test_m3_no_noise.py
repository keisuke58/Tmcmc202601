...
print("  data_M3_no_noise vs phibar_M3[idx_sparse]:")
print(f"    Max difference: {max_diff:.2e}")
print(f"    Mean difference: {mean_diff:.2e}")

if max_diff < 1e-10:
    print("  ✅ PASS: 完全一致（期待通り）")
else:
    print("  ❌ FAIL: 不一致（予期しない）")
...
for i, sp in enumerate(config_M3["active_species"]):
    ...
    ax.scatter(
        ...
        facecolor="none",
        linewidth=2,
        label=f"Data phibar{sp+1} (no noise)",
        alpha=0.9,
        zorder=10,
        marker="x",
    )
...
plt.suptitle(
    "M3 No-Noise Test: Data points should exactly match the line", fontsize=14, fontweight="bold"
)
plt.tight_layout()
