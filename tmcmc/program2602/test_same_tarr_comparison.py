...
idx_sparse_M3_orig = select_sparse_data_indices(
    len(t_arr_M3_orig), exp_config_n_data, t_arr=t_arr_M3_orig
)
...
for i, sp in enumerate(config_M3["active_species"]):
    data_M3_orig[:, i] = (
        phibar_M3_orig[idx_sparse_M3_orig, i]
...
for i, sp in enumerate(config_M1["active_species"]):
    data_M1[:, i] = (
        phibar_M1[idx_sparse_M1, i]
        + rng_M1.standard_normal(exp_config_n_data) * exp_config_sigma_obs
    )
...
for j in range(x0_M3_orig.shape[1]):
    interp_func = interp1d(
        t_arr_M3_orig, x0_M3_orig[:, j], kind="linear", bounds_error=False, fill_value="extrapolate"
    )
...
idx_sparse_fixed = select_sparse_data_indices(
    len(t_arr_fixed), exp_config_n_data, t_arr=t_arr_fixed
)
...
for i, sp in enumerate(config_M3["active_species"]):
    data_M3_fixed[:, i] = (
        phibar_M3_interp[idx_sparse_fixed, i]
        + rng_M3.standard_normal(exp_config_n_data) * exp_config_sigma_obs
    )
...
if rmse_M3_fixed < rmse_M3_orig * 0.9:
    print("  [YES] 同じt_arrでズレが減った → 時間軸が主因の可能性が高い")
    print(
        f"  RMSE改善: {rmse_M3_orig:.8f} → {rmse_M3_fixed:.8f} ({100*(1-rmse_M3_fixed/rmse_M3_orig):.1f}%改善)"
    )
else:
    print("  [NO] 同じt_arrでもズレは変わらない → 時間軸以外が主因")
