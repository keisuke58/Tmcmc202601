...
for i, sp in enumerate(config["active_species"]):
    data[:, i] = (
        phibar_data_gen[idx_sparse, i]
        + rng.standard_normal(exp_config_n_data) * exp_config_sigma_obs
    )
...
plot_mgr.plot_TSM_simulation(
    t_arr, x0, config["active_species"], name, data, idx_sparse, phibar=phibar_data_gen
)
