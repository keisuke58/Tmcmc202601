...
def test_parallelization():
    ...
    for n_jobs in n_jobs_list:
        ...
        if max_diff > 1e-10:
            print(f"  ⚠️  警告: 結果に差分があります（並列化の問題の可能性）")
        else:
            print(f"  ✅ 結果は一致しています")
    ...
    for n_jobs in n_jobs_list:
        ...
        print(f"{n_jobs:<10} {r['parallel_time']:<12.2f} {r['sequential_time']:<12.2f} {r['speedup']:<12.2f} {status:<10}")
    ...
    if best_speedup > 1.5:
        print(f"✅ 並列化が効果的に動作しています")
        print(f"   推奨 n_jobs: {best_n_jobs} (高速化率: {best_speedup:.2f}x)")
    elif best_speedup > 1.1:
        print(f"⚠️  並列化は動作していますが、効果が限定的です")
        print(f"   推奨 n_jobs: {best_n_jobs} (高速化率: {best_speedup:.2f}x)")
        print(f"   原因: 計算時間が短すぎる、またはオーバーヘッドが大きい可能性")
    else:
        print(f"❌ 並列化が効果的に動作していません")
        print(f"   原因を調査してください:")
        print(f"   1. ProcessPoolExecutorが正しく動作しているか")
        print(f"   2. 関数がpickle可能か")
        print(f"   3. 計算時間が十分に長いか")
