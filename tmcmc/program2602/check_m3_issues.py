...
for model_name in models_to_test:
    ...
    print(f"   正規化方法: (t_arr - t_min) / (t_max - t_min)")
    ...
    if max_time_diff < 0.01:
        print(f"   [OK] 時間のずれは許容範囲内")
    else:
        print(f"   [WARNING] 時間のずれが大きい！")
    ...
    for i, sp in enumerate(config["active_species"]):
        data_with_noise[:, i] = phibar[idx_sparse, i] + rng.standard_normal(exp_config_n_data) * exp_config_sigma_obs
    
    # Check residuals
    residuals_with_noise = data_with_noise - phibar[idx_sparse, :]
    print(f"   Residuals mean: {np.mean(residuals_with_noise):.8f}")
    print(f"   Residuals std: {np.std(residuals_with_noise):.6f} (expected: {exp_config_sigma_obs})")
    print(f"   Residuals range: [{np.min(residuals_with_noise):.6f}, {np.max(residuals_with_noise):.6f}]")
    ...
    if max_diff < 1e-10:
        print(f"   [OK] no-noiseデータはphibarと完全一致")
    else:
        print(f"   [ERROR] no-noiseデータがphibarと一致しない！")
    ...
    if np.mean(second_half_residuals) > 1.5 * np.mean(first_half_residuals):
        print(f"   [WARNING] 後半の乖離が大きい！時間正規化の問題の可能性")
    else:
        print(f"   [OK] 前半と後半の乖離に大きな差なし")
    ...
    if model_name == "M1":
        ...
        if np.array_equal(m1_idx_sparse, idx_sparse):
            print(f"   [OK] データポイントインデックスはM1とM3で同じ")
        else:
            print(f"   [WARNING] データポイントインデックスがM1とM3で異なる")
        ...
        if np.allclose(m1_t_normalized[idx_sparse], t_normalized[idx_sparse], atol=1e-6):
            print(f"   [OK] 正規化時間はM1とM3で同じ")
        else:
            print(f"   [WARNING] 正規化時間がM1とM3で異なる")
            print(f"   Max difference: {np.max(np.abs(m1_t_normalized[idx_sparse] - t_normalized[idx_sparse])):.8f}")
