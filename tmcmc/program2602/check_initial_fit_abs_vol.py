...
for i in range(5):
    ...
    ax.scatter(
        (
            days * 1e-4 * (750 / days.max()) if days.max() > 0 else 0
        ),  # Map days to time? No, plot against indices or map time?
        # t_arr is time (0 to 0.075). days are 1, 3, 7.
        # t_idx maps days to steps. t_arr[t_idx] is the time.
        exp_data[:, i],
        label="Exp Data (Scaled)",
        color="red",
        marker="o",
    )
...
for i in range(5):
    ...
    print(
        f"Species {i}: Max Sim={max_sim:.4f}, Max Exp={max_exp:.4f}, Ratio={max_sim/max_exp if max_exp>0 else 0:.2f}"
    )

total_sim_max = np.max(total_vol_sim)
total_exp_max = np.max(total_vol_exp)
print(
    f"Total Vol: Max Sim={total_sim_max:.4f}, Max Exp={total_exp_max:.4f}, Ratio={total_sim_max/total_exp_max if total_exp_max>0 else 0:.2f}"
)
