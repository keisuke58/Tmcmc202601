...
def get_theta_true() -> np.ndarray:
    ...
    return np.array(
        [
            ...
            1.0,
            2.0,
            1.0,  # M3 cross-interaction block
        ],
        dtype=np.float64,
    )
...
if HAS_NUMBA:
    ...
    def _compute_Q_vector_numba(
        ...
        for i in range(4):
            if active_mask[i] == 1:
                term1 = (-2.0 * Kp1) / ((psi_new[i] - 1.0) ** 2 * psi_new[i] ** 3) - (2.0 * Kp1) / (
    ...
    def _compute_jacobian_numba(
        ...
        for i in range(4):
            if active_mask[i] == 1:
                v = phi_new[i]
                phi_p[i] = (Kp1 * (-4.0 + 8.0 * v)) / ((v**3) * (v - 1.0) ** 3) - (
                    Kp1 * (2.0 - 4.0 * v)
                ) * (3.0 / ((v**4) * (v - 1.0) ** 3) + 3.0 / ((v**3) * (v - 1.0) ** 4))
                v = psi_new[i]
                psi_p[i] = (4.0 * Kp1 * (3.0 - 5.0 * v + 5.0 * v**2)) / ((v**4) * (v - 1.0) ** 4)

        v0 = phi0_new
        phi0_p = (Kp1 * (-4.0 + 8.0 * v0)) / ((v0**3) * (v0 - 1.0) ** 3) - (
            Kp1 * (2.0 - 4.0 * v0)
        ) * (3.0 / ((v0**4) * (v0 - 1.0) ** 3) + 3.0 / ((v0**3) * (v0 - 1.0) ** 4))
        ...
        for i in range(4):
            if active_mask[i] == 1:
                ...
                K[i, i + 5] += (1.0 / Eta_vec[i]) * (
                    ...
                    (Interaction[i] + A[i, i] * phi_new[i] * psi_new[i])
...
def _compute_Q_vector_numpy(
    ...
    for i in range(4):
        if active_mask[i] == 1:
            phi_new[i] = (
                np.clip(phi_new[i].real, eps, 1.0 - eps) + 1j * phi_new[i].imag
                if np.iscomplexobj(phi_new)
                else np.clip(phi_new[i], eps, 1.0 - eps)
            )
            psi_new[i] = (
                np.clip(psi_new[i].real, eps, 1.0 - eps) + 1j * psi_new[i].imag
                if np.iscomplexobj(psi_new)
                else np.clip(psi_new[i], eps, 1.0 - eps)
            )
        else:
            phi_new[i] = 0.0
            psi_new[i] = 0.0
    ...
    phi0_new = (
        np.clip(phi0_new.real, eps, 1.0 - eps) + 1j * phi0_new.imag
        if np.iscomplexobj(phi0_new)
        else np.clip(phi0_new, eps, 1.0 - eps)
    )
    ...
    def _newton_step_jit(
        ...
        for _ in range(max_newton_iter):
            ...
            while step > 1e-4:
                ...
                Q_trial = _compute_Q_vector_numba(
                    phi_t,
                    phi0_t,
                    psi_t,
                    gamma_t,
    ...
    def run_deterministic(self, theta, show_progress: bool = False):
        ...
        if self.use_numba and HAS_NUMBA:
            t_arr, g_arr = _run_deterministic_jit(
                ...
                self.eps,
                self.Kp1,
                self.Eta_vec.astype(np.float64),
                self.Eta_phi_vec.astype(np.float64),
                self.c_const,
                self.alpha_const,
                self.phi_init,
                self.active_mask,
