...
def patch_biofilm_solver(verbose: bool = False):
    ...
    theta_test = np.array(
        [0.8, 2.0, 1.0, 0.1, 0.2, 1.5, 1.0, 2.0, 0.3, 0.4, 2.0, 1.0, 2.0, 1.0], dtype=np.complex128
    )
    ...
    if np.abs(np.imag(A[0, 0])) < 1e-40:
        raise RuntimeError("Complex imaginary part not propagated correctly in theta_to_matrices.")
