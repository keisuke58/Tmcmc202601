"""Fix plot data mismatch by ensuring phibar is computed consistently."""
...
plt.title("TSM Simulation (φ̄) - M1 (Corrected)", fontsize=14)
plt.xlim(0.0, 1.0)
plt.grid(True, alpha=0.3)
plt.legend(fontsize=10)
plt.tight_layout()

output_path = run_dir / "figures" / "TSM_simulation_M1_with_data_corrected.png"
...
# Verify data points match model
print("\nVerification:")
for i, sp in enumerate(config["active_species"]):
    ...
    print(
        f"Species {sp+1}: residual std = {np.std(residuals):.6f}, max |residual| = {np.max(np.abs(residuals)):.6f}"
    )
