            ...
            if run_log.exists():
                try:
                    ...
                    if last_update_min < 5:
                        print(f"   [{datetime.now().strftime('%H:%M:%S')}] 実行中... (経過: {elapsed_min:.1f}分, 最終更新: {last_update_min:.1f}分前)")
                    else:
                        print(f"   [{datetime.now().strftime('%H:%M:%S')}] 実行中... (経過: {elapsed_min:.1f}分, 最終更新: {last_update_min:.1f}分前) ⚠️ 更新が遅い")
                except Exception:
                    pass
...
def review_1000_results(run_id: str, runs_root: Path) -> dict:
    ...
    if review["ess_min"] is not None:
        if review["ess_min"] < 100:
            ...
            review["issues"].append(f"ESS_min ({review['ess_min']:.1f}) < 100 - 5000パーティクルで改善が期待されます")
            review["improvements"].append("ESS_minを100以上にするため、パーティクル数を5000に増やす")
        elif review["ess_min"] < 300:
            review["recommend_5000"] = True
            review["improvements"].append(f"ESS_min ({review['ess_min']:.1f}) < 300 - 5000パーティクルでESS_min ≥ 300を目標")
        else:
    ...
    if review["rmse"] is not None and review["rmse"] > 0.05:
        review["improvements"].append(f"RMSE ({review['rmse']:.6f}) > 0.05 - 5000パーティクルで精度向上が期待されます")
    
    if review["accept_rate_mean"] is not None:
        if review["accept_rate_mean"] < 0.3:
            review["issues"].append(f"受容率 ({review['accept_rate_mean']:.3f}) < 0.3 - 低すぎる可能性")
        elif review["accept_rate_mean"] > 0.8:
            review["issues"].append(f"受容率 ({review['accept_rate_mean']:.3f}) > 0.8 - 高すぎる可能性（探索不足）")
...
"""
    ...
    if review.get('ess_min') is not None:
        ess_eval = "✅ 良好" if review['ess_min'] >= 300 else ("⚠️ やや低い" if review['ess_min'] >= 100 else "❌ 不足")
        content += f"| ESS_min | {review['ess_min']:.2f} | {ess_eval} |\n"
    
    if review.get('rmse') is not None:
        rmse_eval = "✅ 良好" if review['rmse'] < 0.03 else ("⚠️ 中程度" if review['rmse'] < 0.05 else "❌ 大きい")
...
def check_target_achieved(review: Dict, target_criteria: Dict) -> Tuple[bool, List[str]]:
    ...
    if "status" in target_criteria:
        ...
        if review.get("status") != target_status:
            achieved = False
            unmet.append(f"ステータス: {review.get('status', 'UNKNOWN')} != {target_status} (目標: {target_status})")
...
def determine_next_config(review: Dict, iteration: int, max_iterations: int) -> Dict:
    ...
    if review.get("ess_min") is not None:
        if review["ess_min"] < 100:
            ...
            print(f"  🔧 微調整: ESS_min ({review['ess_min']:.1f}) < 100 → パーティクル数{config['n_particles']}, ステージ数{config['n_stages']}")
            ...
            print(f"  🔧 微調整: ESS_min ({review['ess_min']:.1f}) < 200 → パーティクル数{config['n_particles']}, ステージ数{config['n_stages']}")
        elif review["ess_min"] < 250:
            # ESS_minが目標に近い → ステージ数を少し増やす
            config["n_stages"] = max(config["n_stages"], 35)
            print(f"  🔧 微調整: ESS_min ({review['ess_min']:.1f}) < 250 → ステージ数{config['n_stages']}")
    ...
    if review.get("rmse") is not None:
        if review["rmse"] > 0.05:
            ...
            print(f"  🔧 微調整: RMSE ({review['rmse']:.6f}) > 0.05 → ステージ数{config['n_stages']}, Mutation steps {config['n_mutation_steps']}")
        elif review["rmse"] > 0.03:
            # RMSEがやや高い → ステージ数とMutation stepsを増やす
            config["n_stages"] = max(config["n_stages"], 40)
            config["n_mutation_steps"] = max(config["n_mutation_steps"], 7)
            print(f"  🔧 微調整: RMSE ({review['rmse']:.6f}) > 0.03 → ステージ数{config['n_stages']}, Mutation steps {config['n_mutation_steps']}")
        elif review["rmse"] > 0.025:
            # RMSEが目標に近い → ステージ数を少し増やす
            config["n_stages"] = max(config["n_stages"], 35)
            print(f"  🔧 微調整: RMSE ({review['rmse']:.6f}) > 0.025 → ステージ数{config['n_stages']}")
    ...
    if review.get("accept_rate_mean") is not None:
        if review["accept_rate_mean"] < 0.2:
            ...
            print(f"  🔧 微調整: 受容率 ({review['accept_rate_mean']:.3f}) < 0.2 → Mutation steps {config['n_mutation_steps']}")
            ...
            print(f"  🔧 微調整: 受容率 ({review['accept_rate_mean']:.3f}) > 0.9 → Mutation steps {config['n_mutation_steps']}")
    ...
    if not review.get("converged", False):
        ...
        config["n_stages"] = max(config["n_stages"], 40)
        print(f"  🔧 微調整: 未収束 → チェーン数{config['n_chains']}, ステージ数{config['n_stages']}")
    
    return config


def run_5000_particles(runs_root: Path, base_dir: Path, config: Optional[Dict] = None) -> Optional[str]:
    """
    5000パーティクル実行を開始
    ...
    cmd = [
        ...
        "--n-stages", str(config["n_stages"]),
        "--n-mutation-steps", str(config["n_mutation_steps"]),
        "--n-chains", str(config["n_chains"]),
        "--max-delta-beta", str(config["max_delta_beta"]),
        "--target-ess-ratio", str(config["target_ess_ratio"]),
        "--seed", "42",
        "--run-id", run_id
    ]
    ...
    try:
        ...
        print(f"進捗確認コマンド:")
        print(f"  cd \"{base_dir}\"")
        print(f"  python tmcmc\\check_running.py")
        print()
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=300.0,
        help="目標ESS_min（デフォルト: 300）"
    )
    parser.add_argument(
        "--target-rmse-max",
        type=float,
        default=0.02,
        help="目標RMSE最大値（デフォルト: 0.02）"
    )
    parser.add_argument(
        "--target-status",
        type=str,
    ...
    if not args.skip_review:
        ...
        if review['ess_min'] is not None:
            ...
            if review['ess_min'] < 100:
                print(f"  ⚠️ ESS_min < 100 - 5000パーティクルで改善が期待されます")
            elif review['ess_min'] < 300:
                print(f"  ⚠️ ESS_min < 300 - 5000パーティクルでESS_min ≥ 300を目標")
            else:
                print(f"  ✅ ESS_min ≥ 300 - 十分な品質")
        
        if review['rmse'] is not None:
            print(f"RMSE: {review['rmse']:.6f}")
            if review['rmse'] < 0.03:
                print(f"  ✅ RMSE < 0.03 - 良好な精度")
            elif review['rmse'] < 0.05:
                print(f"  ⚠️ RMSE < 0.05 - 中程度の精度")
            else:
                print(f"  ⚠️ RMSE ≥ 0.05 - 改善の余地あり")
        ...
        print(f"\n{'='*80}")
        if review['recommend_5000']:
            print(f"✅ 5000パーティクル実行を推奨します")
        else:
            print(f"⚠️ 5000パーティクル実行は任意です（現在の結果で十分な品質）")
        print(f"{'='*80}")
        
        if not review['recommend_5000']:
            print(f"\n⚠️ 5000パーティクル実行は推奨されませんが、実行を続行しますか？")
            print(f"   5秒後に自動的に続行します...")
            time.sleep(5)
            print(f"   続行します\n")
    ...
    if args.iterative:
        print(f"\n{'='*80}")
        print(f"🔄 反復実行モード: 目標精度に達するまで繰り返します")
        print(f"{'='*80}")
        print(f"目標基準:")
        ...
        while iteration < args.max_iterations:
            ...
            if iteration >= args.max_iterations:
                ...
                print(f"最終実行ID: {current_run_id}")
                print(f"目標精度に達していませんが、実行を終了します")
                return 1
            ...
            try:
                ...
                print(f"  ✅ 設定変更履歴を保存: config_history.json")
            except Exception as e:
            ...
            if saved_files_next:
                ...
                print(f"  ⚠️ 主要ファイルがまだ保存されていません（数秒後に再確認）")
        ...
        if run_id_5000:
            print(f"✅ 5000パーティクル実行を開始しました")
            print(f"   実行ID: {run_id_5000}")
            return 0
        else:
            print(f"❌ 5000パーティクル実行の開始に失敗しました")
            return 1
