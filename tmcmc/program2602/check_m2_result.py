...
def run_simulation(theta, model_config_name="M2"):
    ...
    if theta.shape[0] == len(active_indices):
        ...
        print(
            f"Warning: Theta shape {theta.shape} does not match active indices length {len(active_indices)}. Assuming full vector or mismatch."
        )
        if theta.shape[0] == 20:
            theta_full = theta

    # Setup solver
    solver = BiofilmNewtonSolver5S(active_species=active_species, use_numba=HAS_NUMBA)
...
def main():
    ...
    for i, sp_idx in enumerate(active_species):
        ...
        plt.plot(
            ...
            "--",
            color=colors[sp_idx],
            label=f"Est S{sp_idx+1} (RMSE={rmse:.4f})",
        )
