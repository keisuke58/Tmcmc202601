...
if MAP_M1 is None:
    ...
    print(f"  実行が完了するまで待つか、別の実行結果を使用してください")
    sys.exit(1)
...
print(f"  [NOTE] M2の推定結果は利用できませんでした")
print(f"  実行が完了したら、M2の推定誤差も確認してください")
...
if np.max(relative_error_MAP_M1) > 0.1:
    print(f"  [WARNING] M1の相対誤差が大きい（>10%）")
    print(f"    最大相対誤差: {np.max(relative_error_MAP_M1):.2%}")
else:
    print(f"  [OK] M1の相対誤差は小さい（<10%）")
    print(f"    最大相対誤差: {np.max(relative_error_MAP_M1):.2%}")
...
print(f"  4. 実行が完了したら、実際のM2推定誤差も確認する")

print(f"\n{'='*80}")
print("確認完了")
print(f"{'='*80}")
print(f"\n[NOTE] M2の推定結果は利用できませんでした")
print(f"  実行が完了したら、実際のM2推定誤差も確認してください")
