...
def main():
    ...
    t_data = t[idx_sparse]
    for i in range(5):
        plt.scatter(t_data, data[:, i], color=colors[i], marker='o', alpha=0.3, label=f"Data {labels[i]}")
        
    # Plot Simulation
    for i in range(5):
        plt.plot(t, phibar[:, i], color=colors[i], linestyle='-', linewidth=2, label=f"Sim {labels[i]}")
