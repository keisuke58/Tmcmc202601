...
for model_name in ["M1", "M2", "M3"]:
    ...
    for idx, sigma in enumerate(sigma_values):
        ...
        for i, sp in enumerate(config["active_species"]):
            # Plot line
            ax.plot(t_normalized, phibar[:, i], 
                   label=f"phibar{sp+1}" if i == 0 else "", 
                   linewidth=1.5, alpha=0.7, color=plt.cm.tab10(i))
            
            # Plot data points
            ax.scatter(t_obs_normalized, data[:, i], s=30, 
                      alpha=0.8, zorder=10, marker='x', 
                      color=plt.cm.tab10(i), linewidths=1.5)
    ...
    for species_idx, sp in enumerate(config["active_species"]):
        ...
        for idx, sigma in enumerate(sigma_values):
            ...
            # Plot line
            ax.plot(t_normalized, phibar[:, species_idx], 
                   linewidth=2, alpha=0.7, color='blue', label='Model')
            
            # Plot data points
            ax.scatter(t_obs_normalized, data[:, species_idx], s=50, 
                      alpha=0.9, zorder=10, marker='x', 
                      color='red', linewidths=2, label='Data')
        ...
        plt.suptitle(f"{model_name} - Species {sp} (phibar{sp+1}): Sigma Comparison", 
...
for model_idx, model_name in enumerate(["M1", "M2", "M3"]):
    ...
    ax.loglog(sigmas_plot, max_diffs, 'o-', linewidth=2, markersize=8, 
             label='Max difference', color='red')
    ax.loglog(sigmas_plot, mean_diffs, 's-', linewidth=2, markersize=8, 
             label='Mean difference', color='blue')
    
    # Add reference line (sigma itself)
    ax.loglog(sigmas_plot, sigmas_plot, '--', linewidth=1, alpha=0.5, 
             color='gray', label='σ (reference)')
