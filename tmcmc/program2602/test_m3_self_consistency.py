...
print(f"  data vs phibar_dense[idx_sparse]:")
print(f"    Max difference: {max_diff:.2e}")
print(f"    Mean difference: {mean_diff:.2e}")

if max_diff < 1e-10:
    print(f"  ✅ PASS: data完全一致（期待通り）")
else:
    print(f"  ❌ FAIL: data不一致（予期しない）")
    print(f"     → これはバグです！")
...
for i, sp in enumerate(config_M3["active_species"]):
    ...
    ax.plot(t_normalized, phibar_dense[:, i], 
            label=f"φ̄{sp+1} (model)", linewidth=2, color='blue', alpha=0.7)
    
    # Plot data points
    ax.scatter(t_obs_normalized, data[:, i], s=60, edgecolor='k', 
              label=f"Data φ̄{sp+1}", alpha=0.9, zorder=10, color='red', marker='x')
    
    # Plot model values at observation points (should match data exactly)
    ax.scatter(t_obs_normalized, phibar_dense[idx_sparse, i], s=30, 
              marker='o', color='green', alpha=0.5, zorder=9, 
