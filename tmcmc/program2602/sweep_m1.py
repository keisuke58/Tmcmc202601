...
def validate_environment() -> None:
    ...
    print("  [OK] run_pipeline.py found")
    ...
    try:
        ...
        print("  [OK] --runs-root flag supported")
    ...
    if report.exists():
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                exit_code = subprocess.call(cmd, cwd=str(root_dir), timeout=timeout_seconds)
                ...
                if exit_code == 0:
                    ...
                    if attempt < max_retries:
                        ...
                        print(
                            f"[retry] {run_id} attempt {attempt + 1}/{max_retries} after {wait_time}s"
                        )
                        time.sleep(wait_time)
                    else:
                        print(
                            f"[fail] {run_id} (exit_code={exit_code} after {max_retries + 1} attempts)"
                        )
...
def select_best(summary_csv: Path) -> None:
    ...
    print(
        f"BEST run_id={best.get('run_id')} status={best.get('status')} "
...
def main():
    ...
    parser.add_argument("--n-stages", type=int, default=30, help="Number of stages (default: 30)")
    parser.add_argument(
        "--max-jobs", type=int, default=8, help="Maximum parallel jobs (default: 8)"
    )
    parser.add_argument("--mode", type=str, default="debug", help="Mode (default: debug)")
    parser.add_argument(
        "--debug-level", type=str, default="MINIMAL", help="Debug level (default: MINIMAL)"
    )
    parser.add_argument("--models", type=str, default="M1", help="Models (default: M1)")
    parser.add_argument(
        "--sweep-prefix", type=str, default=None, help="Sweep prefix (default: auto-generated)"
    )
    parser.add_argument(
        "--runs-root", type=str, default=None, help="Runs root directory (default: tmcmc/_runs)"
    )
    parser.add_argument(
        "--max-retries", type=int, default=2, help="Maximum retries per job (default: 2)"
    )
    parser.add_argument(
