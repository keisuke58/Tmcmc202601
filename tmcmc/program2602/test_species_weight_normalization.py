...
for i, sp in enumerate(config_M3["active_species"]):
    data_M3[:, i] = phibar_M3[idx_sparse_M3, i] + rng_M3.standard_normal(exp_config_n_data) * exp_config_sigma_obs
...
def log_likelihood_standard(mu, data, sigma_obs):
    ...
    logL = -0.5 * np.sum(residuals**2 / var_total) - 0.5 * len(residuals) * np.log(2 * np.pi * var_total)
    return logL
...
def log_likelihood_normalized(mu, data, sigma_obs, normalize_by_std=False):
    ...
    logL = -0.5 * np.sum(residuals_norm**2 / var_total_norm) - 0.5 * len(residuals_norm) * np.log(2 * np.pi * var_total_norm)
    return logL
...
for i, sp in enumerate(config_M3["active_species"]):
    ...
    sigma_obs_sp = exp_config_sigma_obs * (data_std / np.mean(np.abs(data_sp))) if np.mean(np.abs(data_sp)) > 0 else exp_config_sigma_obs
...
for i, sp in enumerate(config_M3["active_species"]):
    ...
    logL_sp_norm = log_likelihood_normalized(mu_sp, data_sp, exp_config_sigma_obs, normalize_by_std=True)
    ...
    print(f"    Normalized contribution: {logL_sp_norm:.4f} ({100*logL_sp_norm/logL_normalized:.1f}%)")
...
if ratio_std > 2.0:
    print(f"  [WARNING] 標準尤度でspecies間の差が大きい（一部speciesが支配）")
if ratio_norm < ratio_std * 0.5:
    print(f"  [OK] 正規化によりspecies間の差が縮小")
