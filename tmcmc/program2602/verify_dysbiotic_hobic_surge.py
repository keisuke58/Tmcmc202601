...
def main():
    ...
    data_abs, t_days, sigma_obs, phi_init_exp, metadata = load_experimental_data(
        DATA_5SPECIES_ROOT, condition=condition, cultivation=cultivation
    )
    ...
    for i in range(5):
        ...
        # Plot simulation (line)
        plt.plot(
            t_plot_days, g_arr_eval_hyp[:, i], color=species_colors[i], linestyle="--", linewidth=2
        )

    plt.xlabel("Time (Days)")
    plt.title(
        f"Hypothetical (Positive Interaction)\nVei->Pg: {theta_hyp[18]}, Fn->Pg: {theta_hyp[19]}"
    )
    plt.grid(True, alpha=0.3)
    ...
    print("Timepoint: Day 21")
    print(
        f"P. gingivalis (Red): Exp = {pg_exp:.4f}, Sim = {pg_sim:.4f}, Diff = {pg_sim - pg_exp:.4f}"
    )
    print(
        f"F. nucleatum (Purple): Exp = {fn_exp:.4f}, Sim = {fn_sim:.4f}, Diff = {fn_sim - fn_exp:.4f}"
    )
    print(
        f"Veillonella (Gold):  Exp = {vei_exp:.4f}, Sim = {vei_sim:.4f}, Diff = {vei_sim - vei_exp:.4f}"
    )
