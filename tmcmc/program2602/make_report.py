...
def _collect_model_metrics(
    ...
    if theta0_df is not None and "step_norm" in theta0_df.columns:
        ...
        try:
            step = (
                pd.to_numeric(theta0_df["step_norm"], errors="coerce")
                if pd is not None
                else theta0_df["step_norm"]
            )
...
def _overall_status(per_model: Dict[str, ModelKeyMetrics]) -> Tuple[str, List[str]]:
    ...
    acc_mean_all = [
        m.accept_rate_mean for m in per_model.values() if m.accept_rate_mean is not None
    ]
...
def _plot_timeseries_assets(
    ...
    for m in per_model.keys():
        ...
        for chain_id, g in df.groupby("chain"):
            plt.plot(
                g["stage"], g["accept_rate"], marker="o", label=f"chain {chain_id}", linewidth=1.5
            )
        plt.ylim(0.0, 1.0)
        plt.xlabel("stage")
    ...
    for m in per_model.keys():
        ...
        plt.axhline(
            WARN_ROM_ERROR_MAX, linestyle="--", linewidth=1, color="orange", alpha=0.7, label="WARN"
        )
        plt.axhline(
            FAIL_ROM_ERROR_MAX, linestyle="--", linewidth=1, color="red", alpha=0.7, label="FAIL"
        )
    ...
    for m in per_model.keys():
        ...
        df2 = df.copy()
        df2["step_norm"] = (
            pd.to_numeric(df2["step_norm"], errors="coerce") if pd is not None else df2["step_norm"]
        )
        plt.figure(figsize=(7, 3))
        n_lines = 0
        for chain_id, g in df2.groupby("chain"):
            ...
            if gg.empty:
                continue
            plt.plot(
                gg["update"], gg["step_norm"], marker="o", label=f"chain {chain_id}", linewidth=1.5
            )
            n_lines += 1
        plt.xlabel("linearization update")
        plt.ylabel("||Δθ0||")
    ...
    for m in per_model.keys():
        ...
        try:
            ...
            plt.axhline(
                WARN_ESS_MIN, linestyle="--", linewidth=1, color="orange", alpha=0.7, label="WARN"
            )
            plt.axhline(
                FAIL_ESS_MIN, linestyle="--", linewidth=1, color="red", alpha=0.7, label="FAIL"
            )
...
def _render_report(
    ...
    comp_headers = [
        ...
        "rmse_total(MAP)",
        "mae_total(MAP)",
        "rom_error_MAP_vs_FOM",
        "rom_error_MEAN_vs_FOM",
    ]
    ...
    acc_mean_all = [
        per_model[m].accept_rate_mean for m in models if per_model[m].accept_rate_mean is not None
    ]
    rom_last_all = [
        per_model[m].rom_error_last for m in models if per_model[m].rom_error_last is not None
    ]
    ess_min = min(ess_min_all) if ess_min_all else None
    acc_mean = min(acc_mean_all) if acc_mean_all else None
    rom_last = max(rom_last_all) if rom_last_all else None
    ...
    if ess_min is not None and ess_min < WARN_ESS_MIN:
        ...
        next_lines.append(
            "  - Consider loosening likelihood (e.g., larger `--sigma-obs`) for exploration, then tighten."
        )
    if any(
        (per_model[m].beta_final is not None and per_model[m].beta_final < 0.999) for m in models
    ):
        next_lines.append("- **If beta does not reach 1.0 (not fully tempered to posterior)**:")
        next_lines.append(
            "  - Increase `--n-stages` and/or adjust ESS target so β can progress to 1.0."
        )
    if acc_mean is not None and acc_mean < 0.1:
        next_lines.append("- **If acceptance rate is low**:")
        next_lines.append(
            "  - Reduce proposal scale (or lower mutation scale inside TMCMC) and/or increase mutation steps."
        )
        next_lines.append(
            "  - Check for numerical issues (NaN/Inf) by using `--debug-level ERROR` or `VERBOSE`."
        )
    if rom_last is not None and rom_last > WARN_ROM_ERROR_MAX:
        next_lines.append("- **If ROM error is high**:")
        next_lines.append(
            "  - Enable/strengthen linearization updates (observation-based update) and ensure updates actually happen."
        )
        next_lines.append(
            "  - Increase `--cov-rel` only if it represents uncertainty; otherwise reduce step sizes / update frequency."
        )
    ...
    md.append(
        f"- **tmcmc**: n_particles={tmcmc_cfg.get('n_particles','missing')}, n_stages={tmcmc_cfg.get('n_stages','missing')}, n_mutation_steps={tmcmc_cfg.get('n_mutation_steps','missing')}, n_chains={tmcmc_cfg.get('n_chains','missing')}"
    )
    md.append(
        f"- **experiment**: n_data={exp_cfg.get('n_data','missing')}, sigma_obs={exp_cfg.get('sigma_obs','missing')}, cov_rel={exp_cfg.get('cov_rel','missing')}"
    )
    md.append("")
    ...
    md.append("---")
    md.append(
        f"_Generated by `tmcmc/make_report.py` at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}_"
    )
    md.append("")
    return "\n".join(md)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    g.add_argument(
        "--run-dir", type=str, default=None, help="Run directory path (tmcmc/_runs/<run_id>)"
    )
    g.add_argument("--run-id", type=str, default=None, help="Run id under --runs-root")
    p.add_argument(
        "--runs-root", type=str, default="tmcmc/_runs", help="Root directory containing runs"
    )
