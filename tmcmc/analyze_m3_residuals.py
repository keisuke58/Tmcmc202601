...
if not np.array_equal(idx_M3, select_sparse_data_indices(len(t_arr_M3), len(data_M3), t_arr=t_arr_M3)):
    print(f"  [WARNING] インデックスが一致しない可能性")
...
for i, sp in enumerate(config_M3["active_species"]):
    ...
    print(f"  1σ以内: {n_within_1sigma}/{len(residuals_sp)} ({100*n_within_1sigma/len(residuals_sp):.1f}%)")
    print(f"  2σ以内: {n_within_2sigma}/{len(residuals_sp)} ({100*n_within_2sigma/len(residuals_sp):.1f}%)")
    print(f"  3σ以内: {n_within_3sigma}/{len(residuals_sp)} ({100*n_within_3sigma/len(residuals_sp):.1f}%)")
    
    # Check for systematic bias
    if abs(np.mean(residuals_sp)) > 2 * sigma_obs / np.sqrt(len(residuals_sp)):
        print(f"  [WARNING] 系統的なバイアスが検出されました（平均残差が大きい）")
    ...
    if late_mean > 1.5 * early_mean:
        print(f"  [WARNING] 後半の残差が大きい（時間依存のバイアス）")
...
# Species comparison
print(f"\nSpecies間の比較:")
rmse_per_species = [np.sqrt(np.mean(residuals[:, i]**2)) for i in range(len(config_M3["active_species"]))]
...
if ratio > 2.0:
    ...
    print(f"  [WARNING] Species間の差が大きい")
    print(f"    最悪: Species {config_M3['active_species'][worst_species_idx]} (RMSE: {max_rmse:.8f})")
    print(f"    最良: Species {config_M3['active_species'][best_species_idx]} (RMSE: {min_rmse:.8f})")
...
stats = {
    ...
    "mean_residuals": [float(np.mean(residuals[:, i])) for i in range(len(config_M3["active_species"]))],
    "std_residuals": [float(np.std(residuals[:, i])) for i in range(len(config_M3["active_species"]))],
    "max_abs_residuals": [float(np.max(np.abs(residuals[:, i]))) for i in range(len(config_M3["active_species"]))],
