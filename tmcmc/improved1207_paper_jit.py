...
def _compute_Q_vector_numpy(phi_new, phi0_new, psi_new, gamma_new,
    ...
    eps = 1e-12
    phi_new = np.clip(phi_new, eps, 1.0 - eps)
    psi_new = np.clip(psi_new, eps, 1.0 - eps)
    phi0_new = np.clip(phi0_new, eps, 1.0 - eps)
    ...
    def _newton_step_jit(g_prev, dt, Kp1, Eta_vec, Eta_phi_vec, c_val, alpha_val, A, b_diag,
        ...
        for _ in range(max_newton_iter):
            ...
            Q = _compute_Q_vector_numba(phi_new, phi0_new, psi_new, gamma_new,
                                        ...
                                        dt, Kp1, Eta_vec, Eta_phi_vec,
                                        c_val, alpha_val, A, b_diag)
            # write back clipped values so the iteration is consistent with kernels
            g_new[0:4] = phi_new
            g_new[4] = phi0_new
            g_new[5:9] = psi_new
            g_new[9] = gamma_new
...
class BiofilmNewtonSolver:
    ...
    def __init__(
        ...
        # Supported forms:
        # - {"switch_time": 0.5, "alpha_before": 0.0, "alpha_after": 50.0}
        # - {"switch_step": 500, "alpha_before": 0.0, "alpha_after": 50.0}
        # If absent/None, antibiotics are constant = alpha_const.
    ...
    def alpha(self, t: float) -> float:
        ...
        try:
            ...
            # Fail-safe: fall back to constant antibiotics
            return float(self.alpha_const)
    ...
    def run_deterministic(self, theta, show_progress: bool = False):
        ...
        if self.use_numba and HAS_NUMBA:
            ...
            for i in range(4):
                mask[i] = 1 if (i in self.active_species) else 0
            t_arr, g_arr = _run_deterministic_jit(
                ...
                self.c_const, self.alpha_const, self.phi_init, mask
            )
            return t_arr, g_arr
