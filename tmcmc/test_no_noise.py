...
for i, sp in enumerate(config["active_species"]):
    data_with_noise[:, i] = phibar[idx_sparse, i] + rng.standard_normal(exp_config_n_data) * exp_config.sigma_obs
...
for i, sp in enumerate(config["active_species"]):
    if exp_config.no_noise:
        ...
        data_no_noise[:, i] = phibar[idx_sparse, i] + rng.standard_normal(exp_config_n_data) * exp_config.sigma_obs
...
plot_mgr.plot_TSM_simulation(t_arr, x0, config["active_species"], "M1_with_noise", data_with_noise, idx_sparse, phibar=phibar)
plot_mgr.plot_TSM_simulation(t_arr, x0, config["active_species"], "M1_no_noise", data_no_noise, idx_sparse, phibar=phibar)
