...
def plot_pbox_comparison(
    ...
    for i, (name, y_pos) in enumerate(zip(param_names, y)):
        ...
        ax.barh(
            y_pos, prior_max - prior_min, left=prior_min, 
            height=height, color='lightgray', edgecolor='gray', alpha=0.5,
        ...
        ax.barh(
            y_pos, post_max - post_min, left=post_min,
            height=height * 0.6, color='skyblue', edgecolor='blue', alpha=0.9,
        ...
        if theta_true is not None:
            val = theta_true[i]
            ax.vlines(
                val, y_pos - height/2, y_pos + height/2, 
        ...
        if theta_map is not None:
             val = theta_map[i]
             ax.plot(
                 val, y_pos, 'o', color='green', markersize=5,
