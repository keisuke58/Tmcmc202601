        ...
        # Priority 2: Checkpoint samples (for ongoing runs)
            ...
            try:
                ...
                if isinstance(chains, list) and len(chains) > 0:
                    ...
                    logger.info(
                        f"Loaded checkpoint samples (Stage {ckpt.get('stage')}) from {run_dir}"
                    )
                else:
...
def run_simulation(
    config: Dict, theta_full: np.ndarray, phi_init_override: Optional[np.ndarray] = None
) -> tuple:
    ...
    t_arr, phi_res, _, _ = solver.solve(theta_full, steps=sim_steps)
...
def plot_subplot(ax, run_dir: Path, title: str, condition: str):
    ...
    if "samples" in data:
        ...
        for theta_sample in samples_subset:
            t_arr, phibar = run_simulation(
                data["config"], theta_sample, phi_init_override=phi_init_override
            )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="combined_summary.png", help="Output filename"
    )
