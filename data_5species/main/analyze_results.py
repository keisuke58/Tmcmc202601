...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default=None, help="Output directory (default: run_dir/analysis)")
    parser.add_argument("--n-samples", type=int, default=100, help="Number of samples for uncertainty quantification")
...
def run_uncertainty_quantification(samples, map_data, config, n_samples=100):
    ...
    solver_kwargs = {
        ...
        "phi_init": np.array(config["phi_init"]) if isinstance(config.get("phi_init"), list) else config.get("phi_init", 0.01),
        "use_numba": True
    }
...
def main():
    ...
    try:
        uq_results, raw_history = run_uncertainty_quantification(samples, map_data, config, args.n_samples)
