...
from tmcmc.utils import save_json, save_npy
...
def parse_args():
    parser = argparse.ArgumentParser(description="Global Refinement for Biofilm Models (Linearized)")
    parser.add_argument("--run-id", type=str, required=True, help="Run ID of the completed estimation")
    parser.add_argument("--output-root", type=str, default=None, help="Root directory for outputs")
    parser.add_argument("--refine-range", type=float, default=0.2, help="Relative range for refinement prior (default: 0.2 = +/- 20%%)")
    parser.add_argument("--n-particles", type=int, default=1000, help="Number of particles for refinement")
    ...
    parser.add_argument("--sigma-obs", type=float, default=0.001, help="Observation noise (sigma_obs)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--n-jobs", type=int, default=None, help="Number of parallel jobs")
    parser.add_argument("--debug-level", type=str, default="INFO", choices=["DEBUG", "INFO", "WARNING", "ERROR"])
    parser.add_argument("--no-linearization", action="store_true", help="Disable linearization (use full model, slower)")
    
    # ★ New Argument for Model Selection
    parser.add_argument("--model", type=str, default="5s", choices=["2s", "4s", "5s"], 
                        help="Model type: 2s (Species 1-2), 4s (Species 1-4), 5s (Species 1-5)")
    parser.add_argument("--stage", type=str, default=None, choices=["M1", "M2", "M3", "M4", "M5"],
                        help="Specific stage to run (M1..M5). Overrides --model if set.")
    parser.add_argument("--mode", type=str, default="refine", choices=["refine", "stage"],
...
def main():
    ...
    if data.shape[1] > len(active_species):
        if max(active_species) < data.shape[1]:
             ...
             logger.warning(f"Data shape {data.shape} vs Active Species {active_species}. Slicing first {len(active_species)} columns.")
    ...
    if data.shape[1] > len(active_species):
        if max(active_species) < data.shape[1]:
            ...
            logger.warning(f"Data shape {data.shape} vs Active Species {active_species}. Cannot slice by index. Using first {len(active_species)} columns.")
            data_for_plots = data[:, :len(active_species)]
    
    plot_mgr.plot_TSM_simulation(
        t_fit, x_fit, active_species, "Refined_MAP_Fit", 
        data_for_plots, idx_sparse
    )
    ...
    plot_mgr.plot_pairplot_posterior(
        samples, theta_true_sub, refined_map, refined_mean,
        param_names, f"Refined_{args.stage or args.model}"
    )
    ...
    if n_draws > 0:
            ...
            if len(t_s) != len(t_fit) or not np.allclose(t_s, t_fit):
                if HAS_SCIPY:
                    ...
                    for j in range(x_s.shape[1]):
                        interp_func = interp1d(t_s, x_s[:, j], kind='linear', bounds_error=False, fill_value='extrapolate')
        ...
        plot_mgr.plot_posterior_predictive_band(
            t_fit, phibar_samples, active_species, f"Refined_{args.stage or args.model}",
            data=data_for_plots, idx_sparse=idx_sparse,
