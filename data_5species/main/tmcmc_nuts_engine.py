...
def tmcmc_engine(
    ...
    max_delta_beta: Optional[float] = None,
    seed: int = 42,
    ...
    while beta < 1.0 and stage < max_stages:
        ...
        def compute_ess(db):
            log_w = db * logL
            log_w = log_w - np.max(log_w)  # shift for numerical stability
            log_w = np.clip(log_w, -700, 0)
            w = np.exp(log_w)
            w_sum = np.sum(w)
        ...
        if max_delta_beta is not None:
            delta_beta = min(delta_beta, max_delta_beta)
        beta_new = min(beta + delta_beta, 1.0)

        log_w = (beta_new - beta) * logL
        log_w = log_w - np.max(log_w)
        log_w = np.clip(log_w, -700, 0)
        w = np.exp(log_w)
        w_sum = np.sum(w)
        ess_val = w_sum**2 / np.sum(w**2) if w_sum > 1e-300 else 1.0
        w = w / w_sum
        ...
        if mutation == "rw":
            ...
            cov = cov * 0.04
            lo_np = prior_bounds[:, 0]
            hi_np = prior_bounds[:, 1]
            for _ms in range(n_mutation_steps):
                ...
                if len(valid_idx) > 0:
                    ...
                    particles[acc_idx] = proposals[acc_idx]
                    logL[acc_idx] = logL_proposals[accept_mask]
        ...
        n_total_proposals = n_particles * n_mutation_steps if mutation == "rw" else n_particles
        accept_rates.append(n_accept / max(n_total_proposals, 1))
