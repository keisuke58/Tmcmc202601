...
SPECIES_NAMES = [
    "S. oralis", "A. naeslundii", "V. dispar", "F. nucleatum", "P. gingivalis"
]
...
def format_matrix(vec, lang="EN"):
    ...
    # M5
    A[0,4] = vec[16]; A[4,0] = vec[16]
    A[1,4] = vec[17]; A[4,1] = vec[17]
    A[2,4] = vec[18]; A[4,2] = vec[18]
    A[3,4] = vec[19]; A[4,3] = vec[19]
...
def generate_markdown(run_dir, lang="JP"):
    ...
    text = {
        "JP": {
            ...
            "config": "1. 実行設定",
            "item": "設定項目", "value": "値",
            "condition": "条件 (Condition)", "cultivation_label": "培養方法 (Cultivation)",
            "dt": "時間刻み (dt)", "max_step": "最大タイムステップ",
            "sigma": "sigma_obs (観測ノイズ)", "particles": "粒子数 (N_particles)",
            "time": "計算時間", "converged": "収束状態",
            "init_cond": "初期条件", "species": "菌種", "vol_frac": "phi_init (初期体積分率)",
            "data": "2. 実験データ",
            "obs_table": "観測された生菌体積分率 (phibar = phi * psi)",
            "day": "日数", "total": "総体積",
            "fit": "3. フィッティング精度",
            "fit_global": "3.1 全体指標",
            "metric": "指標", "map": "MAP 推定値", "mean": "事後平均",
            "rmse_all": "RMSE (全体)", "mae_all": "MAE (全体)", "max_err": "最大絶対誤差",
            "fit_species": "3.2 菌種別 RMSE",
            "map_rmse": "MAP RMSE", "mean_rmse": "Mean RMSE",
            "params": "4. 推定パラメータ",
            "param_list": "4.1 パラメータ一覧",
            "block": "ブロック", "param": "パラメータ", "bio_mean": "生物学的意味",
            "std": "標準偏差", "ci": "90% 信頼区間",
            ...
            "stats": "5. 対数尤度統計",
            "stat_item": "統計量",
            "max_logl": "最大 logL", "mean_logl": "平均 logL", "med_logl": "中央値 logL", "min_logl": "最小 logL",
            "samples": "サンプル数",
            "figs": "7. 図一覧 (Figures)",
            "fig_std": "標準図", "fig_extra": "追加分析図",
            "file": "ファイル名", "desc": "説明"
        },
        "EN": {
            ...
            "condition": "Condition", "cultivation_label": "Cultivation",
            "dt": "Time Step (dt)", "max_step": "Max Timesteps",
            "sigma": "sigma_obs", "particles": "Particles (N_particles)",
            "time": "Elapsed Time", "converged": "Convergence",
            "init_cond": "Initial Conditions", "species": "Species", "vol_frac": "phi_init (Volume Fraction)",
            "data": "2. Experimental Data",
            "obs_table": "Observed Viable Volume Fraction (phibar = phi * psi)",
            "day": "Day", "total": "Total Volume",
            "fit": "3. Fitting Accuracy",
            "fit_global": "3.1 Global Metrics",
            "metric": "Metric", "map": "MAP Estimate", "mean": "Posterior Mean",
            "rmse_all": "RMSE (Total)", "mae_all": "MAE (Total)", "max_err": "Max Absolute Error",
            "fit_species": "3.2 Per-Species RMSE",
            "map_rmse": "MAP RMSE", "mean_rmse": "Mean RMSE",
            "params": "4. Estimated Parameters",
            "param_list": "4.1 Parameter List",
            "block": "Block", "param": "Parameter", "bio_mean": "Biological Meaning",
            "std": "Std Dev", "ci": "90% CI",
            "matrix_a": "4.2 Interaction Matrix A (MAP)",
            "vec_b": "4.3 Decay Vector b (MAP)",
            "decay": "b (Decay Rate)",
            "stats": "5. Log-Likelihood Statistics",
            "stat_item": "Statistic",
            "max_logl": "Max logL", "mean_logl": "Mean logL", "med_logl": "Median logL", "min_logl": "Min logL",
            "samples": "Num Samples",
            "figs": "7. Figures",
            "fig_std": "Standard Figures", "fig_extra": "Extra Analysis Figures",
            "file": "Filename", "desc": "Description"
        }
    }
    ...
    header = f"| {t['day']} | " + " | ".join([f"*{s}*" for s in SPECIES_NAMES]) + f" | {t['total']} |"
    md.append(header)
    md.append("|:---:|" + ":---:|" * 5 + ":---:|")
    ...
    md.append("|---|:---:|:---:|")
    md.append(f"| {t['rmse_all']} | **{fit_metrics['MAP']['rmse_total']:.4f}** | {fit_metrics['Mean']['rmse_total']:.4f} |")
    md.append(f"| {t['mae_all']} | **{fit_metrics['MAP']['mae_total']:.4f}** | {fit_metrics['Mean']['mae_total']:.4f} |")
    md.append(f"| {t['max_err']} | **{fit_metrics['MAP']['max_abs']:.4f}** | {fit_metrics['Mean']['max_abs']:.4f} |\n")
    
    md.append(f"### {t['fit_species']}\n")
    md.append(f"| {t['species']} | {t['map_rmse']} | {t['mean_rmse']} |")
    md.append("|---|:---:|:---:|")
    for i, sp in enumerate(SPECIES_NAMES):
        md.append(f"| *{sp}* | {fit_metrics['MAP']['rmse_per_species'][i]:.4f} | {fit_metrics['Mean']['rmse_per_species'][i]:.4f} |")
    md.append("\n---\n")
    
    # 4. Parameters
    md.append(f"## {t['params']}\n")
    md.append(f"### {t['param_list']}\n")
    md.append(f"| {t['block']} | {t['param']} | {t['bio_mean']} | MAP | Mean | {t['std']} | {t['ci']} |")
    md.append("|:---:|:---:|:---:|:---:|:---:|:---:|:---:|")
    ...
    for i, name in enumerate(PARAM_NAMES):
        ...
        md.append(f"| **{blk}** | {name} | {desc} | {val_map:.4f} | {val_mean:.4f} | {val_std:.4f} | [{val_p05:.2f}, {val_p95:.2f}] |")
