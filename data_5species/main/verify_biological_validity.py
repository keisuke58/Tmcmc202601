...
def verify_negative_control(run_data, condition_name):
    ...
    solver = BiofilmNewtonSolver5S(
        dt=0.01, maxtimestep=2500,  # Assuming standard
        phi_init=0.01,
        c_const=1.0, alpha_const=10.0 # Default approx
    )
...
def verify_dysbiotic_hobic(run_data):
    ...
    solver = BiofilmNewtonSolver5S(
        dt=0.01, maxtimestep=2500,
        phi_init=0.01,
        c_const=1.0, alpha_const=10.0
    )
    t_model, phi_sim = solver.solve(theta)
    phi_total = np.sum(phi_sim, axis=1)
    phi_rel = phi_sim / phi_total[:, None]
    ...
    if red_day21 > red_day10 + 0.10: # Increased by at least 10%
        print(f"  ✅ C. Surge Curve: Red increases from {red_day10*100:.1f}% (Day 10) to {red_day21*100:.1f}% (Day 21)")
    else:
        print(f"  ❌ C. Surge Curve: No significant surge ({red_day10*100:.1f}% -> {red_day21*100:.1f}%)")
