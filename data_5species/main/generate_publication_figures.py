...
def plot_map_fit(data_dict, config, output_dir):
    ...
    solver = BiofilmNewtonSolver5S(
        ...
        phi_init=np.array([1e-6]*5) # Default small init
    )
    ...
    for i in range(5):
        ...
        # Plot Simulation
        # x_sim[:, i] is phi_i
        ax.plot(t_sim * 3, np.log10(x_sim[:, i] + 1e-10), label=f"{name} (Model)", color=color, linewidth=2.5)
        ...
        if exp_data.shape[1] > i:
            ax.scatter(days_data, exp_data[:, i], color=color, marker='o', s=50, edgecolors='k', label="Experiment")
    ...
    fig.text(0.04, 0.5, 'Log10 Biomass / Volume Fraction', va='center', rotation='vertical', fontsize=14)
...
def plot_posterior_band(data_dict, config, output_dir):
    ...
    for i in range(5):
        ...
        ax.fill_between(t_sim, mu - 2*sigma, mu + 2*sigma, color=color, alpha=0.2, label="95% CI")
...
def plot_heatmap(data_dict, config, output_dir):
    ...
    sns.heatmap(A, annot=True, fmt=".2f", cmap="RdBu_r", center=0,
               xticklabels=species_names, yticklabels=species_names,
               ax=ax, square=True, cbar_kws={"label": "Interaction Strength"})
...
def plot_residuals(data_dict, config, output_dir):
    ...
    for i in range(5):
        ...
        mask = ~np.isnan(obs)
        if np.sum(mask) > 0:
            ax.scatter(obs[mask], pred[mask], color=colors[i], label=config["species_names"][i], s=60, alpha=0.8, edgecolors='k')
