...
def plot_posterior_band(
    t_arr: np.ndarray, phibar_samples: np.ndarray, data: Dict[str, Any], output_path: Path
):
    ...
    plt.legend(
        by_label.values(), by_label.keys(), fontsize=10, loc="upper left", bbox_to_anchor=(1.05, 1)
    )
...
def main():
    ...
    for run_dir_str in args.run_dirs:
        ...
        if "samples" in data:
            ...
            plot_posterior_band(
                t_arr_ref, phibar_samples, data, output_dir / "nishioka_PosteriorBand"
            )
        else:
