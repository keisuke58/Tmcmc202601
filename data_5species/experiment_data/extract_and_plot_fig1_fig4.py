...
def generate_od600_data():
    ...
    for model, params_list in [("Dysbiotic", dysbiotic_params), ("Commensal", commensal_params)]:
...
def generate_interaction_network():
    ...
    return pd.DataFrame(
        interactions, columns=["source", "target", "interaction_type", "arrow_color"]
    )
...
def plot_fig1b(df_od):
    ...
    for model, color, alpha_range in [
        ...
        for rep_i in sorted(df_m["replicate"].unique()):
            ...
            ax.plot(df_r["time_hours"], df_r["OD600"], color=color, alpha=alpha, linewidth=lw)
...
def plot_fig4ab(df_ph, df_ging):
    ...
    ax1.text(-0.08, 1.05, "A", fontsize=18, fontweight="bold", transform=ax1.transAxes, va="top")

    # ── Panel B: Gingipain ──
    days = df_ging["day"].values
    ax2.errorbar(
        days,
        df_ging["dysbiotic_mean"],
        yerr=df_ging["dysbiotic_error"],
        fmt="o-",
        color="#ff7f0e",
        linewidth=2,
        markersize=7,
    ...
    ax2.errorbar(
        days,
        df_ging["commensal_mean"],
        yerr=df_ging["commensal_error"],
        fmt="o-",
        color="#1f77b4",
        linewidth=2,
        markersize=7,
    ...
    ax2.text(-0.08, 1.05, "B", fontsize=18, fontweight="bold", transform=ax2.transAxes, va="top")
...
def plot_fig4c(df_network):
    ...
    for _, row in df_network.iterrows():
        G.add_edge(
            row["source"], row["target"], color=row["arrow_color"], type=row["interaction_type"]
        )
    ...
    nx.draw_networkx_nodes(
        G, pos, node_color=node_colors, node_size=node_sizes, edgecolors="black", linewidths=1.0
    )
...
def plot_fig4c_with_values(df_network, theta_values, output_dir, source_name=""):
    ...
    if "V. parvula" in G.nodes():
        ...
        fixed_pos["V. parvula"] = (
            (radius_species * 1.05) * math.cos(base_angle + 0.1),
    ...
    pos_temp = nx.spring_layout(
        G_temp, k=1.5, pos=initial_pos, fixed=fixed_nodes_temp, seed=42, iterations=100
    )
    ...
    nx.draw_networkx_labels(
        G, pos, labels=labels, font_size=10, font_family="sans-serif", font_weight="bold"
    )
    ...
    legend_elements = [
        Line2D(
            ...
            lw=3,
            linestyle="--",
            label="Positive Interaction (Promotion)",
        ),
        Line2D(
            [0],
            [0],
            ...
            lw=3,
            linestyle="--",
            label="Negative Interaction (Inhibition)",
        ),
    ...
    if os.path.exists(target_path):
        try:
            ...
            if theta_vals:
                ...
                plot_fig4c_with_values(
                    df_network, theta_vals, output_dir=target_dir, source_name=source_name
                )
            else:
