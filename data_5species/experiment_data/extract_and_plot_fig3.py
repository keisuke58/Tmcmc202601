...
COMMENSAL_SPECIES = [
    "S. oralis", "A. naeslundii", "V. dispar", "F. nucleatum", "P. gingivalis_20709"
]
DYSBIOTIC_SPECIES = [
    "S. oralis", "A. naeslundii", "V. parvula", "F. nucleatum", "P. gingivalis_W83"
]
...
def make_legend(species_list, color_map):
    ...
    for sp in species_list:
        ...
        patches.append(mpatches.Patch(facecolor=color_map[sp], edgecolor='black',
...
def main():
    ...
    summary = df.groupby(["condition", "cultivation", "day", "species"])["distribution_pct"].agg(
        ["median", "mean", "std", "min", "max",
         lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)]
    ).reset_index()
    summary.columns = ["condition", "cultivation", "day", "species",
    ...
    # Panel titles
    panel_configs = [
        (axes[0, 0], "Commensal", "Static", COMMENSAL_SPECIES, COMMENSAL_COLORS, "Static Cultivation"),
        (axes[0, 1], "Commensal", "HOBIC",  COMMENSAL_SPECIES, COMMENSAL_COLORS, "HOBIC Cultivation"),
        (axes[1, 0], "Dysbiotic", "Static", DYSBIOTIC_SPECIES, DYSBIOTIC_COLORS, "Static Cultivation"),
        (axes[1, 1], "Dysbiotic", "HOBIC",  DYSBIOTIC_SPECIES, DYSBIOTIC_COLORS, "HOBIC Cultivation"),
    ]
    ...
    axes[0, 0].annotate("Commensal\nModel", xy=(-0.18, 0.5), xycoords='axes fraction',
                         fontsize=14, fontweight='bold', ha='center', va='center',
                         rotation=90)
    axes[1, 0].annotate("Dysbiotic\nModel", xy=(-0.18, 0.5), xycoords='axes fraction',
                         fontsize=14, fontweight='bold', ha='center', va='center',
                         rotation=90)
    ...
    axes[0, 1].legend(handles=commensal_patches, loc='upper right',
                       fontsize=9, framealpha=0.95, edgecolor='gray',
                       title="Commensal", title_fontsize=10)

    dysbiotic_patches = make_legend(DYSBIOTIC_SPECIES, DYSBIOTIC_COLORS)
    axes[1, 1].legend(handles=dysbiotic_patches, loc='upper right',
                       fontsize=9, framealpha=0.95, edgecolor='gray',
                       title="Dysbiotic", title_fontsize=10)

    plt.tight_layout(rect=[0.06, 0.02, 1, 0.98])
    plt.subplots_adjust(hspace=0.32, wspace=0.28)

    fig_path = "/home/nishioka/IKM_Hiwi/Tmcmc202601/data_5species/experiment_fig/fig3_reproduced.png"
