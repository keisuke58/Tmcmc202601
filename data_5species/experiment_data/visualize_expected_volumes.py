...
def plot_heatmaps(df):
    ...
    for idx, (cond, cult) in enumerate(CONDITIONS):
        ...
        pivot = sub.pivot_table(
            index="species", columns="day", values="species_volume_x1e6", aggfunc="first"
        )
...
def plot_small_multiples(df):
    ...
    fig.suptitle(
        "Species Volume Trajectories — Each Species × Each Condition",
        fontsize=14,
        fontweight="bold",
        y=0.99,
    )
    ...
    for i, sp in enumerate(SPECIES_DISPLAY):
        for j, (cond, cult) in enumerate(CONDITIONS):
            ax = axes[i][j]
            sub = df[
                (df["condition"] == cond) & (df["cultivation"] == cult) & (df["species"] == sp)
            ].sort_values("day")

            color = SPECIES_COLORS[sp]
            ax.fill_between(sub["day"], 0, sub["species_volume_x1e6"], color=color, alpha=0.25)
            ax.plot(
                sub["day"], sub["species_volume_x1e6"], "o-", color=color, linewidth=2, markersize=5
            )

            # Also plot total volume (gray dashed)
            ax.plot(
                sub["day"],
                sub["total_volume_median_x1e6"],
                "--",
                color="gray",
                linewidth=1,
                alpha=0.5,
...
def plot_normalized_stacked(df):
    fig, axes = plt.subplots(2, 2, figsize=(16, 10))
    fig.suptitle(
        "Species Composition Shift (100% Stacked)  —  Fraction of Total Volume",
        fontsize=14,
        fontweight="bold",
        y=0.99,
    )
    ...
    for idx, (cond, cult) in enumerate(CONDITIONS):
        ...
        for sp in SPECIES_DISPLAY:
            ...
            fracs = [
                sp_data.loc[d, "species_fraction_median"] if d in sp_data.index else 0 for d in days
            ]
            # Normalize to sum=1
...
def plot_radar(df):
    ...
    fig.suptitle("Species Volume Radar — Day 1 vs Day 21", fontsize=14, fontweight="bold", y=1.0)
    ...
    for idx, (cond, cult) in enumerate(CONDITIONS):
        ...
        for day, (ls, marker, alpha) in day_styles.items():
            ...
            values = [
                sp_data.loc[sp, "species_volume_x1e6"] if sp in sp_data.index else 0
                for sp in SPECIES_DISPLAY
            ]
            values += values[:1]
...
def plot_bubble(df):
    ...
    for (cond, cult), color in cond_colors.items():
        ...
        for _, row in sub.iterrows():
            ...
            ax.scatter(
                x, y, s=size, c=color, alpha=0.7, edgecolors="black", linewidth=0.5, zorder=3
            )
    ...
    ax.set_axisbelow(True)
    ax.set_title(
        "Species Volume Bubble Chart  —  Bubble Size ∝ Volume", fontsize=14, fontweight="bold"
    )

    # Legend
    legend_elements = [
        plt.scatter([], [], s=80, c=c, edgecolors="black", linewidth=0.5, label=f"{cond} {cult}")
...
def plot_streamgraph(df):
    ...
    fig.suptitle("Species Volume Streams Over Time", fontsize=15, fontweight="bold", y=0.99)
    ...
    for idx, (cond, cult) in enumerate(CONDITIONS):
        ...
        for sp in SPECIES_DISPLAY:
            ...
            ax.plot(days_fine, y_stack + y_vals, color=SPECIES_COLORS[sp], linewidth=0.8, alpha=0.8)
            y_stack += y_vals
        ...
        for sp in SPECIES_DISPLAY:
            ...
            ax.plot(
                sp_data["day"],
                sp_data["species_volume_x1e6"].cumsum() * 0
    ...
    # Shared legend
    legend_elements = [
        plt.Line2D([0], [0], color=SPECIES_COLORS[sp], lw=8, alpha=0.6, label=sp)
        for sp in SPECIES_DISPLAY
    ]
    fig.legend(
        handles=legend_elements,
...
def plot_condition_comparison(df):
    ...
    fig.suptitle(
        "Condition Comparison at Each Timepoint  —  Grouped Bars",
        fontsize=14,
        fontweight="bold",
        y=0.99,
    )
...
def plot_slope(df):
    ...
    fig.suptitle(
        "Species Volume Change:  Day 1 → Day 21  (Slope Chart)",
        fontsize=14,
        fontweight="bold",
        y=0.99,
    )
    ...
    for idx, (cond, cult) in enumerate(CONDITIONS):
        ...
        for sp in SPECIES_DISPLAY:
            ...
            if len(d1) > 0 and len(d21) > 0:
                ...
                color = SPECIES_COLORS[sp]
                ax.plot(
                    [0, 1], [v1, v21], "o-", color=color, linewidth=2.5, markersize=8, alpha=0.8
                )
                # Labels
                ax.text(
                    -0.08,
    ...
    # Legend
    legend_elements = [
        Line2D([0], [0], color=SPECIES_COLORS[sp], lw=2.5, marker="o", markersize=6, label=sp)
        for sp in SPECIES_DISPLAY
    ]
    fig.legend(
        handles=legend_elements,
