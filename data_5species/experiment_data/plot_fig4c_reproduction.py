...
def plot_fig4c():
    ...
    for name, pos in METABOLITE_POS.items():
        ...
        ax.add_patch(rect)
        # Label
        ax.text(pos[0], pos[1] - 0.02, name, ha="center", va="top", fontsize=7, zorder=11)
    ...
    # V. dispar circle
    c1 = plt.Circle(
        (inset_x + 0.02, inset_y), 0.018, color="#FF9800", ec="black", lw=1.0, zorder=10
    )
    ax.add_patch(c1)
    ax.text(
        inset_x + 0.045,
    ...
    c2 = plt.Circle(
        (inset_x + 0.02, inset_y - 0.05), 0.018, color="#FF9800", ec="black", lw=1.0, zorder=10
    )
    ax.add_patch(c2)
    ax.text(
        inset_x + 0.045,
    ...
    # Species circle
    c_leg = plt.Circle(
        (leg_x + 0.025, leg_y - 0.045), 0.012, color="gray", ec="black", lw=0.8, zorder=13
    )
    ax.add_patch(c_leg)
    ax.text(leg_x + 0.05, leg_y - 0.045, "Species", ha="left", va="center", fontsize=7, zorder=13)

    # Metabolite square
    rect_leg = plt.Rectangle(
        (leg_x + 0.095, leg_y - 0.052),
        0.012,
        0.012,
        fill=True,
        facecolor="white",
    ...
    ax.text(
        leg_x + 0.115, leg_y - 0.045, "Metabolite", ha="left", va="center", fontsize=7, zorder=13
    )
    ...
    ax.text(leg_x + 0.05, ey, "Enzyme", ha="left", va="center", fontsize=7, zorder=13)

    # Production/utilization arrows
    arrow_y = leg_y - 0.115
    # Production arrow (species -> metabolite)
    c_p = plt.Circle((leg_x + 0.02, arrow_y), 0.008, color="gray", ec="black", lw=0.6, zorder=13)
    ax.add_patch(c_p)
    draw_curved_arrow(
        ax,
        (leg_x + 0.03, arrow_y),
    ...
    # Panel label
    ax.text(
        0.01, 0.99, "C", fontsize=20, fontweight="bold", va="top", transform=ax.transAxes, zorder=15
    )
