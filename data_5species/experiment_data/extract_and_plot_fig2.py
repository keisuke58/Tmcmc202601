...
def plot_membrane_barplot(ax, df_panel):
    ...
    bars_intact = ax.bar(x, intact_vals, bar_width,
                          color='#2ca02c', edgecolor='black', linewidth=0.5,
                          label='Intact Membrane')
    # Damaged (red) on top
    bars_damaged = ax.bar(x, damaged_vals, bar_width,
                           bottom=intact_vals,
                           color='#d62728', edgecolor='black', linewidth=0.5,
                           label='Damaged Membrane')

    # Error bars at the boundary (intact/damaged interface)
    ax.errorbar(x, intact_vals, yerr=error_vals, fmt='none',
                ecolor='black', elinewidth=1.0, capsize=3, capthick=1.0)
...
def main():
    ...
    vol_summary = df_vol.groupby(["condition", "cultivation", "day"])["biofilm_volume_x1e6"].agg(
        ["median", "mean", "std", "min", "max"]
    ).reset_index()
    vol_summary.columns = ["condition", "cultivation", "day",
                            "volume_median", "volume_mean", "volume_std",
    ...
    for row, col, ptype, condition, cultivation in panels:
        ...
        if ptype == "boxplot":
            df_panel = df_vol[(df_vol["condition"] == condition) &
                              (df_vol["cultivation"] == cultivation)]
            plot_volume_boxplot(ax, df_panel)
        else:
            df_panel = df_mem[(df_mem["condition"] == condition) &
                              (df_mem["cultivation"] == cultivation)]
    ...
    # Column titles
    fig.text(0.27, 0.97, "Static Cultivation", fontsize=15, fontweight='bold',
             ha='center', va='top')
    fig.text(0.75, 0.97, "HOBIC Cultivation", fontsize=15, fontweight='bold',
             ha='center', va='top')

    # Row labels
    fig.text(0.02, 0.73, "Commensal\nModel", fontsize=14, fontweight='bold',
             ha='center', va='center', rotation=90)
    fig.text(0.02, 0.30, "Dysbiotic\nModel", fontsize=14, fontweight='bold',
             ha='center', va='center', rotation=90)

    # Panel label
    fig.text(0.03, 0.97, "A", fontsize=20, fontweight='bold', va='top')

    fig_path = "/home/nishioka/IKM_Hiwi/Tmcmc202601/data_5species/experiment_fig/fig2_reproduced.png"
