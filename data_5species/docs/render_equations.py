...
def render_matrix(filename):
    ...
    for sp, cx in zip(species, cols):
        ax.text(cx, 5.8, sp, fontsize=11, ha="center", va="center", color="#0072CE", style="italic")
    ...
    for ri, (row, ry) in enumerate(zip(rows, y_pos)):
        ...
        for ci, (cell, cx) in enumerate(zip(row, cols)):
            ...
            ax.text(
                cx, ry, cell, fontsize=16, ha="center", va="center", color=c, math_fontfamily="cm"
            )
            if is_locked:
                ax.text(
                    cx,
