...
パラメータ推定は以下の4段階で逐次的に実行される。
この構成はパラメータ間の相関（Coupling）と探索空間の次元数を考慮して
最適化されている。
...
\textbf{構成の妥当性検証：}
\begin{itemize}
    \item \textbf{細分化リスク}：これ以上段階を細かくすると、
    各段階のデータ量に対してパラメータが過剰となり、
    過学習（Overfitting）のリスクが増大する。

    \item \textbf{粗い構成のリスク}：段階を減らすと、
    パラメータ間の強い相関により収束が困難になる。

    \item \textbf{現構成の利点}：生物学的に意味のあるグループ
    （初期定着菌、中間菌、後期定着菌）に基づいた分割により、
    各段階での推定が安定する。
\end{itemize}
