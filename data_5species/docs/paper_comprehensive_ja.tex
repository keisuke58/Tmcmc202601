...
\textbf{概要}: 流れのある健康な状態 (HOBIC). S.oralisの高い増殖('Blue Bloom')と病原菌の抑制によって特徴付けられる.

\textbf{主要な発見}: モデルは初期定着菌の優位性を正しく特定しつつ, 病原菌集団を低く抑えており, 'Blue Bloom'の観察結果と一致している.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Commensal_HOBIC_20260208_002100/Fig_A02_per_species_panel.png}
\caption{Commensal HOBIC の事後分布適合. 陰影領域は95\%信用区間を示す. モデル(青帯)は実験データ(赤点)を密接に追跡しており, 良好な適合品質を確認できる.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{../_runs/Commensal_HOBIC_20260208_002100/Fig_A01_interaction_matrix_heatmap.png}
\caption{Commensal HOBIC の推定相互作用行列(MAP). 赤は正(協力的)相互作用, 青は負(競合的)相互作用を示す. 条件に関連する特定のブロック構造に注目されたい.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Commensal_HOBIC_20260208_002100/Fig_A05_parameter_violins.png}
\caption{Commensal HOBIC のパラメータ不確実性(バイオリンプロット). 狭い分布は高い識別性を示し, 広い分布はパラメータの不感応性または相関を示唆する.}
\end{figure}

\clearpage
\subsection{Dysbiotic HOBIC (Surge)}
\textbf{概要}: 流れのある疾患状態 (HOBIC). V.parvulaとP.gingivalisの爆発的な増殖('Surge')によって特徴付けられる.

\textbf{主要な発見}: パラメータロックを全解除(Discovery Mode)することで, モデルは病原菌の非線形な急増(Surge)を再現することに成功し, VeillonellaとP.gingivalis間の強い協力相互作用(正のフィードバック)を明らかにした.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Dysbiotic_HOBIC_20260208_002100/Fig_A02_per_species_panel.png}
\caption{Dysbiotic HOBIC (Surge) の事後分布適合. 陰影領域は95\%信用区間を示す. モデル(青帯)は実験データ(赤点)を密接に追跡しており, 良好な適合品質を確認できる.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{../_runs/Dysbiotic_HOBIC_20260208_002100/Fig_A01_interaction_matrix_heatmap.png}
\caption{Dysbiotic HOBIC (Surge) の推定相互作用行列(MAP). 赤は正(協力的)相互作用, 青は負(競合的)相互作用を示す. 条件に関連する特定のブロック構造に注目されたい.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Dysbiotic_HOBIC_20260208_002100/Fig_A05_parameter_violins.png}
\caption{Dysbiotic HOBIC (Surge) のパラメータ不確実性(バイオリンプロット). 狭い分布は高い識別性を示し, 広い分布はパラメータの不感応性または相関を示唆する.}
\end{figure}

\clearpage
\subsection{Commensal Static}
\textbf{概要}: 静的培養下の健康な状態. 栄養制限により, 安定的だが低いバイオマスとなる.

\textbf{主要な発見}: 厳格なパラメータロックにより病原菌の増殖が防がれ, 静的実験で観察された安定した共生状態を正確に反映している.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Commensal_Static_20260208_002100/Fig_A02_per_species_panel.png}
\caption{Commensal Static の事後分布適合. 陰影領域は95\%信用区間を示す. モデル(青帯)は実験データ(赤点)を密接に追跡しており, 良好な適合品質を確認できる.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{../_runs/Commensal_Static_20260208_002100/Fig_A01_interaction_matrix_heatmap.png}
\caption{Commensal Static の推定相互作用行列(MAP). 赤は正(協力的)相互作用, 青は負(競合的)相互作用を示す. 条件に関連する特定のブロック構造に注目されたい.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Commensal_Static_20260208_002100/Fig_A05_parameter_violins.png}
\caption{Commensal Static のパラメータ不確実性(バイオリンプロット). 狭い分布は高い識別性を示し, 広い分布はパラメータの不感応性または相関を示唆する.}
\end{figure}

\clearpage
\subsection{Dysbiotic Static}
\textbf{概要}: 静的培養下の疾患状態. 病原菌は存在するが, 代謝産物の蓄積により制限されている.

\textbf{主要な発見}: 病原菌の相互作用は推定されたが, HOBIC条件と比較してその規模は小さく, 完全なディスバイオシスの進行には流れ(Flow)が不可欠であることを裏付けている.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Dysbiotic_Static_20260207_203752/Fig_A02_per_species_panel.png}
\caption{Dysbiotic Static の事後分布適合. 陰影領域は95\%信用区間を示す. モデル(青帯)は実験データ(赤点)を密接に追跡しており, 良好な適合品質を確認できる.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{../_runs/Dysbiotic_Static_20260207_203752/Fig_A01_interaction_matrix_heatmap.png}
\caption{Dysbiotic Static の推定相互作用行列(MAP). 赤は正(協力的)相互作用, 青は負(競合的)相互作用を示す. 条件に関連する特定のブロック構造に注目されたい.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../_runs/Dysbiotic_Static_20260207_203752/Fig_A05_parameter_violins.png}
\caption{Dysbiotic Static のパラメータ不確実性(バイオリンプロット). 狭い分布は高い識別性を示し, 広い分布はパラメータの不感応性または相関を示唆する.}
\end{figure}
