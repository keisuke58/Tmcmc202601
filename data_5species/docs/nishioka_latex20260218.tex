...
\begin{itemize}
  ...
  \item \textbf{TSM sensitivity approximation}: The forward sensitivity equations used for the Taylor Series Model (TSM) surrogate drop the history propagation term $\partial Q / \partial g_\mathrm{old} \cdot x_{1,\mathrm{old}}$. This quasi-static approximation is accurate for slow dynamics but may underestimate sensitivity variance during rapid transients.
  \item \textbf{MCMC mutation strategy}: In the current implementation, the $K$-step mutation (Algorithm~\ref{alg:tmcmc}, Step 2e) generates all $K$ proposals from the initial resampled state rather than chaining sequentially (i.e., proposal $k$ is drawn from the original particle, not from the accepted state at step $k{-}1$). This parallelisation-friendly design preserves the correct stationary distribution (the MH ratio is evaluated at the tempered posterior) but may reduce mixing efficiency compared to sequential chaining. Acceptance rate monitoring across all runs ($>25\%$) confirms that this choice does not compromise posterior accuracy for the present problem dimensions.
...
\begin{itemize}
  ...
  \item DeepONet-accelerated TMCMC for high-throughput posterior exploration (~80$\times$ per-sample, ~29$\times$ E2E demonstrated)
