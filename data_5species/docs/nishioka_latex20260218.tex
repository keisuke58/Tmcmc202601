...
\author{Keisuke Nishioka (西岡佳祐)\\[4pt]
...
\begin{abstract}
We present a multiscale pipeline from Bayesian parameter estimation of a 5-species oral biofilm model to 3D finite-element stress analysis. TMCMC with biologically-constrained reduction estimates a Hamilton interaction model from experimental time-course data under four conditions, achieving 30\% RMSE improvement. A Dysbiosis Index (DI) based on Shannon entropy bridges to a DI-dependent elastic modulus, yielding a 28-fold stiffness range (909~Pa vs.\ 32~Pa) and 29.4-fold displacement ratio on patient-specific 3D geometry; Bayesian model comparison strongly favours the DI model (pseudo Bayes factor $= 1.35 \times 10^9$). A differentiable surrogate (DeepONet + DEM) enables 3.8~million-fold speedup and gradient-based NUTS with 1.75$\times$ higher acceptance. Basin analysis shows commensal equilibria are fragile (96\% of perturbed samples jump to dysbiosis), underscoring the need for uncertainty quantification in clinical predictions.

\vspace{0.5cm}
\noindent\textbf{Keywords:} TMCMC, oral biofilm, multiscale modelling, dysbiosis index, DeepONet, uncertainty quantification, FEM, peri-implantitis
\end{abstract}
...
Understanding the dynamics of multi-species biofilms is crucial for the prevention and treatment of oral diseases. Computational modelling of biofilm mechanics has progressed from smoothed-particle hydrodynamics approaches \cite{Soleimani2016SPHBiofilm} through finite-strain visco-elastic growth models \cite{Soleimani2019ViscoElasticGrowth} and stress-induced anisotropic growth formulations \cite{Soleimani2020AnisotropicGrowth} to the multi-species Hamilton framework used in the present work. Klempt et al.~\cite{Klempt2023PAMMBiofilm} compared several of these biofilm growth models and identified the Hamilton variational approach as particularly well-suited for multi-species systems.

Based on the extended Hamilton principle proposed by Junker and Balzani \cite{JunkerBalzani2021ExtendedHamilton} --- whose holistic variational formulation for dissipative systems has recently been generalised in~\cite{JunkerBodeHackl2025Holistic} --- Klempt et al.~\cite{Klempt2024ContinuumGrowth} developed a Hamilton principle-based model for diffusion-driven biofilm growth, subsequently extending it to a continuum multi-species framework with a novel interaction scheme \cite{Klempt2025ContinuumBiofilm}. The numerical discretisation follows the constraint-conforming approach of Bode et al.~\cite{Bode2025ConstraintConforming}, ensuring that the volume-fraction constraint $\sum\phi_\ell = 1$ is satisfied at the discrete level.

On the experimental side, Heine et al.~\cite{Heine2025PeriImplant} investigated the interactions of five major oral bacterial species associated with peri-implantitis, building on the HOBIC (Hannover Oral BIofilm Chamber) flow chamber developed by Kommerein et al.~\cite{Kommerein2018HOBIC}. Soleimani et al.~\cite{Soleimani2023CoAggregation} provided numerical and experimental validation of multi-species bacterial co-aggregation, while Mukherjee et al.~\cite{Mukherjee2025PgTitanium} studied \textit{P.~gingivalis} biofilm development on titanium surfaces using a hybrid \textit{in vitro}--\textit{in silico} approach. Furthermore, Fritsch et al.~\cite{Fritsch2025BayesianMicrofilms} discussed Bayesian updating methods for bacterial microfilms under hybrid uncertainties using a novel surrogate model, employing the time-separated stochastic mechanics framework of Geisler et al.~\cite{GeislerJunker2025TSM}.
...
The 5-species biofilm model \cite{Klempt2024ContinuumGrowth, Klempt2025ContinuumBiofilm} describes the dynamics of bacterial volume fractions $\phi_i$ and viability fractions $\psi_i$ through a coupled ODE system. The interaction term for species $i$ is:
\begin{equation}
...
The time-course data are from Heine et al.~\cite{Heine2025PeriImplant}, obtained using the HOBIC flow chamber \cite{Kommerein2018HOBIC}. Table~\ref{tab:exp_data_detail} summarises the dataset characteristics for reproducibility.

\begin{table}[H]
\centering
\caption{Experimental dataset characteristics (Heine et al.~\cite{Heine2025PeriImplant})}
\label{tab:exp_data_detail}
\begin{tabular}{ll}
\toprule
\textbf{Item} & \textbf{Value} \\
\midrule
Time points & Day 1--21 (typically 7--10 observations per condition) \\
Species & 5 (S.~oralis, A.~naeslundii, Veillonella, F.~nucleatum, P.~gingivalis) \\
Measurement & qPCR (relative abundance) \\
Noise $\sigma_i$ & 10--25\% of observed range per species (fixed in TMCMC) \\
Detection limit & F.n, P.g below limit in Commensal conditions (Table~\ref{tab:free_params}) \\
Conditions & 4 (Commensal/Dysbiotic $\times$ Static/HOBIC) \\
\bottomrule
\end{tabular}
\end{table}
...
The Hill gate parameters $K$ and $n$ (Section~\ref{sec:hill_gate}) control the F.~nucleatum colonisation threshold for P.~gingivalis establishment. We performed a grid search over $K \in \{0.01, 0.05, 0.10, 0.20\}$ and $n \in \{2, 4, 6, 8\}$ using the Dysbiotic HOBIC condition (where P.~gingivalis dynamics are most pronounced). The optimal combination $(K=0.05, n=4)$ was selected based on: (i)~lowest total RMSE across all five species, (ii)~biologically plausible estimates for the cross-species interaction parameters $a_{35}$ and $a_{45}$, and (iii)~convergence diagnostics ($\hat{R} < 1.1$, ESS $> 100$). A detailed sensitivity table is provided in the supplementary material. A follow-up sensitivity analysis over $K$ and $n$ shows that the Dysbiosis Index (DI) varies by less than 2\% across the grid for both dysbiotic and commensal equilibria, confirming that the main conclusions (CS $>$ DH ordering, 28-fold stiffness range) are robust to the choice of Hill gate parameters.
...
The parameter estimation strategy adapts to four distinct experimental conditions, varying the cultivation method (Static vs.\ HOBIC \cite{Kommerein2018HOBIC}) and the community state (Commensal vs.\ Dysbiotic). Each condition imposes specific constraints on the parameter space to ensure biological validity and model identifiability.
...
\subsubsection{Hyperparameter Sensitivity (DH and CH)}

To improve MAP RMSE for Dysbiotic HOBIC (DH) and Commensal HOBIC (CH), we performed a hyperparameter grid search over $\sigma_{\mathrm{scale}}$ (0.5, 1.0), $\lambda_{\mathrm{Pg}}$ (5, 8), and $\lambda_{\mathrm{late}}$ (3, 4), where $\sigma_{\mathrm{scale}}$ scales the observation noise and $\lambda_{\mathrm{Pg}}$, $\lambda_{\mathrm{late}}$ weight P.\,gingivalis and late-time residuals in the likelihood.
The sweep was distributed across six compute nodes (500 particles, 30 stages per run).
\emph{Results will be incorporated when the runs complete; the best configuration will be reported and used for subsequent analysis.}

% \begin{table}[H]
% \centering
% \caption{DH/CH hyperparameter sweep results (MAP RMSE). To be filled when runs complete.}
% \label{tab:hyperparam_sweep}
% \begin{tabular}{llcc}
% \toprule
% \textbf{Config} & $\sigma_{\mathrm{scale}}$ & $\lambda_{\mathrm{Pg}}$ & $\lambda_{\mathrm{late}}$ & RMSE (DH) & RMSE (CH) \\
% \midrule
% baseline & 1.0 & 5 & 3 & --- & --- \\
% ... \\
% \bottomrule
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Commensal_HOBIC_Fig_A02_per_species_panel.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Commensal_HOBIC_Fig_A01_interaction_matrix_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Commensal_HOBIC_Fig_A05_parameter_violins.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Dysbiotic_HOBIC_Fig_A02_per_species_panel.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Dysbiotic_HOBIC_Fig_A01_interaction_matrix_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Dysbiotic_HOBIC_Fig_A05_parameter_violins.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Commensal_Static_Fig_A02_per_species_panel.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Commensal_Static_Fig_A01_interaction_matrix_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Commensal_Static_Fig_A05_parameter_violins.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Dysbiotic_Static_Fig_A02_per_species_panel.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Dysbiotic_Static_Fig_A01_interaction_matrix_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Dysbiotic_Static_Fig_A05_parameter_violins.png}
...
While earlier biofilm FEM models coupled growth with visco-elastic constitutive behaviour \cite{Soleimani2019ViscoElasticGrowth} or stress-induced anisotropic nutrient transport \cite{Soleimani2020AnisotropicGrowth}, the present pipeline is, to our knowledge, the first to couple Bayesian-inferred species interactions to macroscale stress via a community-level bridging variable.
...
\begin{equation}
  ...
  \label{eq:hybrid}
\end{equation}
where $\mathrm{DI}_\text{0D}^{(c)}$ is the 0D condition-dependent dysbiosis index and $\alpha_\mathrm{Monod}(x)$ is the 1D spatially resolved Monod growth integral. Here $\alpha_\mathrm{Monod}$ is a \emph{volumetric} growth measure (the time-integrated Monod biomass production); for isotropic growth the volume ratio is $J_g = 1+\alpha_\mathrm{Monod}$, so the corresponding per-direction linear eigenstrain is $\varepsilon_\mathrm{growth} = J_g^{1/3}-1 \approx \alpha_\mathrm{Monod}/3$ (small-strain limit of the isotropic multiplicative growth split $\mathbf{F}_g = J_g^{1/3}\,\mathbf{I}$), which is why the factor $1/3$ appears in Eq.~\eqref{eq:hybrid}.
...
\subsubsection[Bayesian Model Comparison: DI vs phi-Pg vs Virulence]{Bayesian Model Comparison: DI vs.\ \texorpdfstring{$\varphi_{Pg}$}{phi-Pg} vs.\ Virulence}
...
To assess the mechanical consequences of microbial dysbiosis on a realistic geometry, we perform 3D linear-elastic FEM simulations on a patient-specific tooth model (T23, upper left canine). The growth--mechanics coupling follows a similar philosophy to recent tissue-turnover models applied in atherosclerosis \cite{Soleimani2025Atherosclerosis}, where biologically driven volume changes are translated into eigenstrain fields for structural analysis.
...
\subsection{Theoretical Consistency with Klempt et al.}
\label{sec:consistency}

The present framework is built upon the continuum multi-species biofilm model of Klempt et al.~\cite{Klempt2025ContinuumBiofilm}, itself grounded in the extended Hamilton principle of Junker and Balzani~\cite{JunkerBalzani2021ExtendedHamilton} and its predecessor for single-species diffusion-driven growth~\cite{Klempt2024ContinuumGrowth}. A systematic comparison reveals full theoretical consistency:

\begin{enumerate}
  \item \textbf{Variational principle}: Both formulations derive the governing equations from $\delta\int(G + C + D)\,\mathrm{d}t = 0$, where $G$, $C$, $D$ denote the Gibbs energy, constraint, and dissipation contributions, respectively.
  \item \textbf{Free energy and dissipation}: The bilinear free energy $\Psi = -\tfrac{1}{2}c^{*}\bar{\varphi}^T\mathbf{A}\bar{\varphi} + \tfrac{1}{2}\alpha^{*}\boldsymbol{\psi}^T\mathbf{B}\boldsymbol{\psi}$ and quadratic dissipation $\Delta^{s} = \tfrac{1}{2}\dot{\bar{\varphi}}^T\boldsymbol{\eta}\dot{\bar{\varphi}}$ are identical in structure. In the present work, the nutrient scaling $c^{*}$ and antibiotic scaling $\alpha^{*}$ are absorbed into the effective parameters $\mathbf{A}$ and $\mathbf{b}$, which is exact under constant cultivation conditions.
  \item \textbf{Interaction matrix symmetry}: The symmetry $A_{ij} = A_{ji}$ arises from the variational structure (Section~\ref{sec:symmetric_A}) and is preserved in both formulations.
  \item \textbf{Volume constraint}: The saturation constraint $\sum_{\ell=0}^{n}\phi_{\ell} = 1$ with Lagrange multiplier $\gamma$ is treated identically; the constraint-conforming discretisation follows~\cite{Bode2025ConstraintConforming}.
  \item \textbf{Barrier penalty}: The barrier term $K_{p1} = 10^{-4}$ in our Q-vector corresponds to $K_{p,i} = \eta_i \cdot 10^{-4}$ in~\cite{Klempt2025ContinuumBiofilm} with $\eta_i = 1$.
\end{enumerate}

\noindent The present work extends Klempt et al.\ in four directions that are compatible with the underlying variational structure: (i)~the Hill-type gating function for P.~gingivalis (Section~\ref{sec:hill_gate}), a nonlinear modification of the interaction term that preserves Newton convergence; (ii)~spatial extension to 2D/3D FEM via operator splitting (Section~\ref{sec:fem_pipeline}), where the reaction step at each node is the 0D Hamilton model of~\cite{Klempt2025ContinuumBiofilm}; (iii)~TMCMC Bayesian estimation of the interaction matrix, adding an inverse-problem layer to the same forward model; and (iv)~DI-dependent elastic modulus for mechanical coupling, a post-processing extension absent from the original formulation. None of these extensions introduce contradictions with the parent model.
...
\begin{itemize}
  \item \textbf{DI$\to$E mapping}: Phenomenological; not calibrated against simultaneous 16S+AFM. Parameter range consistent with literature; coupled validation would strengthen the model.
  \item \textbf{Symmetric $\mathbf{A}$}: $A_{ij} = A_{ji}$ from variational structure; may not capture asymmetric interactions.
  \item \textbf{Power likelihood}: $\lambda_{Pg} = 5$ improves pathogen fit; sensitivity with $\lambda_{Pg} = 1$ confirms robustness.
  \item \textbf{DeepONet}: MAP error varies (DH 11\% to CS 62\%); Pg parameters under-represented in training. Importance-weighted retraining may improve overlap.
  \item \textbf{Multi-attractor}: Posterior can span basins; point estimates unreliable for commensal static.
  \item \textbf{Spatial homogenisation}: 2D diffusion yields smaller DI than 0D; hybrid approach mitigates but is approximate.
\end{itemize}
...
\begin{itemize}
  ...
  \item DH (Dysbiotic HOBIC) TMCMC refinement: improved MAP RMSE and posterior overlap via 500p$\times$30 stages, GNN prior, and importance-weighted DeepONet retraining (results to be reported when available)
\end{itemize}
