...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{nishioka_figs/froh-06-1649419-g001.jpg}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{nishioka_figs/froh-06-1649419-g002.jpg}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{nishioka_figs/froh-06-1649419-g003.jpg}
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{nishioka_figs/froh-06-1649419-g004.jpg}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{nishioka_figs/TSM_simulation_Commensal_Static_MAP_Fit_with_data.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{nishioka_figs/posterior_predictive_Commensal_Static_PosteriorBand.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{nishioka_figs/residuals_Commensal_Static_Residuals.png}
...
% \begin{figure}[h]
% \centering
% \includegraphics[width=1.0\textwidth]{nishioka_figs/pub_parameter_boxplots.png}
...
% \begin{figure}[h]
% \centering
% \includegraphics[width=0.8\textwidth]{nishioka_figs/pub_interaction_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{nishioka_figs/Fig_A02_per_species_panel.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{nishioka_figs/Fig_A01_interaction_matrix_heatmap.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{nishioka_figs/Fig_A05_parameter_violins.png}
...
\begin{enumerate}
  \item \textbf{0D Hamilton ODE}: For each posterior sample $\boldsymbol{\theta}^{(k)}$, the 5-species Hamilton model is solved for species volume fractions $\varphi_i(t)$ and a global Dysbiosis Index $\mathrm{DI}_\text{0D}$.
...
\begin{itemize}
  \item The current 2D Hamilton solver uses a short simulation time ($T = 0.012$ non-dimensional units) due to computational constraints. Longer simulations with larger time steps (validated by CFL analysis) would better capture late-time dynamics, including the P.~gingivalis ``surge'' observed in dysbiotic conditions.
  \item The DI values from the 2D solver ($\sim 0.05$--$0.08$) are much smaller than the 0D values ($\sim 0.05$--$0.84$), because diffusion in 2D tends to homogenise species composition. The hybrid approach (Eq.~\ref{eq:hybrid}) addresses this but introduces a model approximation.
  \item The 2D FEM uses a simplified plane-strain formulation with uniform Poisson's ratio. Extension to 3D conformal meshes (already implemented for Abaqus) and heterogeneous material properties would increase physical realism.
  \item The material model parameters ($E_\max$, $E_\min$, $\nu$) are currently assumed rather than fitted. Nanoindentation or AFM measurements of biofilm stiffness under controlled microbial conditions would provide experimental validation.
\end{itemize}
...
\begin{itemize}
  ...
  \item 3D conformal FEM on realistic tooth geometry with condition-dependent material fields
  \item Adjoint-based inverse problems coupling the mechanical response back to microbial parameter estimation
  \item Clinical validation through comparison with \textit{in vivo} biofilm removal force measurements
\end{itemize}
...
We have presented a complete multiscale pipeline from Bayesian parameter estimation (TMCMC) of a 5-species oral biofilm model to 2D finite-element stress analysis.
The key contributions are:

\begin{enumerate}
  \item \textbf{Biologically-constrained TMCMC}: Locking absent interactions to zero and narrowing bounds on key pathogen parameters ($a_{35}$, $a_{45}$) reduces the total RMSE by 30\% and produces physically plausible posterior distributions.
  \item \textbf{Multiscale mechanical coupling}: A hybrid micro--macro approach combines 0D condition-dependent DI with 1D spatially resolved growth fields, producing 28-fold stress differences between commensal and dysbiotic biofilms.
  \item \textbf{Uncertainty propagation}: Posterior samples from TMCMC propagate through the full pipeline, revealing that biological prior constraints reduce DI uncertainty by 35-fold, with decay rates ($b_1$, $b_2$, $b_3$) as the universal sensitivity drivers.
\end{enumerate}
