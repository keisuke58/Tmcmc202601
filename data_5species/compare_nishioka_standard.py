...
def compare_parameters(nishioka_data, standard_data, output_dir):
    ...
    bars1 = ax1.bar(
        ...
        width,
        label="Nishioka (MAP)",
        color=colors_nishioka,
        edgecolor="black",
        alpha=0.8,
    )
    bars2 = ax1.bar(
        x + width / 2, standard_map, width, label="Standard (MAP)", color="orange", alpha=0.7
    )
    ...
    ax4.scatter(
        [1] * len(std_locked), std_locked, alpha=0.5, color="red", label="Locked params in Standard"
    )
...
def compare_fit_metrics(nishioka_data, standard_data, output_dir):
    ...
    return {
        ...
        "improvement": (
            (total_rmse_s - total_rmse_n) / total_rmse_s * 100 if total_rmse_s > 0 else 0
        ),
    }
...
def generate_summary_table(nishioka_data, standard_data, param_results, fit_results, output_dir):
    ...
    summary = {
        ...
        "Nishioka": [
            ...
            (
                f"{fit_results.get('nishioka_total_rmse', 'N/A'):.4f}"
                if isinstance(fit_results.get("nishioka_total_rmse"), (int, float))
                else "N/A"
            ),
            "N/A",
            "-",
            (
                f"{fit_results.get('nishioka_rmse', [0])[0]:.4f}"
                if fit_results.get("nishioka_rmse")
                else "N/A"
            ),
            (
                f"{fit_results.get('nishioka_rmse', [0,0])[1]:.4f}"
                if len(fit_results.get("nishioka_rmse", [])) > 1
                else "N/A"
            ),
            (
                f"{fit_results.get('nishioka_rmse', [0,0,0])[2]:.4f}"
                if len(fit_results.get("nishioka_rmse", [])) > 2
                else "N/A"
            ),
        ],
        "Standard": [
            ...
            (
                f"{fit_results.get('standard_total_rmse', 'N/A'):.4f}"
                if isinstance(fit_results.get("standard_total_rmse"), (int, float))
                else "N/A"
            ),
            "N/A",
            f"{fit_results.get('improvement', 0):.1f}%",
            (
                f"{fit_results.get('standard_rmse', [0])[0]:.4f}"
                if fit_results.get("standard_rmse")
                else "N/A"
            ),
            (
                f"{fit_results.get('standard_rmse', [0,0])[1]:.4f}"
                if len(fit_results.get("standard_rmse", [])) > 1
                else "N/A"
            ),
            (
                f"{fit_results.get('standard_rmse', [0,0,0])[2]:.4f}"
                if len(fit_results.get("standard_rmse", [])) > 2
                else "N/A"
            ),
        ],
    }
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="comparison_nishioka_vs_standard",
    ...
    summary_df = generate_summary_table(
        nishioka_data, standard_data, param_results, fit_results, output_dir
    )
