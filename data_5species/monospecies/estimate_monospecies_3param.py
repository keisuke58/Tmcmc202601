# -*- coding: utf-8 -*-
"""
estimate_monospecies_3param.py — 3-parameter TMCMC: (sigma, c_scale, phi_init)

theta = [sigma, c_scale, phi_init]
  sigma    : growth-attachment rate  [prior: Felix ±1 decade]
  c_scale  : effective nutrient scale [prior: 1–100, uniform]
  phi_init : initial biofilm fraction [prior: 0.01–0.4, uniform]

c(t) = c_scale * (1 - phi*psi)
phi_init controls trajectory shape (startup dynamics).
c_scale controls saturation curvature.
affine map  cfu = a * phibar + b  は解析的にプロファイルアウト。

Usage:
    python estimate_monospecies_3param.py [--n_particles 4000] [--outdir DIR]
    python estimate_monospecies_3param.py --temp 37
"""
...
def make_log_likelihood_3param(exp_time, exp_cfu, exp_std, n_steps,
                                time_to_step=10.0, Kp=1e-4, eta=1.0, eta_phi=1.0,
                                dt=1e-4, n_newton=6):
    """
    Log-likelihood for theta = [sigma, c_scale, phi_init].
...
def run_3param_condition(temp, data, n_particles=4000, mutation='rw', seed=42,
                          max_stages=30, verbose=True,
                          dt=1e-4, n_newton=6, target_ess_ratio=0.5,
                          c_scale_lo=1.0, c_scale_hi=100.0,
                          phi_init_lo=0.01, phi_init_hi=0.4):
    ...
    log_lik = make_log_likelihood_3param(
    ...
    if verbose:
        print(f"\n{'='*65}")
        print(f"  {temp}°C [sigma+c_scale+phi_init]: n_steps={n_steps}, n_obs={len(d['time'])}")
        print(f"  theta = [sigma ∈ [{lo_s:.1f},{hi_s:.1f}], "
              f"c_scale ∈ [{c_scale_lo},{c_scale_hi}], "
              f"phi_init ∈ [{phi_init_lo},{phi_init_hi}]]")
    ...
    result = tmcmc_engine(
        ...
        warmup_stages=2,
        seed=seed,
        label=f"{temp}C-3param-{mutation}",
        verbose=verbose,
    )

    samples   = result['samples']          # (N, 3)
    ...
    if verbose:
        print(f"  MAP: sigma={sigma_MAP:.4f}, c_scale={c_scale_MAP:.4f}, "
              f"phi_init={phi_init_MAP:.4f}, RMSE={rmse:.4f}")
        print(f"  Mean: sigma={np.mean(samples[:,0]):.4f}±{np.std(samples[:,0]):.4f}, "
              f"c_scale={np.mean(samples[:,1]):.4f}±{np.std(samples[:,1]):.4f}, "
              f"phi_init={np.mean(samples[:,2]):.4f}±{np.std(samples[:,2]):.4f}", flush=True)
...
def main():
    ...
    parser.add_argument('--outdir', type=str, default='_tmcmc_results_3param')
    ...
    parser.add_argument('--phi-init-lo', type=float, default=0.01)
    ...
    # Summary table
    print(f"\n{'='*90}")
    print(f"sigma + c_scale + phi_init TMCMC Summary ({args.mutation.upper()}, {args.n_particles}p)")
    print(f"{'='*90}")
    print(f"{'Temp':>6s} | {'Felix σ':>8s} | {'MAP σ':>8s} | {'MAP c':>8s} | "
          f"{'MAP φ₀':>8s} | {'RMSE':>7s} | {'Stages':>6s} | {'Time':>6s}")
    print('-' * 90)
    for res in all_results:
        t = res['temp']
        print(f"{t:>4d}°C | {FELIX_SIGMA[t]:>8.2f} | {res['sigma_MAP']:>8.3f} | "
              f"{res['c_scale_MAP']:>8.3f} | {res['phi_init_MAP']:>8.4f} | "
              f"{res['rmse_MAP']:>7.4f} | {res['n_stages']:>6d} | {res['total_time']:>5.0f}s")
    print(f"\nTotal: {elapsed:.0f}s")

    # CSV
    csv_path = os.path.join(args.outdir, 'tmcmc_3param_summary.csv')
    with open(csv_path, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['temp', 'felix_sigma', 'sigma_MAP', 'c_scale_MAP', 'phi_init_MAP',
                    'rmse_MAP', 'sigma_mean', 'sigma_std',
                    'c_scale_mean', 'c_scale_std', 'phi_init_mean', 'phi_init_std',
                    'n_stages', 'time_s'])
        for res in all_results:
            t = res['temp']
            w.writerow([t, FELIX_SIGMA[t],
                        f"{res['sigma_MAP']:.4f}", f"{res['c_scale_MAP']:.4f}",
                        f"{res['phi_init_MAP']:.4f}", f"{res['rmse_MAP']:.4f}",
    ...
    # JSON
    json_path = os.path.join(args.outdir, 'tmcmc_3param_summary.json')
