...
class PlotManager:
    ...
    def plot_posterior_predictive_spaghetti(
        ...
        for i, sp in enumerate(active_species):
            ...
            for j in range(phibar_subset.shape[0]):
                plt.plot(t_plot, phibar_subset[j, :, i], color=color, alpha=0.1, linewidth=1)
        ...
        *,
        ...
        for i, sp in enumerate(active_species):
            ...
            plt.fill_between(
                ...
                alpha=0.2,
                color="steelblue",
                label=f"φ̄{sp+1} 5–95%" if i == 0 else "",
            )
            plt.plot(
                t_plot,
                q50[:, i],
                color="darkblue",
                linewidth=2.5,
                label=f"φ̄{sp+1} median" if i == 0 else "",
                zorder=5,
            )
    ...
    def plot_pairplot_posterior(
        ...
        if samples.shape[1] != n:
            raise ValueError(
                f"Number of parameters mismatch: samples has {samples.shape[1]}, param_names has {n}"
            )
        ...
        for i in range(n):
            for j in range(n):
                ...
                if i == j:
                    ...
                    ax.scatter(
                        df[param_names[j]].values,
                        df[param_names[i]].values,
                        s=2,
                        alpha=0.3,
                        color="steelblue",
                    )
                    ...
                    ax.axvline(
                        theta_MAP[j], color="green", linestyle="--", linewidth=1.5, alpha=0.7
                    )
                    ax.axhline(
                        theta_MAP[i], color="green", linestyle="--", linewidth=1.5, alpha=0.7
                    )
                    ax.axvline(
                        theta_mean[j], color="orange", linestyle="--", linewidth=1.5, alpha=0.7
                    )
                    ax.axhline(
                        theta_mean[i], color="orange", linestyle="--", linewidth=1.5, alpha=0.7
                    )
                else:
    ...
    def plot_paper_fig14_mean_vs_true_with_std(
        ...
        plt.ylabel("Parameter value", fontsize=12)
        plt.title(
            "Paper Fig.14: Identified parameter means vs true values (± posterior std)", fontsize=14
        )
        plt.legend(fontsize=11)
        plt.grid(True, alpha=0.3, axis="y")
        plt.tight_layout()
    ...
    def plot_linearization_improvement(
        ...
        improvement = (
            (total_error_p1 - total_error_p2) / total_error_p1 * 100 if total_error_p1 > 0 else 0
        )

        axes[1].bar(
            ["Phase 1", "Phase 2"],
            [total_error_p1, total_error_p2],
    ...
    def plot_beta_schedule(self, beta_schedules: List[List[float]], name: str):
        ...
        plt.axhline(
            1.0, color="red", linestyle="--", linewidth=1, label="β=1.0 (target)", alpha=0.5
        )
    ...
    def plot_linearization_history(
        ...
        axes[0].axhline(
            1e-3, color="red", linestyle="--", linewidth=1, label="Threshold (1e-3)", alpha=0.5
        )
    ...
    def plot_rom_error_history(
        ...
        plt.title(
            f"ROM Error History: {name}\n"
            + r"$\varepsilon_{ROM} = ||\bar{\phi}_{ROM}(t_{obs}) - \bar{\phi}_{FOM}(t_{obs})||_2 / ||\bar{\phi}_{FOM}(t_{obs})||_2$",
            fontsize=14,
        )
        plt.yscale("log")
        plt.grid(True, alpha=0.3)
    ...
    def plot_map_error_comparison(
        ...
        bars1 = plt.bar(
            x - width / 2, errors_tmcmc, width, label="TMCMC", alpha=0.8, color="steelblue"
        )

        if map_errors_2phase is not None:
            errors_2phase = [map_errors_2phase.get(m, 0) for m in models]
            bars2 = plt.bar(
                x + width / 2, errors_2phase, width, label="2-phase MCMC", alpha=0.8, color="coral"
            )
    ...
    def plot_cost_accuracy_comparison(
        ...
        if cost_2phase is not None and map_errors_2phase is not None:
            ...
            for i, m in enumerate(models):
                plt.annotate(
                    m,
                    (costs_2phase[i], errors_2phase[i]),
                    xytext=(5, 5),
                    textcoords="offset points",
                    fontsize=10,
                )
    ...
    def plot_rom_subset_tradeoff(
        ...
        axes[0].axvline(
            20, color="red", linestyle="--", linewidth=1.5, label="Selected (20)", alpha=0.7
        )
        ...
        if rom_errors is not None:
            ...
            axes[1].axhline(
                0.01, color="gray", linestyle=":", linewidth=1, label="Threshold (0.01)", alpha=0.5
            )
