...
def visualize_graph(json_path, output_path):
    ...
    nx.draw_networkx_edges(G, pos, edgelist=active_edges, width=2, edge_color='black', style='solid')

    # Draw locked edges
    locked_edges = [(u, v) for u, v, d in G.edges(data=True) if d['type'] == 'locked']
    nx.draw_networkx_edges(G, pos, edgelist=locked_edges, width=1, edge_color='gray', style='dashed')
    ...
    plt.title("5-Species Interaction Network (Nishioka Algorithm)\nSolid: Active / Dashed: Locked (Zero)", fontsize=15)
    plt.axis('off')
