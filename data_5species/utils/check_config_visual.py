...
try:
    ...
    print("Warning: Could not import nishioka_model due to dependency issues. Using local definition.")
    
    INTERACTION_GRAPH_JSON = {
      "locked_edges": [
        {"param_idx": 6}, {"param_idx": 12}, {"param_idx": 13}, {"param_idx": 16}, {"param_idx": 17}
      ]
    }
...
def main():
    ...
    # Plot error bars
    plt.errorbar(indices, means, yerr=errors, 
                 fmt='o', capsize=5, label='Search Range (Bounds)', 
                 color='blue', ecolor='gray')

    # Highlight locked parameters
    if locked:
        plt.scatter(locked, [0]*len(locked), color='red', s=100, zorder=5, label='Locked (0.0)', marker='x')

    # Highlight special positive parameter (a18)
    # Check if a18 is indeed positive-only (0, 1) -> mean 0.5, error 0.5
    if bounds[18] == (0.0, 1.0):
        plt.scatter([18], [0.5], color='green', s=100, zorder=5, label='Positive Constraint (Vei->Pg)', marker='^')
    ...
    plt.title("Visualization of Nishioka Model Parameter Bounds\n(Locked vs Active Ranges)", fontsize=14)
