...
\author[Nishioka et al.]{%
  ...
  Rumjhum Mukherjee$^{2,3}$,\;
  Szymon P.\ Szafra\'{n}ski$^{2,3}$,\;
  Meike Stiesch$^{2,3}$}
\institute[IKM/NIFE]{%
  $^1$Institute of Continuum Mechanics, Leibniz Universit\"at Hannover\\
  $^2$Dept.\ Prosthetic Dentistry \& Biomedical Materials Science, MHH\\
  $^3$NIFE, Hannover}
\date{2026}
...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    {\sffamily\footnotesize
      Rumjhum Mukherjee$^{2,3}$,\ Szymon P.\ Szafra\'{n}ski$^{2,3}$,\
      Meike Stiesch$^{2,3}$\par}
    \vspace{4pt}
    {\sffamily\footnotesize\color{midgray}
      $^1$IKM, Leibniz Universit\"at Hannover\quad
      $^2$MHH\quad $^3$NIFE, Hannover\par}
    \vspace{0.45cm}
    ...
    \column{0.52\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{3pt}
        \item[\hlg{$\checkmark$}] \textbf{pH prediction}:
          independent $R^2=\hlg{0.78}$, RMSE\,$=0.13$ ($N=12$ held-out; LOO $R^2=0.92$)\\
          {\scriptsize $\mathrm{pH}=\beta_0+\sum_i\beta_i\,\phi_i$ ($\beta$ from exp.\ LOO-CV, $N=90$;
           pH not used in TMCMC calibration) --- lactate-driven acidification}
...
% ── pH prediction figure (updated, 10,000-particle posterior) ────────────────
\begin{frame}[plain]{Independent Validation: pH Prediction \small(updated --- 10{,}000-particle posterior)}
  \centering
  \includegraphics[width=0.95\textwidth,keepaspectratio]{figures/fig_ph_validation.pdf}
  \par\vspace{3pt}
  {\color{midgray}\rule{0.88\textwidth}{0.3pt}}\par\vspace{1pt}
  {\scriptsize pH from the Phase-2 ($N_p=10{,}000$) Hamilton-ODE posterior via experimental
   LOO-CV ($N=90$); pH was \emph{not} used in TMCMC calibration. \quad
   independent $R^2=0.78$, RMSE\,$=0.13$\,pH units.}
\end{frame}
