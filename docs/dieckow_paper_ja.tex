...
Philipp Junker
\\[6pt]
\small Leibniz Universit\"at Hannover, Institut f\"ur Kontinuumsmechanik, Hannover, Germany
}
...
細菌リード数が 452 のみと極端に少ない（他サンプルの平均 \SI{7693}{reads} に対し 1/15 程度）．
...
\label{eq:glv}
\end{equation}
共有 $10\times10$ 相互作用行列 $A$ と JAX Adam（5,000 エポック，lr\,$=0.003$，
$\ell_2$ 正則化 $\lambda=10^{-3}$，自己制限制約 $A_{ii}\le 0$）で最適化した
患者別固有率 $b^{(p)}$ を持つ．
週 1 初期条件から週 2 および 3 を予測して RMSE\,$= 0.055$，Pearson $r = 0.954$ が得られた
（図\,\ref{fig:guild_A}）．

最も強い推定相互作用は Actinobacteria$\to$Bacilli（$A = +0.38$）であり，
\textit{Actinomyces} ブリッジング種と初期 \textit{Streptococcus} 定着者の
よく確立された共凝集と整合する~\cite{Kolenbrander2010Oral}．
...
右: 週 2 および 3 の観測値 vs. 予測値（$r = 0.954$，RMSE\,$= 0.055$）．}
\label{fig:guild_A}
\end{figure}
...
他の口腔マイクロバイオームモデリング研究でも指摘されており~\cite{Mounier2022NetMismatch}，
...
\item \textbf{ギルドレベル gLV.}
  ...
  週 2/3 予測で $r=0.954$（RMSE\,$=0.055$）を達成し，
