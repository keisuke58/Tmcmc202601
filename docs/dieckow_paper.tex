...
\label{eq:glv}
\end{equation}
with a shared $10\times10$ interaction matrix $A$ and patient-specific intrinsic rates $b^{(p)}$ optimised by scipy L-BFGS-B (5 multi-starts: warm\,$+$\,4 random, $\ell_2$ regularisation $\lambda=10^{-4}$) with self-limitation constraints ($A_{ii}\le 0$).
Predictions of weeks~2 and~3 starting from week~1 give RMSE\,$= 0.050$ and Pearson $r = 0.963$ (Fig.\,\ref{fig:guild_A}).
