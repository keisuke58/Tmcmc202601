...
Leave-one-patient-out cross-validation of the gLV+struct model (Fig.\,\ref{fig:loo_cv}) yields mean LOO-CV RMSE\,$= 0.059$, with the highest held-out error for patients~A and~B ($\approx 0.092$), both CT2-type outliers, and the lowest for patients~E ($0.023$) and~F ($0.038$).
The LOO-CV generalisation gap (train\,$\approx0.060$; LOO\,$=0.059$) is very small, indicating good predictive robustness across the 8-patient cohort.
...
\midrule
gLV (L-BFGS-B, 5 starts)$^{\ddagger}$ & 11 & 8  & \textbf{0.060} & --- & none\\
...
\end{tabular}
\smallskip\\
\footnotesize $^\ddagger$gLV train RMSE\,$=0.060$ is for the 11-guild/8-patient consistent fit (this work).
The 10-guild/10-patient gLV fit (including patients D and L with partial data) achieves RMSE\,$=0.050$, $r=0.963$ (see text and Fig.\,\ref{fig:guild_A}) but is not directly comparable to the other rows.
gLV LOO pending (pure gLV LOO-CV in progress; gLV+struct LOO\,$=0.059$ is shown as proxy in row~2).
\end{table}
