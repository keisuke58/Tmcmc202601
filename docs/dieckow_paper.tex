...
Integrating CLSM structural data (live-biovolume occupancy and PerLive fraction) as patient- and week-specific Hamilton ODE parameters reduces the guild-level Hamilton ODE RMSE by 40\,\% (0.105\,$\to$\,0.061), demonstrating that structural biofilm measurements carry independent predictive information beyond 16S composition.
...
PerLive spans 50--91\,\% across samples, indicating substantial patient-to-patient variability in biofilm viability that is independent of compositional shifts.

Using Eq.\,\eqref{eq:clsm_params} to modulate $c$ and $\alpha$ per patient and week, the Hamilton ODE achieves RMSE\,$\approx 0.061$ at 900 Adam epochs (converging; final value pending).
This represents a 40\,\% improvement over the Hamilton ODE fit without structural data (RMSE\,$= 0.105$, Table~\ref{tab:rmse}), confirming that the CLSM occupancy and viability measurements carry predictive information beyond the 16S compositional data alone.
...
\caption{A-matrix comparison: gLV (left, RMSE\,$=0.050$) vs Hamilton ODE with CLSM-informed parameters (right, RMSE\,$\approx0.061$, preliminary).
