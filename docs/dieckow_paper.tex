...
At the community level, an 11-guild generalised Lotka-Volterra (gLV) model fit to all 53 genera achieves RMSE\,$=\,$0.045 ($r=0.963$) with a KEGG/HMDB-weighted metabolite-flow sign prior; leave-one-out cross-validation on the 8 patients with complete three-week data gives RMSE\,$=\,$0.058.
...
The KEGG/HMDB-weighted prior achieves full sign agreement (19/19, 100\%) while simultaneously improving train RMSE from 0.060 to 0.045 ($r=0.963$).
...
A more quantitative grounding would be provided by genome-scale community metabolic modelling using the AGORA resource~\cite{Thiele2020AGORA}, which could supply predicted metabolic exchange fluxes between guild representatives and replace the binary SF1 sign predictions with continuous flux magnitudes.
