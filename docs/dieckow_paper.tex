...
\label{eq:glv}
\end{equation}
with a shared $10\times10$ interaction matrix $A$ and patient-specific intrinsic rates $b^{(p)}$ optimised by JAX Adam (5{,}000 epochs, lr\,$=0.003$, $\ell_2$ regularisation $\lambda=10^{-3}$) with self-limitation constraints ($A_{ii}\le 0$).
...
\caption{Left: inferred $10\times10$ guild interaction matrix (red\,$=$ positive, blue\,$=$ negative). Right: observed vs predicted relative abundance for weeks~2 and~3 ($r = 0.954$, RMSE\,$= 0.055$).}
\label{fig:guild_A}
\end{figure}
...
The negative Betaproteobacteria$\to$Actinobacteria coefficient ($A=-0.20$) is biologically plausible: \textit{Neisseria} spp.\ produce hydrogen peroxide and bacteriocin-like inhibitory substances that suppress \textit{Actinomyces} growth under aerobic early-colonisation conditions~\cite{Dieckow2025Implant}.
...
Such a divergence between metabolic-potential and realised-dynamics networks has been noted in other oral microbiome modelling studies~\cite{Mounier2022NetMismatch} and underscores the complementary value of data-driven gLV inference alongside database-derived interaction graphs.
