...
$^\dagger$Approximate LOO (shared $A$ fixed from full-cohort fit; patient-specific $b$ re-fitted from W1$\to$W2): Hamilton ODE (no struct) estimated by train/LOO ratio; Hamilton+CLSM based on running computation (patient A LOO\,$=0.132$).}
...
Hamilton ODE (no struct)             & 10 & 0.105 & $\sim$0.20$^\dagger$ & none\\
Hamilton ODE + CLSM (this work)      & 11 & \textbf{0.060} & $\sim$0.13$^\dagger$ & $\tilde\omega$, $q_{p,w}$, $\alpha$\\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Leave-one-patient-out cross-validation RMSE for gLV\,+\,struct.
...
Patient~A and~B show the highest held-out RMSE ($\approx 0.092$), consistent with their atypical CT2 compositions; patients~C, E, and~F generalise well ($\leq 0.046$).}
\label{fig:loo_cv}
\end{figure}
...
To test whether the shared interaction matrix $A$ is consistent across community types, we fit separate gLV models on the CT1 subset (patients E, G, K; $n=3$) and the CT2 subset (patients A, B, C, F, H; $n=5$), using the full-cohort $A$ as warm start.
