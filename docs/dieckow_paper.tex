...
At the community level, an 11-guild generalised Lotka-Volterra (gLV) model with KEGG/HMDB-weighted metabolite-flow sign prior achieves RMSE\,$=\,$0.045 ($r=0.963$) with 100\% sign agreement (19/19 guild pairs); the sign-constrained LOO-CV is 0.074, versus 0.058 for unconstrained gLV, reflecting the trade-off between biological plausibility and generalisation.
Integrating CLSM structural data (live-biovolume occupancy and PerLive fraction) as patient- and week-specific Hamilton ODE initial conditions reduces the guild-level Hamilton ODE RMSE by 43\,\% (0.105\,$\to$\,0.060), with approximate LOO-CV RMSE\,$=\,$0.086 (CT1 patients: 0.063; CT2 patients: 0.099), demonstrating that structural biofilm measurements carry independent predictive information beyond 16S composition.
...
An 11-guild gLV with KEGG/HMDB metabolite-flow prior achieves RMSE\,$=0.045$, $r=0.963$, with 100\% sign agreement (19/19) covering 100\% of sequenced reads; integrating CLSM structural data into the Hamilton ODE reduces guild-level RMSE by 43\%, a gain that is mechanistically attributable to the free-space variable $\varphi_0$ unique to the Hamilton formulation.
...
The gLV replicator ODE with shared $A\in\mathbb{R}^{11\times11}$ and patient-specific rates $b^{(p)}$ is optimised by scipy L-BFGS-B (5 multi-starts, warm initialisation from a preliminary fit, $\ell_2$ regularisation $\lambda=10^{-4}$, $A_{ii}\le0$).

\paragraph{KEGG/HMDB-weighted metabolite-flow sign prior.}
\label{sec:kegg_prior}
To incorporate biochemical knowledge into the gLV and Hamilton ODE fits, we construct a guild-level sign prior from Dieckow Supplementary File~1 (SF1), which catalogues 347 species-level \textsc{produces}/\textsc{uses}/\textsc{is\_inhibited\_by} records linking 95 metabolites to the sequenced taxa.

\textit{Step 1 --- metabolite-flow aggregation.}
For each metabolite $m$ in SF1, we identify the set of producer guilds $P_m$ (\textsc{produces}), consumer guilds $C_m$ (\textsc{uses}), and inhibited guilds $I_m$ (\textsc{is\_inhibited\_by}).
Each producer--consumer pair $(s,t)$ with $s\in P_m$, $t\in C_m$, $s\neq t$ increments a positive-flow counter $F^+_{ts}$; each producer--inhibited pair $(s,t)$ with $t\in I_m$ increments $F^-_{ts}$.
The net guild-level flow is $F_{ts}=F^+_{ts}-F^-_{ts}$; its sign $s^*_{ts}=\mathrm{sgn}(F_{ts})$ is the biochemical prediction for the direction of the $s\to t$ interaction in the $A$ matrix.

\textit{Step 2 --- KEGG/HMDB database weighting.}
Not all SF1 records are equally reliable: metabolites with a KEGG compound identifier ($n_{\mathrm{KEGG}}=26$ metabolites, contributing to all rows where the KEGG flag is set) or an HMDB identifier ($n_{\mathrm{HMDB}}=239$ metabolite--taxon rows) are annotated against curated reference databases, whereas unannotated records rely on literature text mining alone.
We therefore weight each row by $w=2$ for KEGG- or HMDB-annotated metabolites and $w=1$ otherwise when accumulating $F^+$ and $F^-$, giving a weighted net flow $\tilde{F}_{ts}=\sum_m w_m(F^+_{ts,m}-F^-_{ts,m})$.

\textit{Step 3 --- sign penalty in the objective.}
For every guild pair $(t,s)$ with $\tilde{F}_{ts}\neq0$, a soft sign penalty is added to the L-BFGS-B objective:
\begin{equation}
\mathcal{L}_{\mathrm{sign}} = \sum_{(t,s):\,\tilde{F}_{ts}\neq0}
  \frac{|\tilde{F}_{ts}|}{\sigma_{\mathrm{sign}}^2}\,
  \Bigl[\max\!\bigl(0,\,-s^*_{ts}\cdot A_{ts}\bigr)\Bigr]^2,
\label{eq:sign_penalty}
\end{equation}
where $\sigma_{\mathrm{sign}}=0.15$ (selected by grid search; see Results) and $|\tilde{F}_{ts}|$ up-weights pairs with stronger metabolic evidence.
The full objective is $\mathcal{L}=\mathrm{RMSE}+\mathcal{L}_{\mathrm{sign}}+\lambda\|A\|_F^2$.
The penalty is zero when $A_{ts}$ already has the correct sign, and grows quadratically for sign violations.
The same penalty is applied to the Hamilton ODE (JAX-Adam) fits, using the identical $\tilde{F}$ matrix and $\sigma_{\mathrm{sign}}=0.15$.

\paragraph{Leave-one-patient-out cross-validation (LOO-CV).}
For each of the 8 patients $p\in\{A,B,C,E,F,G,H,K\}$, we re-fit the shared $A$ and the remaining 7 patients' $b^{(q)}$ ($q\neq p$) from scratch (5 L-BFGS-B starts), then predict patient $p$'s week-2 and week-3 relative abundances using the held-out $b^{(p)}$ re-fitted from week~1 alone.
This procedure is identical for both the plain gLV and the KEGG/HMDB-prior gLV, allowing a direct comparison of generalisation with and without the metabolite-flow constraint.
...
\label{eq:glv}
\end{equation}
gives RMSE\,$= 0.050$ and Pearson $r = 0.963$ on all 10 patients (10 guilds; Fig.\,\ref{fig:guild_A}).
On the 8-patient/11-guild cohort used for structural data comparison (patients A--C, E--H, K), the same L-BFGS-B procedure yields RMSE\,$= 0.060$ and $r = 0.927$ (Table~\ref{tab:rmse_guild}).
...
The gLV\,+\,struct model achieves RMSE\,$= 0.060$ on the 8-patient, 11-guild cohort; applying the same PerLive-scaled $b$ without the occupancy $\varphi_0$ channel is not possible in the simplex-constrained gLV.
Reverting to the unscaled $b$ from the gLV\,+\,struct fit raises RMSE to 0.114, indicating that the gLV\,+\,struct parameters are tightly coupled to the PerLive signal and cannot simply be transferred to a plain gLV predictor.
This contrasts with the Hamilton ODE, where the free-space channel ($\varphi_0 = 1 - \tilde\omega$) provides an independent structural degree of freedom not available to the simplex-constrained replicator.
Leave-one-patient-out cross-validation of the pure gLV (Fig.\,\ref{fig:loo_cv}) yields mean LOO-CV RMSE\,$= 0.058$, with the highest held-out error for patients~A and~B ($\approx 0.091$), both CT2-type outliers, and the lowest for patient~E ($0.020$).
The gLV+struct LOO-CV (Fig.\,\ref{fig:loo_cmp}) gives 0.059, confirming that PerLive scaling adds no generalisation benefit.
Adding the KEGG/HMDB sign prior raises the mean LOO-CV to 0.074; the per-patient breakdown (Fig.\,\ref{fig:loo_pat_cmp}) reveals that patient~E is a pronounced outlier (RMSE$=0.190$ vs.\ $0.023$ without prior), indicating that the biochemical sign constraints conflict with this patient's atypical ecological trajectory, whereas patients~F (CT2) and~G (CT1) benefit from the prior (RMSE reduced by 0.017 and 0.024, respectively).
The small generalisation gap (train\,$=0.060$; LOO\,$=0.058$) indicates good predictive robustness across the 8-patient cohort.
...
gLV train RMSE uses all 10 patients (D,L included with partial transitions); gLV LOO-CV uses the 8 patients with complete three-week data (A--C, E--H, K).
gLV+struct, Hamilton ODE (no struct), and Hamilton+CLSM train and LOO all use the same 8 patients.
LOO-CV type: gLV uses proper LOO ($A$ re-fitted on 7 patients); Hamilton uses approximate LOO ($A$ fixed from full-cohort; these two LOO values are not directly comparable.
$^\dagger$Hamilton approximate LOO: $A$ fixed from full-cohort fit; patient-specific $b$ re-fitted from W1$\to$W2; Hamilton (no struct) estimated from the train/LOO ratio of Hamilton+CLSM ($1.43\times$).}
\label{tab:rmse_guild}
\begin{tabular}{@{}llcccl@{}}
\toprule
\textbf{Model} & \textbf{Guilds} & \textbf{Patients (train)} & \textbf{Train RMSE} & \textbf{LOO-CV RMSE} & \textbf{Structural data}\\
\midrule
gLV (L-BFGS-B, 5 starts)$^{\ddagger}$ & 11 & 8  & \textbf{0.060} & \textbf{0.058} & none\\
gLV + KEGG/HMDB prior (this work)   & 11 & 8  & 0.045 & 0.074$^{\S}$ & none\\
gLV + struct (PerLive-scaled $b$)    & 11 & 8  & 0.060 & 0.059 & $q_{p,w}$ only\\
Hamilton ODE (no struct)             & 10 & 8  & 0.105 & $\sim$0.15$^\dagger$ & none\\
Hamilton ODE + CLSM (this work)      & 11 & 8  & \textbf{0.060} & \textbf{0.086}$^\dagger$ & $\tilde\omega$, $q_{p,w}$, $\alpha$\\
\bottomrule
\end{tabular}
\smallskip\\
\footnotesize $^\ddagger$gLV train/LOO are for the 11-guild/8-patient consistent fit (this work).
The 10-guild/10-patient gLV fit (patients D,L included with partial data) achieves RMSE\,$=0.050$, $r=0.963$ (see text, Fig.\,\ref{fig:guild_A}) but is not directly comparable to the other rows.
Pure gLV LOO (proper; $A$ re-fitted on 7 patients per fold) yields 0.058, confirming that PerLive scaling (gLV+struct LOO\,$=0.059$) provides no generalisation gain.
$^\S$KEGG/HMDB-prior gLV LOO (0.074) is higher than the plain gLV LOO (0.058): the sign penalty regularises the $A$ matrix toward biological plausibility at the cost of a small generalisation trade-off (8-fold LOO, 8 patients).
\end{table}
...
Dashed line: mean LOO-CV RMSE\,$= 0.058$.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/dieckow/fig_loo_patient_comparison.pdf}
\caption{Per-patient leave-one-out cross-validation RMSE for three models: pure gLV (blue), gLV\,+\,KEGG/HMDB sign prior (green), and Hamilton\,+\,CLSM (red).
Dashed horizontal lines mark the respective cohort means (0.059, 0.074, 0.086).
Background shading: CT2 (yellow) vs.\ CT1 (green).
Patient~E (CT1) is a pronounced outlier under the KEGG prior (RMSE\,$=0.190$), while patients~F and~G benefit (RMSE reduced by 0.017 and 0.024). Hamilton\,+\,CLSM generalises better than the KEGG prior on CT2 patients but worse on CT1.}
\label{fig:loo_pat_cmp}
\end{figure}
...
\caption{Left: inferred $11\times11$ guild interaction matrix on the 8-patient consistent cohort (red\,$=$ positive, blue\,$=$ negative; RMSE\,$=0.060$, $r=0.927$). Right: observed vs predicted relative abundance for weeks~2 and~3. The 10-guild/10-patient historical fit achieved RMSE\,$=0.050$ ($r=0.963$).}
\label{fig:guild_A}
\end{figure}
...
Nodes are the 11 guilds (8-patient consistent cohort, RMSE\,$=0.060$; area $\propto$ mean abundance); edge colour encodes interaction sign (red\,$=$ positive, blue\,$=$ negative) and thickness is proportional to $|A_{ij}|$.
...
Neither gLV nor Hamilton ODE fully captures the magnitude of diversity decline (observed $\Delta H = 0.14$, predicted $\Delta H \approx 0.03$) (Fig.\,\ref{fig:diversity}).
Three mechanisms not encoded in the pair-wise interaction models may account for this gap:
(i) \emph{Priority effects} --- the outcome of early colonisation events depends on order of arrival, which is not captured by equilibrium $A$ coefficients;
(ii) \emph{Higher-order interactions} --- three-way competitive interactions (e.g., Bacilli inhibiting both Actinobacteria and Bacteroidia simultaneously) accelerate diversity loss beyond what pairwise terms predict~\cite{Klempt2025ContinuumBiofilm};
(iii) \emph{Spatial niche-filtering} --- as biofilm matures, spatial stratification creates oxygen and pH gradients that selectively suppress surface guilds, a dynamic captured by the PDE extension of the Hamilton model but not by the 0D replicator ODE.
These observations motivate future extensions with HOI terms or spatial structuring, though the HOI-gLV comparison (Table~\ref{tab:model_cmp}) shows that HOI coefficients converge to near-zero on the current dataset, suggesting that longer time-series or spatial data would be required to identify these effects.
...
On the consistent 8-patient/11-guild cohort, the plain gLV achieves RMSE\,$=0.060$ ($r=0.927$), identical to Hamilton+CLSM (Table~\ref{tab:rmse_guild}); adding the PerLive growth-rate scaling to gLV (gLV+struct) does not improve this further (RMSE\,$=0.060$), while the same PerLive signal reduces Hamilton ODE RMSE from 0.105 to 0.060.
...
Leave-one-patient-out cross-validation confirms that gLV generalises well: pure gLV LOO-CV RMSE\,$= 0.058$ (vs.\ train\,$= 0.060$), and adding PerLive scaling (gLV+struct) yields LOO-CV\,$= 0.059$---a negligible $\Delta = 0.001$ difference---confirming that the PerLive signal provides no generalisation gain for gLV, consistent with the free-space interpretation.
...
We compared the signs of the inferred guild A matrix (11-guild, 8-patient consistent cohort) with the net metabolite-flow predictions derived from Dieckow Supplementary File~1 (347 species-level PRODUCES/USES/IS\_INHIBITED\_BY records, aggregated to the 11-guild level).
Metabolite interactions were weighted by database evidence quality: KEGG-annotated metabolites ($n=26$ rows) received weight~2, HMDB-annotated metabolites ($n=239$ rows) received weight~2, and unannotated records weight~1.
Of 19 guild pairs with at least one Supp.\ File~1 prediction, 19 (100\%) showed sign agreement.

The KEGG/HMDB-weighted prior achieves full sign agreement (19/19, 100\%) while simultaneously improving train RMSE from 0.060 to 0.045 ($r=0.963$); the 8-fold LOO-CV RMSE is 0.074, slightly higher than the plain gLV LOO (0.058), reflecting the trade-off between biological plausibility and generalisation imposed by the sign penalty.
A grid search over KEGG weight $\in\{2,3,5\}$, HMDB weight $\in\{1.5,2.0\}$, and $\sigma\in\{0.10,0.12,0.15\}$ revealed that the 100\% agreement region corresponds to HMDB weight\,$=2.0$ and $\sigma\!=\!0.15$; KEGG weight had negligible additional effect beyond the HMDB signal, confirming that the 239 HMDB-annotated rows carry the dominant constraint.

All 19 ecologically meaningful pairs are now consistent: the most prominent include Actinobacteria$\to$Bacilli ($+$, co-aggregation mediated by fimbrial adhesins), Negativicutes$\to$Bacilli ($+$, lactic-acid cross-feeding from \textit{Streptococcus} to \textit{Veillonella}), and Bacteroidia$\to$Negativicutes ($+$, amino-acid cross-feeding).
This result demonstrates that KEGG/HMDB-derived metabolite flow encodes genuine ecological signal that is recoverable from in-vivo 16S time-series data.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/dieckow/fig9_guild_vs_suppfile1.pdf}
\caption{Comparison of inferred guild A matrix (11-guild, 8-patient, RMSE\,=\,0.060) with Dieckow Supp.\ File~1 predicted interactions. Left: inferred A matrix (KEGG/HMDB-weighted sign prior, L-BFGS-B). Centre: net metabolite-flow score from Supp.\ File~1 (positive = net PRODUCES$\to$USES; negative = net PRODUCES$\to$IS\_INHIBITED\_BY); metabolites are weighted by database evidence (KEGG$\times$2, HMDB$\times$2). Right: sign agreement (green\,=\,agree, red\,=\,disagree, grey\,=\,no Supp.\ File~1 prediction). All 19 predicted pairs agree (100\%) with the KEGG/HMDB-weighted sign prior (kegg$\times$2, HMDB$\times$2, $\sigma\!=\!0.15$).}
\label{fig:sf1cmp}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/dieckow/fig_top_interactions.pdf}
\caption{Guild interaction strength and metabolite-mediated pathways.
\textbf{A.}~Top 10 off-diagonal entries of the KEGG/HMDB-prior gLV $A$ matrix ranked by $|A_{ij}|$ (RMSE\,$=0.045$, $r=0.963$, 8 patients, 11 guilds).
Green bars indicate guild pairs for which at least one metabolite mediator is recorded in Dieckow Supp.\ File~1; grey bars indicate pairs lacking a direct SF1 annotation.
Metabolite labels (italic) are shown for SF1-supported interactions.
\textbf{B.}~Schematic of the three dominant metabolite-mediated pathways recovered from SF1.
Green arrows: positive (cross-feeding) interactions; red arrows: negative (inhibitory) interactions.
The nitrite channel (Actinobacteria$\to$Bacilli, $A=+4.51$), the H$_2$O$_2$ channel (Bacilli$\to$Actinobacteria, $A=-1.27$), and the CO$_2$ cross-feeding (Negativicutes$\to$Bacilli, $A=+0.89$) are each supported by primary literature on oral ecology.}
\label{fig:top_interactions}
\end{figure}

\paragraph{Independent metabolic network validation.}
To assess whether the sign structure of the KEGG/HMDB-prior gLV $A$ matrix is consistent with first-principles metabolic modelling, we constructed manually curated minimal COBRApy flux-balance models for each guild (10--12 reactions per model, based on HOMD and established oral ecology~\cite{Kolenbrander2010OralMultispecies}).
Each model was optimised in an oral-fluid medium using parsimonious FBA; sign predictions were assigned for guild pairs where net cross-feeding flux exceeded threshold (positive) or where a toxic metabolite (H$_2$O$_2$, H$_2$S) was secreted in excess (negative).
Of 7 guild pairs for which the minimal models yielded a definite sign prediction, 6 (86\,\%) agreed with the KEGG/HMDB-prior gLV inferred signs (Fig.~\ref{fig:minimal_model_signs}).
The three strongest positive predictions matched: Gammaproteobacteria$\to$Actinobacteria via L-lactate ($A=+0.44$), Bacilli$\to$Betaproteobacteria via H$_2$O$_2$ ($A=+0.42$), and Fusobacteriia$\to$\{Actinobacteria, Negativicutes\} via acetate.
The single disagreement (Bacteroidia$\to$Gammaproteobacteria, gLV $A=-0.081$, minimal model: +succinate cross-feeding) is biologically plausible: Bacteroidia also produce H$_2$S, whose inhibitory effect can dominate over succinate cross-feeding under oral conditions.

As a complementary genome-scale check, we ran parsimonious FBA on AGORA~v1.03 SBML reconstructions for each guild representative (10 organisms; oral-fluid medium with carbon sources capped, vitamins unrestricted).
The AGORA models confirm the two canonical cross-feeding axes at the flux level:
(i)~\textit{Streptococcus gordonii} (Bacilli) secretes L-lactate at 982~mmol~gDW$^{-1}$~h$^{-1}$, and Veillonella (Negativicutes) carries an open exchange for L-lactate, supporting $A[\text{Negativicutes}\!\leftarrow\!\text{Bacilli}]=+0.04$;
(ii)~\textit{Eikenella corrodens} (Betaproteobacteria) secretes succinate at 53~mmol~gDW$^{-1}$~h$^{-1}$, which Fusobacteriia can import, consistent with $A[\text{Fusobacteriia}\!\leftarrow\!\text{Betaproteo}]=+0.14$.
Inhibitory interactions mediated by H$_2$O$_2$ (e.g.\ $A[\text{Actinobacteria}\!\leftarrow\!\text{Bacilli}]=-1.27$) cannot be recovered by single-organism pFBA, as reactive oxygen species dynamics require explicit community modelling; these remain a limitation of the genome-scale validation.
Guild-resolved secretion profiles are provided as a supplementary heatmap (Fig.~\ref{fig:agora_sign_comparison}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/dieckow/fig_minimal_model_signs.pdf}
\caption{Independent validation of gLV interaction signs using manually curated minimal metabolic models.
Ten guild-level COBRApy flux-balance models (10--12 reactions each, one per guild, based on HOMD and Kolenbrander 2010) were optimised in an oral-fluid medium using parsimonious FBA.
Sign predictions (positive = net cross-feeding; negative = H$_2$O$_2$ or H$_2$S toxicity) are compared with KEGG/HMDB-prior gLV inferred signs for all guild pairs with a definite prediction.
Of 7 pairs with a definite prediction, 6 (86\,\%) agreed.
Driver metabolites are annotated for each predicted pair (lac = L-lactate; h2o2 = hydrogen peroxide; ac = acetate; succ = succinate).
The single disagreement (Bacteroidia$\to$Gammaproteobacteria) is attributed to competing H$_2$S toxicity dominating over succinate cross-feeding.}
\label{fig:minimal_model_signs}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/dieckow/fig_agora_sign_comparison.pdf}
\caption{AGORA~v1.03 pFBA secretion profiles for the 10 oral guilds under an oral-fluid medium.
Each cell shows the net secretion flux (mmol~gDW$^{-1}$~h$^{-1}$) for a key metabolite class from parsimonious FBA on the AGORA~v1.03 (With Mucins) SBML reconstruction of the guild representative organism.
Carbon sources were capped at oral-fluid levels; vitamins and cofactors were left at model defaults to ensure feasible growth.
Guild representatives: Actinomyces~naeslundii (Actinobacteria), Atopobium~parvulum (Coriobacteriia), Streptococcus~gordonii (Bacilli), Parvimonas~micra (Clostridia), Veillonella~parvula~Te3 (Negativicutes), Prevotella~melaninogenica (Bacteroidia), Capnocytophaga~sputigena (Flavobacteriia), Fusobacterium~nucleatum (Fusobacteriia), Eikenella~corrodens (Betaproteobacteria), Haemophilus~parainfluenzae (Gammaproteobacteria).
The strong lactate secretion by Bacilli (Streptococcus) and succinate secretion by Betaproteobacteria (Eikenella) and Bacteroidia (Prevotella) are consistent with the corresponding positive gLV entries.}
\label{fig:agora_sign_comparison}
\end{figure}
...
Patients~D and~L were excluded from the Hamilton ODE and LOO analysis due to incomplete time-series data (D: week~2 only; L: week~3 only), leaving only three CT1 patients (E, G, K) among the eight used for the guild model.
This imbalance means CT1 is under-represented relative to CT2 ($n=3$ vs $n=5$), and CT1-specific interaction estimates are based on fewer independent replicates; future cohorts with complete data from all participants would reduce this bias.
The Hamilton LOO reported here is approximate (shared $A$ fixed from full-cohort fit), which is optimistic relative to a proper LOO ($A$ re-fitted on $n-1$ patients); the true generalisation error is likely higher than 0.086.
...
\item \textbf{Guild-level gLV with metabolite-flow prior.}
An 11-guild gLV with KEGG/HMDB-weighted sign prior achieves RMSE\,$=0.045$ ($r=0.963$), with all 19 predicted guild pairs agreeing in sign with the SF1 metabolite-flow network (100\%; $\sigma=0.15$, grid-searched).
The three strongest off-diagonal entries ($|A_{ij}|>0.89$) each have a direct metabolite-pathway explanation from SF1:
(i)~Actinobacteria$\to$Bacilli ($A=+4.51$) via nitrite cross-feeding (oral nitrate reducers stimulate \textit{Streptococcus});
(ii)~Bacilli$\to$Actinobacteria ($A=-1.27$) via hydrogen peroxide inhibition (\textit{S.~gordonii} H$_2$O$_2$ suppresses \textit{Actinomyces});
(iii)~Negativicutes$\to$Bacilli ($A=+0.89$) via CO$_2$ cross-feeding (\textit{Veillonella} CO$_2$ by-product sustains \textit{Streptococcus} CO$_2$-dependent growth).
Four of the top eight interactions by strength lack a direct SF1 record, marking gaps in metabolite annotation as candidates for experimental validation (Fig.~\ref{fig:top_interactions}).
Independently, manually curated minimal metabolic models (COBRApy pFBA, one per guild) achieve 6/7 (86\,\%) sign agreement with the inferred gLV signs (Fig.~\ref{fig:minimal_model_signs}), confirming the metabolic plausibility of the dominant interaction signs.
...
The free-space channel~$\varphi_0 = 1 - \tilde\omega$, unique to the Hamilton formulation, is the dominant structural-data carrier; the simplex-constrained gLV replicator cannot represent this degree of freedom.
\end{enumerate}
...
A proper leave-one-patient-out CV for the Hamilton+CLSM model (with $A$ re-fitted on $n-1$ patients) would replace the approximate LOO (RMSE\,$=0.086$) with a more reliable generalisation estimate.
Extending the cohort to include more patients---especially CT1-type patients D and L for whom complete time-series could be obtained---would reduce the current under-representation of CT1 ($n=3$ among the 8 analysis patients) and improve CT-stratified model reliability.
...
\textbf{Biological network validation.}
The four top-strength guild interactions lacking SF1 metabolite annotation (Fig.~\ref{fig:top_interactions}) are candidates for targeted experimental follow-up.
As a preliminary step, manually curated minimal COBRApy guild models achieve 6/7 (86\,\%) sign agreement with gLV-inferred signs (Fig.~\ref{fig:minimal_model_signs}); the single disagreement (Bacteroidia$\to$Gammaproteobacteria) is consistent with competing H$_2$S toxicity.
A more comprehensive grounding would be provided by genome-scale community metabolic modelling using the AGORA2 resource~\cite{Thiele2020AGORA}, which could supply continuous flux magnitudes and replace binary sign predictions with quantitative interaction strengths.
Per-interaction annotation against primary literature (e.g.\ the Kolenbrander oral ecology compendium) would further strengthen causal mechanistic assignments for the top guild pairs.
