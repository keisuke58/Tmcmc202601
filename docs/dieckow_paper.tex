...
``---'' entries: LOO-CV runs in progress.}
...
Hamilton ODE (no struct)             & 10 & 0.105 & ---   & none\\
Hamilton ODE + CLSM (this work)      & 11 & \textbf{0.060} & --- & $\tilde\omega$, $q_{p,w}$, $\alpha$\\
\bottomrule
\end{tabular}
\end{table}
...
\caption{CT-stratified gLV interaction matrices. Left: CT1-specific $A$ (patients E, G, K). Centre: CT2-specific $A$ (patients A, B, C, F, H). Right: difference $\Delta A = A_{\text{CT2}} - A_{\text{CT1}}$ (green\,$=$ stronger in CT2, purple\,$=$ stronger in CT1).}
\label{fig:ct_Amat}
\end{figure}
