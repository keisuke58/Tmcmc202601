...
**cuSolver エラー (gpusolverDnCreate failed)**:
- `hamilton_ode_jax.py` で `jnp.linalg.solve` を純 JAX 実装に置換済み（cuSOLVER 不使用）
...
# 対処: JAX_PLATFORMS=cuda を明示（スクリプトに組み込み済み）
JAX_PLATFORMS=cuda bash run_jax_ode_nuts.sh --test

# LD_LIBRARY_PATH がシステム CUDA と競合する場合
...
# GPU 環境セットアップ
cd data_5species/main
bash setup_jax_gpu.sh --existing   # 既存 klempt_fem2 に追加
...
```bash
# JAX 0.9.0.1 使用。数値検証は verify_jax_ode_numerical.py で実施
cd data_5species/main
python verify_jax_ode_numerical.py
# PASS: 観測インデックスでの差 ~1e-4、実用上問題なし
```
...
1. テスト: `bash run_jax_ode_nuts.sh --test` または Stuttgart `--quick`
2. 数値検証: `python verify_jax_ode_numerical.py` で JAX vs NumPy 一致確認
3. 考察: ログ・samples.npy を確認
4. 本番: `bash run_jax_ode_nuts.sh --production` または Stuttgart 本番

※ Stuttgart は libffi.so.8 等の conda 環境依存で失敗する場合あり。その場合は fifawc で本番実行。

**GPU が使えない環境（copaam 等）**:

```bash
# jax[cuda12] の代わりに jax[cpu] を入れると cuInit エラーを回避
pip uninstall jax jaxlib jax-cuda12-plugin -y 2>/dev/null
pip install -r requirements-jax-cpu.txt
# または: pip install "jax[cpu]" pandas requests numba equinox matplotlib
```
...
| JAX ODE 数値検証 | `data_5species/main/verify_jax_ode_numerical.py` |
