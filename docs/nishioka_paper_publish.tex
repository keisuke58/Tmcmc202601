...
The results (Table~\ref{tab:pairwise}) show that matrices within the same health state share a common backbone ($\rho_{\text{CH--CS}}=+0.26$, $\rho_{\text{DH--DS}}=+0.40$), whereas most cross-state comparisons yield negative correlations ($\rho_{\text{DH--CS}}=-0.49$, $\rho_{\text{CS--DS}}=-0.27$) and large Frobenius distances ($\Delta_F\geq0.88$).
The exception is the CH--DH pair ($\rho=+0.24$), which share the HOBIC cultivation mode; the positive correlation suggests that flow-induced shear partially preserves the interaction topology across health states.
...
\midrule
CS & \textbf{0.273} & 0.135 & 0.111 & 0.244 \\
CH & 0.229 & \textbf{0.168} & 0.127 & 0.233 \\
DS & 0.273 & 0.130 & \textbf{0.062} & 0.252 \\
DH & 0.301 & 0.129 & 0.044 & \textbf{0.262} \\
\bottomrule
\end{tabular}
