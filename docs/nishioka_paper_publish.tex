...
\textbf{Note:} diagonal values exceed the Phase~2 in-sample RMSE (Table~\ref{tab:rmse}) because all cross-condition evaluations---including self---apply the Phase~2 MAP $\hat{\btheta}$ with $\psi_i$ fixed to the \emph{target} condition's experimental values (fix-$\psi$ mode), ensuring a like-for-like comparison; the Phase~2 in-sample fit additionally benefits from the jointly optimised $\psi_i$ trajectory.}
