...
The forward model is compiled via JAX and evaluated in parallel on GPU (NVIDIA RTX\,4090), achieving a $\sim$170$\times$ per-stage speedup over the CPU baseline.
...
Second, we compile the forward model in JAX~\cite{jax2018github} and evaluate the entire particle ensemble on GPU via batched \texttt{vmap}, achieving a $\sim$170$\times$ per-stage speedup over the CPU baseline.
...
Four remaining pairs lack confirmed metabolic or co-aggregation pathways; rather than enforcing these as hard constraints ($\theta_k\equiv0$), we estimate all 15 parameters and let the posterior determine which interactions are active.
...
This yields a $\sim$170$\times$ per-stage speedup ($\sim$150$\times$ end-to-end including CPU overhead; Table~\ref{tab:timing}).
...
\quad Per stage     & 2\,400\,s   & 14\,s    & $\mathbf{170\times}$\\
\quad Total         & 21\,600\,s  & 147\,s   & $\sim$150$\times$$^\ddagger$\\
\midrule
...
The matched benchmark isolates the GPU speedup: per-stage (pure XLA kernel) achieves $\mathbf{170\times}$; the end-to-end total ($\sim$150$\times$) is lower because resampling, $\beta$-bisection, and covariance estimation run on CPU.
...
$^\ddagger$21\,600\,s\,/\,147\,s\,$\approx$\,147$\times$; per-stage ratio 2\,400/14\,$\approx$\,170$\times$.}
\label{tab:timing}
\end{table}
...
The proposal covariance (Eq.~\ref{eq:cov}) is scaled by $\gamma^2$, initialised to $2.38^2/d\approx0.38$ ($d=15$) following the Roberts et al.\ optimal scaling for Gaussian targets, and adapted online within each stage: $\gamma$ is halved when the acceptance rate drops below 5\% and increased by 50\% when it exceeds 40\%.
...
The RMSE increase from Phase~1 to Phase~2 ($\Delta$RMSE$\leq0.034$ across all conditions) confirms that the Phase~1 interaction structure is robust to the release of $\psi_i$.
...
In dysbiotic states, the model captures the pathogen dynamics with high fidelity: DH achieves $R^2>0.8$ for An, Vei, and Pg; DS achieves $R^2=0.91$ for Vei and $0.85$ for Pg (Phase~2).
So shows negative $R^2$ across most conditions in both phases.
This is a genuine model limitation: the symmetric quadratic interaction structure produces a So trajectory that overshoots the observed decline after the initial growth peak, yielding residual variance $\mathrm{SS}_{\mathrm{res}} > \mathrm{SS}_{\mathrm{tot}}$.
In Phase~2, the additional $\psi$-dynamics compound this by shifting the MAP away from the So-optimal region to accommodate viability constraints (CH is the exception: $R^2=0.40$ in Phase~2).
So also exhibits the highest replicate variability ($\sigma_{\mathrm{So}}\approx0.19$), so even moderate absolute errors translate to $R^2<0$.
The remaining species and the global RMSE are not affected; this limitation of the symmetric $\mathbf{A}$ for the dominant early coloniser is noted as a target for future model refinement.
...
For Phase~2, a full multi-chain analysis is computationally prohibitive ($\sim$2\,h per run); convergence is instead supported by three indirect checks: (i)~the $\beta$-schedule reaches $\beta_M=1$ within $M=4$--$7$ stages, consistent with Phase~1, indicating no tempering failure; (ii)~the log-evidence increment $\Delta\ln\hat{Z}$ is monotonically decreasing and stabilises at the final stage for all conditions; and (iii)~the Phase~1 and Phase~2 MAP estimates are structurally consistent (Figure~\ref{fig:phase_scatter}), suggesting Phase~2 converges to the same basin as Phase~1 for commensal conditions.
The acceptance rates of 7--13\% are below the theoretically optimal 23\% for $d=15$ Gaussian targets~\cite{RobertsRosenthal2001Optimal}, reflecting the non-Gaussianity and bounded support of the posterior; nevertheless, the high ESS confirms that the TMCMC resampling--mutation cycle provides sufficient mixing.
...
The $\sim$170$\times$ per-stage GPU speedup (Table~\ref{tab:timing}) exploits the embarrassingly parallel RW mutation via \texttt{jax.vmap}.
...
Compiling the forward model via JAX and batching all particles on GPU achieves a $\sim$170$\times$ per-stage speedup (Table~\ref{tab:timing}), making production runs with $N_p=10{,}000$ particles feasible.
