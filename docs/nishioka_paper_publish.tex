...
\textit{pH prediction} (LOO-$R^2 = 0.920$; independent $R^2 = 0.778$):
...
To evaluate the predictive power of the inferred $\mathbf{A}$ matrix, we drew 500 samples from the Phase-2 TMCMC posterior ($10\,000$-particle NUTS run) and integrated the Hamilton ODE forward using Day-1 experimental species fractions as initial conditions, yielding posterior species trajectories $\phi_i(t)$ at $t=1,3,6,10,15,21$\,days.
...
\begin{figure}[htbp]
  ...
  \caption{%
    ...
    circles: experimental data from Heine et al.~\cite{Heine2025PeriImplant}.
