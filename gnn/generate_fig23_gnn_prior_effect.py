...
def main():
    ...
    for k, tidx in enumerate(ACTIVE_THETA_IDX):
        ...
        print(
            f"  {EDGE_NAMES[k]:8s} (θ[{tidx:2d}]): OL={ol:.3f}, "
    ...
    for idx in range(n_params):
        ...
        ax.set_title(
            f"{PARAM_NAMES[idx]}{star}  OL={ol:.2f}",
            fontsize=8.5,
            color=olc,
    ...
    fig.suptitle(
        "Fig. 23: GNN-Informed Prior Effect on TMCMC Posterior (Dysbiotic HOBIC)",
        fontsize=13,
        fontweight="bold",
        y=0.98,
    )
    ...
    for k, tidx in enumerate(ACTIVE_THETA_IDX):
        ...
        if gnn_pred is not None:
            ax.axvline(gnn_pred[k], color=c_pred, lw=2.5, ls="--", alpha=0.8, label="GNN pred")
    ...
    fig2.suptitle(
        "GNN Prior Effect on Interaction Parameters (DH condition)", fontsize=11, fontweight="bold"
    )
