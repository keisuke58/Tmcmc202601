...
from material_models import (
    ...
    E_MAX_PA, E_MIN_PA, PHI_PG_CRIT, HILL_M,
    _hill_sigmoid, IDX_PG, IDX_FN,
)

# ── 条件定義 ──
CONDITIONS = {
    "commensal_static" : {"run": "commensal_static",  "color": "#1f77b4", "label": "Commensal\nStatic"},
    "commensal_hobic"  : {"run": "commensal_hobic",   "color": "#2ca02c", "label": "Commensal\nHOBIC"},
    "dysbiotic_static" : {"run": "dysbiotic_static",  "color": "#ff7f0e", "label": "Dysbiotic\nStatic"},
...
def solve_0d(theta_np, n_steps=2500, dt=0.01):
    ...
    params = {
        ...
        "A": A, "b_diag": b_diag,
        "active_mask": active_mask, "newton_steps": 6,
    }
    ...
    return {
        "phi_final": phi_final, "phi_traj": phi_traj,
        "di": di, "phi_Pg": float(phi_final[IDX_PG]),
        "phi_Fn": float(phi_final[IDX_FN]),
        "A": np.array(A), "b_diag": np.array(b_diag),
    }
...
def main():
    ...
    for ckey, meta in CONDITIONS.items():
        ...
        res["theta"] = theta
        results[ckey] = res
        print(f"[{ckey}] DI={res['di']:.3f}  φ_Pg={res['phi_Pg']:.4f}  "
    ...
    for sp_idx in range(5):
        ...
        bars = ax_comp.bar(x, vals, bottom=bottom, width=0.65,
                           color=SP_COLORS[sp_idx], label=SPECIES[sp_idx],
                           edgecolor="white", linewidth=0.5)
        # annotate dominant species
        for i, v in enumerate(vals):
            if v > 0.3:
                ax_comp.text(x[i], bottom[i] + v/2, f"{SP_SHORT[sp_idx]}\n{v:.2f}",
    ...
    ax_net.axis("off")
    ax_net.set_title("(b) Hamilton model interaction network\n"
                     "(locked edges dashed, Hill gate = red)", fontsize=9)
    ...
    for i in range(5):
        ...
        ax_net.text(cx, cy, SP_SHORT[i], ha="center", va="center",
    ...
    for i, j, locked, label in edges:
        ...
        if locked:
            ...
            ax_net.text(mx, my, "X", ha="center", va="center",
                        fontsize=8, color="gray", fontweight="bold", zorder=2)
        else:
            ...
            ax_net.text(mx+nx, my+ny, label, ha="center", va="center",
                        fontsize=6, color=color, style="italic", zorder=4)

    # Hill gate annotation
    ax_net.annotate("Hill gate\n(K=0.05, n=4)",
                    xy=((positions[3][0]+positions[4][0])/2,
    ...
    for ck in ckeys:
        r = results[ck]
        meta = CONDITIONS[ck]
        ax_di.scatter(r["di"], r["phi_Pg"], s=200, c=meta["color"],
                      edgecolor="black", zorder=5, marker="o")
        dominant = SP_SHORT[np.argmax(r["phi_final"])]
        ax_di.annotate(f"{meta['label'].replace(chr(10),' ')}\n"
                       f"dominant: {dominant}",
                       (r["di"], r["phi_Pg"]),
    ...
    ax_di.axhline(PHI_PG_CRIT, color="red", ls="--", alpha=0.5,
    ...
    ax_di.annotate("DI discriminates\nconditions well\n(horizontal spread)",
                   xy=(0.5, 0.02), fontsize=7, color="blue",
                   ha="center", style="italic")
    ax_di.annotate("$\\varphi_{Pg}$ fails\n(all below $\\varphi_{crit}$)",
                   xy=(0.1, PHI_PG_CRIT + 0.02), fontsize=7, color="red",
                   ha="center", style="italic")
    ...
    w = 0.3
    ax_E.bar(x - w/2, E_di_vals, w, label="DI model",
             color=[CONDITIONS[ck]["color"] for ck in ckeys],
             alpha=0.85, edgecolor="black")
    ax_E.bar(x + w/2, E_pg_vals, w, label=r"$\varphi_{Pg}$ model",
             color=[CONDITIONS[ck]["color"] for ck in ckeys],
             alpha=0.35, edgecolor="black", hatch="///")
    ...
    if E_di_vals[0] > 0 and E_di_vals[2] > 0:
        ...
        ax_E.text(0.5, 0.02, f"DI ratio: {ratio_di:.0f}x    $\\varphi_{{Pg}}$ ratio: {ratio_pg:.1f}x",
                  transform=ax_E.transAxes, fontsize=8, ha="center",
    ...
    for sp_idx in range(5):
        ax_traj.plot(t_axis, ds_res["phi_traj"][:, sp_idx],
                     color=SP_COLORS[sp_idx], lw=1.5, label=SP_SHORT[sp_idx])
    ax_traj.set_xlabel("T* (dimensionless time)")
    ax_traj.set_ylabel("Volume fraction")
    ax_traj.set_title("(e) 0D trajectory: Dysbiotic Static\n(S. oralis dominates, NOT P. gingivalis)")
    ...
    ax_sigma.bar(x - w/2, sigma_di, w, label="DI model",
                 color=[CONDITIONS[ck]["color"] for ck in ckeys],
                 alpha=0.85, edgecolor="black")
    ax_sigma.bar(x + w/2, sigma_pg, w, label=r"$\varphi_{Pg}$ model",
                 color=[CONDITIONS[ck]["color"] for ck in ckeys],
                 alpha=0.35, edgecolor="black", hatch="///")
    ...
    print("="*80)
    print(f"{'Condition':<22} {'Dominant':>8} {'φ_dom':>7} {'DI':>6} {'φ_Pg':>7} "
          f"{'E_di':>6} {'E_φPg':>7} {'ratio':>6}")
    print("-"*80)
    for i, ck in enumerate(ckeys):
        r = results[ck]
        dom_idx = np.argmax(r["phi_final"])
        print(f"  {ck:<20} {SP_SHORT[dom_idx]:>8} {r['phi_final'][dom_idx]:>7.3f} "
