...
THETA_BLOCKS = (
    ["M1"]*5 + ["M2"]*5 + ["M3"]*4 + ["M4"]*2 + ["M5"]*4
)
BLOCK_COLORS = {"M1":"royalblue","M2":"forestgreen","M3":"goldenrod",
                "M4":"crimson","M5":"mediumpurple"}
...
def fig1_heatmaps(phi, t, x, out_path: Path, condition: str = ""):
    ...
    for i, ax in enumerate(axes):
        ...
        try:
            cs = ax.contour(t, x, z, levels=[0.25*vmax, 0.5*vmax, 0.75*vmax],
...
def fig2_time_series(phi, t, x, out_path: Path, condition: str = "",
    ...
    fig.suptitle(f"Time Series at Key Spatial Locations{_sfx(condition)}",
...
def fig3_spatial_profiles(phi, t, x, out_path: Path, condition: str = ""):
    ...
    fig.suptitle(f"Spatial Profiles at Key Times{_sfx(condition)}",
                 fontsize=13, fontweight="bold")
...
def fig4_final_composition(phi, phi0, t, x, out_path: Path, condition: str = ""):
    ...
    fig.suptitle(f"Final Spatial Composition (t={t[-1]:.4f}){_sfx(condition)}",
                 fontsize=13, fontweight="bold")
    ...
    ax = axes[1]
    ax.stackplot(x, [rel_phi[:, i] for i in range(5)],
                 labels=SPECIES_NAMES, colors=SPECIES_COLORS, alpha=0.80)
    ax.set_xlim(0, x[-1])
    ax.set_ylim(0, 100)
    ax.set_xlabel("Position x  (← implant surface | bulk →)", fontsize=10)
...
def fig5_pathogen_front(phi, t, x, out_path: Path, condition: str = ""):
    ...
    ax.set_xlabel("Position x"); ax.set_ylabel("φ_Pg")
    ax.set_title("Spatial profile over time (purple→yellow = early→late)")
    ax.set_xlim(0, x[-1]); ax.set_ylim(bottom=0); ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Hamilton time t"); ax.set_ylabel("Invasion front x")
    ax.set_title("Front propagation (rightmost position exceeding threshold)")
    ax.legend(fontsize=9); ax.grid(True, alpha=0.3)
    ax.set_xlim(t[0], t[-1]); ax.set_ylim(0, x[-1])
...
def fig6_dysbiotic_index(phi, t, x, out_path: Path, condition: str = ""):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle(f"Dysbiotic Index DI(x,t) = 1 − H/H_max{_sfx(condition)}",
                 fontsize=13, fontweight="bold")
    ...
    ax.set_xlabel("Hamilton time t"); ax.set_ylabel("Position x")
    ax.set_title("DI(x,t)  [red = dysbiotic, green = healthy]")

    # Right: time series at key nodes
    ax = axes[1]
    k0 = 0; km = len(x)//2; kL = len(x)-1
    ...
    ax.set_xlabel("Hamilton time t"); ax.set_ylabel("Dysbiotic Index DI")
...
def fig7_surface_vs_bulk(phi, t, x, out_path: Path, condition: str = "",
    ...
    fig.suptitle(f"Ecology: Implant Surface vs Bulk{_sfx(condition)}",
                 fontsize=13, fontweight="bold")

    for ax, k, loc in zip(axes, [k0, kL],
                          ["x = 0  (implant surface)", f"x = {x[-1]:.1f}  (bulk medium)"]):
        for i in range(5):
...
def fig8_summary_panel(phi, phi0, t, x, theta, out_path: Path, condition: str = ""):
    ...
    fig.suptitle(f"FEM Spatial Extension — Summary{_sfx(condition)}",
    ...
    for i in range(5):
        ax.plot(t, gaussian_filter1d(phi[:, k0, i], 0.5),
                color=SPECIES_COLORS[i], label=SPECIES_SHORT[i], lw=1.8)
    ax.set_title("(D)  Surface (x=0) dynamics", fontweight="bold")
    ax.set_xlabel("Hamilton time"); ax.set_ylabel("φᵢ")
    ...
    ax.set_xlabel("Position x"); ax.set_ylabel("Volume fraction")
    ax.legend(fontsize=7, loc="upper right", ncol=2)
    ax.set_xlim(0, x[-1]); ax.set_ylim(0, 1.0)
    ax.axhline(1.0, color="black", lw=0.8, ls="--", alpha=0.4)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_xlabel("Hamilton time"); ax.set_ylabel("Front position x")
...
def parse_args():
    ...
    p.add_argument("--condition", default="",
                   help="Condition label for plot titles")
    return p.parse_args()
...
def main():
    ...
    if not rdir.exists():
        print(f"Results directory not found: {rdir}"); sys.exit(1)
