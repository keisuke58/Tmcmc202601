...
def main():
    ...
    ap.add_argument("--pressures", nargs="+", type=float,
    ...
    ap.add_argument("--dry-run",  action="store_true",
                    help="Generate INPs but do not run Abaqus")
    ...
    for p_pa in pressures_pa:
        ...
        if not args.dry_run:
            ...
            if os.path.exists(elem_csv) and os.path.exists(node_csv):
                ...
                print(f"  MISES median={results[-1]['mises_median']:.4f} MPa  "
                      f"max={results[-1]['mises_max']:.4f} MPa  "
                      f"|U|_max={results[-1]['umag_max']:.3e} mm")
        else:
            results.append({"pressure_mpa": p_mpa,
                             "mises_median": None, "mises_max": None,
                             "umag_max": None, "umag_outer_med": None})
