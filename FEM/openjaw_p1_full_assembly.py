...
def _parse(argv):
    ...
    while i < len(argv):
        a = argv[i]
        def nxt(): return argv[i+1] if i+1 < len(argv) else None
        if a == "--bbox-json"    and nxt(): cfg["bbox_json"]    = nxt(); i+=2; continue
        if a == "--field-csv"   and nxt(): cfg["field_csv"]    = nxt(); i+=2; continue
        if a == "--case"        and nxt(): cfg["case"]          = nxt(); i+=2; continue
        if a == "--biofilm-frac"and nxt(): cfg["biofilm_frac"] = float(nxt()); i+=2; continue
        if a == "--pocket-depth"and nxt(): cfg["pocket_depth"] = float(nxt()); i+=2; continue
        if a == "--pocket-width"and nxt(): cfg["pocket_width"] = float(nxt()); i+=2; continue
        if a == "--aniso-ratio" and nxt(): cfg["aniso_ratio"]  = float(nxt()); i+=2; continue
        if a == "--n-bins"      and nxt(): cfg["n_bins"]        = int(nxt());   i+=2; continue
        if a == "--e-max"       and nxt(): cfg["e_max"]         = float(nxt()); i+=2; continue
        if a == "--e-min"       and nxt(): cfg["e_min"]         = float(nxt()); i+=2; continue
        if a == "--di-scale"    and nxt(): cfg["di_scale"]      = float(nxt()); i+=2; continue
        if a == "--di-exponent" and nxt(): cfg["di_exponent"]   = float(nxt()); i+=2; continue
        if a == "--nu"          and nxt(): cfg["nu"]             = float(nxt()); i+=2; continue
        if a == "--pressure-mpa"and nxt(): cfg["pressure_mpa"] = float(nxt()); i+=2; continue
        if a == "--poly-from-json":        cfg["poly_from_json"]= True;         i+=1; continue
        if a == "--cae-out"     and nxt(): cfg["cae_out"]        = nxt(); i+=2; continue
        if a == "--crown-job"   and nxt(): cfg["crown_job"]      = nxt(); i+=2; continue
        if a == "--slit-job"    and nxt(): cfg["slit_job"]       = nxt(); i+=2; continue
        i += 1
...
def _assign_di_bins(part, cx, cy, R_inner, biofilm_th,
                     coords, di_vals, di_max, n_bins):
    """
...
def build_crown_case(model, cfg, bb23, coords, di_vals):
    from abaqusConstants import (
        THREE_D, DEFORMABLE_BODY, ON, OFF,
        FROM_SECTION, MIDDLE_SURFACE, CARTESIAN,
        SYSTEM, AXIS_1, ROTATION_NONE, STACK_3,
    )
    from regionToolset import Region
    ...
    bin_labels = _assign_di_bins(
        part, t_cx, t_cy, R_inner, biofilm_th,
        coords, di_vals, di_max, cfg["n_bins"])
    ...
    part.MaterialOrientation(
        ...
        localCsys=local_csys, fieldName="",
...
def build_crown_bcs(model, inst, t_zmin, t_sz, pressure_pa, seed_size):
    ...
    if top_faces:
        ...
        print("  Crown BCs: pressure %.3g Pa on %d top faces" % (
            pressure_pa, len(top_faces)))
...
def build_slit_case(model, cfg, bb30, bb31, coords, di_vals):
    ...
    """
    from abaqusConstants import (
        THREE_D, DEFORMABLE_BODY, ON, CARTESIAN,
        SYSTEM, AXIS_1, ROTATION_NONE, STACK_3,
    )
    from regionToolset import Region

    c30 = bb30["center"]; s30 = bb30["size"]
    c31 = bb31["center"]; s31 = bb31["size"]
    ...
    print("  Slit: mid=(%.2f,%.2f)  depth=%.2f  width=%.2f  z=%.2f..%.2f" % (
    ...
    for elem in part.elements:
        ...
        sx = sy = 0.0; nc = 0
        for nd in nodes:
            c = nd.coordinates; sx += c[0]; sy += c[1]; nc += 1
        if nc == 0:
            continue
        ex, ey = sx/nc, sy/nc
...
def build_slit_bcs(model, inst, z_lo, slit_h, slit_half_depth, nx, ny,
    ...
    if top_faces:
        model.Pressure(
            ...
            region=Region(side1Faces=top_faces),
            magnitude=pressure_pa)
        print("  Slit BCs: pressure %.3g Pa on %d top faces" % (
            pressure_pa, len(top_faces)))
...
def _build_and_run(cfg):
    ...
    if run_crown:
        ...
        crown_job = mdb.Job(
            name=cfg["crown_job"], model=model_name, numCpus=1,
    ...
    if run_slit:
        ...
        build_slit_bcs(slit_model, inst_slit, z_lo, slit_h, half_dep, nx, ny,
                       mid_x, mid_y, pressure_pa, seed_size_slit)

        slit_job = mdb.Job(
            name=cfg["slit_job"], model=slit_model_name, numCpus=1,
...
def main():
    ...
    print("  pocket_depth : %.1f mm  pocket_width: %.1f mm" % (
