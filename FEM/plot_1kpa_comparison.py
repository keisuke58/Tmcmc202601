...
def fig_bar_comparison():
    ...
    for i, cond in enumerate(CONDITIONS):
        ...
        ax.plot([0, 1], [u_1k, u_1m], "o-", color=COND_COLORS[cond],
                label=COND_LABELS[cond], markersize=8, linewidth=2)
    ax.set_xticks([0, 1]); ax.set_xticklabels(load_labels, fontsize=10)
    ax.set_ylabel("$U_{max}$ [mm]", fontsize=11)
    ax.set_yscale("log")
    ...
    for cond in CONDITIONS:
        e = R_1KPA[cond]["E_pa"]
        u = R_1KPA[cond]["U_max"]
        ax.scatter(e, u, s=150, c=COND_COLORS[cond], edgecolor="k",
                   zorder=5, label=COND_LABELS[cond])
    # Fit line
