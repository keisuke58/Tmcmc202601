...
def plot_condition_comparison(results, outdir):
    ...
    bars = ax.bar(range(n), di_means, color=[colors[i] for i in range(n)],
    ...
    ax.bar(range(n), svm_max, color=[colors[i] for i in range(n)],
           alpha=0.8, edgecolor="k", linewidth=0.5)
    ax.set_xticks(range(n))
    ax.set_xticklabels([c.replace("_", "\n") for c in conds], fontsize=8)
    ...
    ax.bar(range(n), e_mins, color=[colors[i] for i in range(n)],
           alpha=0.8, edgecolor="k", linewidth=0.5)
    ax.set_xticks(range(n))
    ax.set_xticklabels([c.replace("_", "\n") for c in conds], fontsize=8)
    ...
    fig.suptitle("Cross-Condition 2D Stress Comparison",
...
def main():
    ...
    for cond in args.conditions:
        ...
        try:
            result = run_2d_stress_pipeline(
                theta, Nx=nx, Ny=ny,
                n_macro=n_macro, dt_h=1e-5, n_react_sub=20,
                save_every=n_macro,
    ...
    for cond, s in summaries.items():
        print(f"{cond:<25} {s['DI_mean']:8.4f} {s['sigma_vm_max_pa']:10.3f} "
