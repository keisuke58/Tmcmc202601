...
def get_pdl_layer(vertices_p, faces_p, vertices_s, faces_s, param):
    """
    ...
    intt_face_idxs = cargen.get_initial_surface(
        vertices_p, faces_p, vertices_s, faces_s, param.gap_distance
    )

    # trim the cartilage boundary layer
    int_face_idxs = cargen.trim_boundary(
        faces_p, intt_face_idxs, face_adjacency, cumulative_sum, param.trimming_iteration
    )

    # removing extra components
    int_face_idxs = cargen.get_largest_component(faces_p, int_face_idxs)

    # remove ears
    for i in range(3):
        int_face_idxs = cargen.remove_ears(faces_p, int_face_idxs)

    base_face_idxs = np.copy(int_face_idxs)

    # viz
    frame = mp.plot(
        vertices_p, faces_p, c=cargen.mandible, shading=cargen.sh_false, return_plot=True
    )
    print(frame)
    ...
    # neighbour info
    base_b_vertex_idxs, boundary_face_idxs, neigh_face_list = cargen.neighbouring_info(
        vertices_p, faces_p[base_face_idxs]
    )
    ...
    print(
        "Base layer: max dihedral angle before smoothing is ",
        max_angle,
        "radians (",
        np.round(math.degrees(max_angle), 2),
        "degrees).",
    )
    ...
    for ss in range(param.smoothing_iteration_base):
        ...
        smoothed_max_angles = cargen.get_dihedral_angle(
            vertices_p, faces_p, base_face_idxs, neigh_face_list
        )
    ...
    print(
        "Base layer: max dihedral angle after smoothing is ",
        smoothed_max_angle,
        "radians (",
        np.round(math.degrees(smoothed_max_angle), 2),
        "degrees).",
    )
    ...
    if len(folded_vertex_idxs) != 0:
        ...
        print(
            "- Solution 1: To prevent this issue, decrease your 'param.smoothing_iteration' to ",
            iteration[0],
        )
        print(
        ...
        frame = mp.plot(
            ...
            faces_p[base_face_idxs],
            c=cargen.blue,
            shading=cargen.sh_false,
            return_plot=True,
        )
        frame.add_points(
            vertices_p[base_b_vertex_idxs[folded_vertex_idxs]],
        ...
        for i in folded_vertex_idxs:
            frame.add_mesh(
                vertices_p,
                faces[np.array(neigh_face_list[i])],
                c=cargen.sweet_pink,
                shading=cargen.sh_true,
            )
            frame.add_mesh(
                vertices_p,
                faces[np.array(boundary_face_idxs[i])],
                c=cargen.pastel_yellow,
                shading=cargen.sh_true,
            )

        if param.fix_boundary:
            base_face_idxs = cargen.fix_boundary(
                vertices_p, faces_p, base_face_idxs, base_b_vertex_idxs, folded_vertex_idxs
            )
    ...
    # cartilage area
    cartilage_area = cargen.get_area(vertices_p, faces_p[base_face_idxs])
    " Part B. Extrusion "
    thickness_profile = cargen.assign_thickness(
        vertices_p, faces_p, vertices_bs, faces_s, base_face_idxs, param.thickness_factor
    )
    ...
    # extrude surface
    ex_vertices_p = cargen.extrude_cartilage(vertices_p, faces_p, base_face_idxs, weights)
    ...
    # neighbour info
    ex_base_b_vertex_idxs, boundary_face_idxs, neigh_face_list = cargen.neighbouring_info(
        ex_vertices_p, faces_p[ex_base_face_idxs]
    )
    # measure dihedral angle
    max_angles = cargen.get_dihedral_angle(
        ex_vertices_p, faces_p, ex_base_face_idxs, neigh_face_list
    )
    max_angle = np.max(max_angles)
    max_angle = np.round(max_angle, 2)

    print(
        "Extruded layer: max dihedral angle before smoothing is ",
        max_angle,
        "radians (",
        np.round(math.degrees(max_angle), 2),
        "degrees).",
    ...
    if param.smoothing_iteration_extruded_base != 0:
        ...
        for ss in range(param.smoothing_iteration_extruded_base):
            ex_vertices_p = cargen.smooth_boundary(
                ex_vertices_p, ex_base_b_vertex_idxs, param.smoothing_factor
            )
            ex_vertices_p = cargen.snap_to_surface(ex_vertices_p, vertices_bs, faces_s)
            smoothed_max_angles = cargen.get_dihedral_angle(
                ex_vertices_p, faces_p, ex_base_face_idxs, neigh_face_list
        ...
        print(
            "Extruded layer: max dihedral angle after smoothing is",
            smoothed_max_angle,
            "radians (",
            np.round(math.degrees(smoothed_max_angle), 2),
            "degrees).",
    ...
    if len(folded_vertex_idxs) != 0:
        ...
        frame = mp.plot(
            ex_vertices_p,
            faces_p[ex_base_face_idxs],
            c=cargen.pastel_blue,
            shading=cargen.sh_false,
            return_plot=True,
        )
        frame.add_points(
            ex_vertices_p[ex_base_b_vertex_idxs[folded_vertex_idxs]],
            shading={"point_size": 0.2, "point_color": "red"},
        )
        ...
        if param.fix_boundary:
            ex_base_face_idxs = cargen.fix_boundary(
                ex_vertices_p, faces_p, ex_base_face_idxs, ex_base_b_vertex_idxs, folded_vertex_idxs
            )
            ex_base_b_vertex_idxs = igl.boundary_loop(faces_p[ex_base_face_idxs])
            print("normal visualization of the fixed result:")
            centroids, end_points = cargen.norm_visualization(
                ex_vertices_p, faces_p[ex_base_face_idxs]
            )
            frame = mp.plot(
                ex_vertices_p,
    ...
    frame.add_mesh(vertices_p, faces_p[base_face_idxs], c=cargen.blue, shading=cargen.sh_true)
    frame.add_mesh(
        ex_vertices_p, faces_p[ex_base_face_idxs], c=cargen.green, shading=cargen.sh_true
    )
    ...
    # build wall
    roof_faces = cargen.get_wall(vertices_p, base_b_vertex_idxs, ex_base_b_vertex_idxs)

    # merge left, top, and right faces
    si_vertices = np.concatenate((ex_vertices_p, vertices_p))
    si_faces = np.concatenate(
        (faces_p[ex_base_face_idxs], roof_faces, base_faces + len(ex_vertices_p))
    )
    ...
    # final touch
    usi_vertices, usi_faces = cargen.clean(usi_vertices, usi_faces)

    roof_vertices, roof_faces = cargen.clean(roof_vertices, roof_faces)
    ...
    frame.add_points(
        vertices_p[base_b_vertex_idxs], shading={"point_size": 0.3, "point_color": "red"}
    )
    frame.add_points(
        ex_vertices_p[ex_base_b_vertex_idxs], shading={"point_size": 0.3, "point_color": "red"}
    )
