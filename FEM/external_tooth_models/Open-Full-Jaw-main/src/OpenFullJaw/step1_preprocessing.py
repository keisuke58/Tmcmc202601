...
def preprocess_bone(path_bone, params):
    ...
    b_remeshing_info = {
        "epsilon": params.preprocess_bone_eps,
...
def smooth_bone(v_b, f_b, params):
    ...
    then = time.time()
    vs, cs = laplace_beltrami_smoothing1(
        v_b, f_b, params.preprocess_smooth_bone_iter, params.preprocess_smooth_bone_stepsize
    )
    now = time.time()
    ...
    frame = mp.plot(vs[-1], shading={"point_size": 1.0, "point_color": "gray"})
    frame.add_points(
        vs[-1][dist > max(dist) / 4], shading={"point_size": 2.0, "point_color": "red"}
    )
    ...
    b_smoothing_info = {
        "smoothing_iteration": params.preprocess_smooth_bone_iter,
...
def remesh_teeth(path_all_teeth, params):
    ...
    teeth_remeshing_info = {
        "epsilon": params.preprocess_teeth_eps,
