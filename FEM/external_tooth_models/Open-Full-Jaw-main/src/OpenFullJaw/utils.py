...
from scipy.sparse.linalg import (
    spsolve,
)  # for solving the smoothing matrix(in the laplace_beltrami_smoothing function)
...
def keep_larget_component(v, f):
    ...
    largest_comp = pymesh.submesh(
        input_mesh, orig_f_idx, num_rings=0
    )  # this is to adjust the faces matrix based on the new vertices ids
...
def laplace_beltrami_smoothing1(v, f, num_iter=30, step_size=0.0008):
    ...
    for i in range(num_iter):
        ...
        if np.isnan(np.sum(v)):
            break
            print(
                "----------- Error: there was an issue during the smoothing process of this mesh."
            )
        vs.append(v)
        cs.append(c)
...
def split_teeth_to_separate_meshes(V_ts, F_ts, centers, unn, params):
    ...
    for i in meshes:
        ...
        igl.write_triangle_mesh(
            params.output_dir + "/tooth_{}_temp.obj".format(unn[idx]), i.vertices, i.faces
        )
...
def generate_json_file3(path_json, path_t, path_p, path_b, operation):
    ...
    dictionary = {
        "operation": operation,
        "left": path_t,
        "right": {"operation": operation, "left": path_p, "right": path_b},
    }
...
def generate_vol_mesh(params, path_input):
    ...
    path_output = (
        params.output_dir
        + params.output_fname
        + "_with_eps_{}_l_{}".format(params.preprocess_bone_eps, params.preprocess_bone_l)
    )
    ...
    os.system(
        ...
        + path_input
        + " --level "
        + str(params.verbose_level)
        + " --max-its "
        + str(params.max_iter)
        + " -e "
        + str(params.preprocess_bone_eps)
        + " -l "
        + str(params.preprocess_bone_l)
        + " -o "
        + path_output
        + " --no-binary --no-color"
    )
...
def generate_json_file2(path_json, path_1, path_2, operation):
    ...
    dictionary = {"operation": operation, "left": path_1, "right": path_2}
...
def convert_str_to_numpy(string):
    ...
    s = list(
        map(float, string.split(","))
    )  # split the string based on the "," and convert each of them to float
...
def add_meterial(material_, mat_id, name_, type_, E_, v_, ro_):
    ...
    mat = ET.SubElement(
        material_,
        "material",
        attrib={"id": "{}".format(mat_id), "name": "{}".format(name_), "type": "{}".format(type_)},
    )
    mat.tail = "\n"
...
class simulation_parameters:
    ...
    def users_materials(E_t, v_t, ro_t, type_t, E_p, v_p, ro_p, type_p, E_b, v_b, ro_b, type_b):
...
class Params:
    ...
    def __init__(self):
        ...
        self.verbose_level = 3
        self.max_iter = 20
        self.volume_mesh_json_path = (
            os.getcwd() + "/" + "mid-output" + "/" + "logical_operation.json"
        )
