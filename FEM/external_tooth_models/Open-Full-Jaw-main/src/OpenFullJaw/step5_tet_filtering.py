...
def label_raw_mesh(V, T, path_tooth, path_pdl, path_bone, params):
    ...
    t1 = np.logical_and(
        D_t > 0, D_t < params.tet_filtering_dist
    )  # hollow of tets around the tooth with positive sdf values
    t2 = np.logical_and(
        D_b > 0, D_b < params.tet_filtering_dist
    )  # hollow of tets around the bone with positive sdf values
    ...
    frame = mp.plot(
        V + np.array([0, 0, 5]), T[np.where(t3)], c=pdl_c, shading=sh_true, return_plot=True
    )
    frame = mp.plot(
        V + np.array([0, 0, 10]),
        T[np.where(D_t <= 0)],
