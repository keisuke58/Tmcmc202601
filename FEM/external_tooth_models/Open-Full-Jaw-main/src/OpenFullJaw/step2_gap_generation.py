...
def shrink_bone(v_b_new, f_b, params):
    ...
    print(
        "---- will assume its offset limit is {} to avoid paper thin walls in the bone mesh. ",
        offset_bone,
    )
    ...
    if offset_bone < space * bone_ratio:
        comment = "bone offset limit({}) is less than desired gap ({}).".format(
            offset_bone, space * bone_ratio
        )
        offset = offset_bone
    ...
    s1_normals = igl.per_vertex_normals(
        v_b_new, f_b, 2
    )  # 2 corresponds to igl.PER_VERTEX_NORMALS_WEIGHTING_TYPE_ANGLE angle weighted
    ...
    shrinking_info = {
        "desired_gap": params.gap_thickness,
...
def shrink_teeth(teeth_dir, rest, shrinking_info, params):
    ...
    for file in sorted(glob.glob("*.stl")):
        ...
        igl.write_triangle_mesh(
            params.output_dir + "tooth_{}_shrinked.obj".format(tnum), v_prime, f_prime
        )
    ...
    frame = mp.plot(
        all_v, all_f, c=tooth_c, shading=sh_false, return_plot=True
    )  # The mesh with all teeth
    frame = mp.plot(
        teeth_v, teeth_f, c=tooth_c, shading=sh_false, return_plot=True
    )  # The mesh with all eroded teeth
