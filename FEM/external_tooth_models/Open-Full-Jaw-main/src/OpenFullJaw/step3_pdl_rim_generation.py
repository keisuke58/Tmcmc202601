...
def generate_pdls_rims(cargen_bone_input, mid_o_teeth_dir, unn, impacted, params):
    ...
    for i in unn:
        ...
        s2_vertices, s2_faces = cargen.read_and_clean(
            params.input_dir + params.input_fname.format(i), io_dim
        )

        margin = 0.5
        bl, ur = loose_bb(s2_vertices, margin)

        v_idx_orig = np.where(np.all(np.logical_and(bl <= s1_vertices, s1_vertices <= ur), axis=1))[
            0
        ]
        f_idx_orig = []
        ...
        s1_v = s1_vertices[
            v_idx_orig
        ]  # original vertex indices is idx and the corresponding new one is np.arrange(0, len(s1_vertices))
        ...
        # run
        _, _, top_vertices, top_faces = cargen.get_pdl_layer(
            s1_v, s1_f, s2_vertices, s2_faces, param_cargen
        )
