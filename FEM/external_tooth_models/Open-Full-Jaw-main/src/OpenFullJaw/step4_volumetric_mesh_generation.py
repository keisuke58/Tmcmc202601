...
def generate_raw_mesh(path_1, path_2, path_3, params):
    ...
    path_output = (
        params.output_dir
        + params.output_fname
        + "_with_eps_{}_l_{}".format(params.volume_mesh_eps, params.volume_mesh_l)
    )
    ...
    os.system(
        params.ftet_path
        + " --csg "
        + params.volume_mesh_json_path
        + " --level "
        + str(params.verbose_level)
        + " --max-its "
        + str(params.max_iter)
        + " -e "
        + str(params.volume_mesh_eps)
        + " -l "
        + str(params.volume_mesh_l)
        + " -o "
        + path_output
        + " --no-binary --no-color"
        + " --export-raw"
    ...
    gluing_info = {
        "epsilon": params.volume_mesh_eps,
