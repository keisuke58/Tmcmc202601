...
for setname in list(inst.elementSets.keys()):
    sn = setname.upper()
    if   sn.startswith('T23_BIN_'): tooth = 'T23'
    elif sn.startswith('T30_BIN_'): tooth = 'T30'
    elif sn.startswith('T31_BIN_'): tooth = 'T31'
    else: continue
    try:
...
_sorted_sets = sorted(
    ...
    key=lambda s: _set_priority.get(
        'APPROX' if 'APPROX' in s.upper()
        else 'INNER' if 'INNER' in s.upper()
        else 'OUTER' if 'OUTER' in s.upper()
        else 'ZZZ', 99))
...
frame = step.frames[-1]
print("[6/7] Extracting field outputs from step '%s', frame %d (time=%.4f = full load) ..." % (
...
elem_mises = {lbl: elem_mises_sum[lbl] / elem_mises_cnt[lbl]
              for lbl in elem_mises_sum}
print("       %d element MISES values (avg over %d integration pts each)" % (
...
with open(nodes_csv, 'w') as f:
    ...
    for lbl, (x, y, z) in zip(node_labels, node_xyz):
        ...
        f.write("%d,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%.8g,%s,%s\n" % (
