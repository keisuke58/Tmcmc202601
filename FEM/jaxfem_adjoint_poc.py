...
def di_to_eeff_jax(di, e_max=E_MAX, e_min=E_MIN,
...
def build_forward_0d(K_hill=0.05, n_hill=4.0, n_steps=500, dt=0.01):
    ...
    def forward(theta):
        ...
        def step(phi, _):
            ...
            phi_new = jnp.where(phi_sum > 0.999,
                                phi_new * 0.999 / phi_sum, phi_new)
...
def run_inverse_poc(args):
    ...
    forward_fn, _ = build_forward_0d(
        K_hill=args.k_hill, n_hill=args.n_hill, n_steps=n_steps)
    ...
    if theta_target is None:
        ...
        print(f"  Using synthetic target theta")
    else:
    ...
    for idx, name in enumerate(param_names):
        ...
        print(f"    {name:5s}: target={t_val:7.3f}  recovered={r_val:7.3f}  "
