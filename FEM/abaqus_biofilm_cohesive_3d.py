...
def _parse_args(argv):
    ...
    while i < len(argv):
        ...
        if a == "--t-max"        and i+1 < len(argv): cfg["t_max_0"]     = float(argv[i+1]); i+=2; continue
        if a == "--gc-max"       and i+1 < len(argv): cfg["gc_max"]      = float(argv[i+1]); i+=2; continue
        ...
        if a == "--n-layers-cz"  and i+1 < len(argv): cfg["n_layers_cz"] = int(argv[i+1]);   i+=2; continue
        if a == "--u-max"        and i+1 < len(argv): cfg["u_max"]       = float(argv[i+1]); i+=2; continue
        if a == "--n-steps"      and i+1 < len(argv): cfg["n_steps"]     = int(argv[i+1]);   i+=2; continue
        if a == "--n-bins"       and i+1 < len(argv): cfg["n_bins"]      = int(argv[i+1]);   i+=2; continue
        if a == "--mesh-size"    and i+1 < len(argv): cfg["mesh_size"]   = float(argv[i+1]); i+=2; continue
        i += 1
...
def _build_and_run(cfg):
    ...
    from abaqusConstants import (
        ...
        FROM_SECTION, MIDDLE_SURFACE,
        KINEMATIC, ELASTIC, DISPLACEMENT,
        UNIFORM, DEFAULT, SINGLE,
        PERCENTAGE, ABSOLUTE,
        QUADS_AND_TRIS, FREE,
        HARD, FRICTIONLESS, AUGMENTED_LAGRANGE,
        XPLICIT,
    )
    try:
        from abaqusConstants import (
    ...
    print("Field: %d nodes  di=[%.4f, %.4f]" % (
        len(coords), min(di_vals), max(di_vals)))
    ...
    print("  t_max_eff=%.4g Pa  Gc_eff=%.4g J/m^2  Knn=%.4g Pa/m" % (
        t_max_eff, gc_eff, knn))
    ...
    p_sub = model.Part(name="Substrate", dimensionality=THREE_D,
    ...
    p_bio = model.Part(name="Biofilm", dimensionality=THREE_D,
                       type=DEFORMABLE_BODY)
    ...
    sec_sub = model.HomogeneousSolidSection(name="SEC_SUB",
                                            material="MAT_SUB", thickness=None)
    ...
    for b in range(n_bins):
        di_b  = (b + 0.5) * bin_w
        e_b   = _di_to_E(di_b, cfg["e_max"], cfg["e_min"],
                         cfg["di_scale"], cfg["n_exp_bio"])
    ...
    for cell in all_cells:
        ...
        best_di = min(di_vals,
                      key=lambda dv, xv=cx, yv=cy, zv=cz:
                      (coords[di_vals.index(dv)][0] - xv) ** 2 +
    ...
    def _find_face(inst, z_target, tol_v):
        faces_ok = []
        for f in inst.faces:
            pts = [inst.nodes[nd - 1].coordinates
                   if hasattr(inst, "nodes") else (0, 0, z_target)
    ...
    print("  [out] %s" % out_csv)
    print("  di_mean=%.4f  t_max=%.4gPa  Gc=%.4gJ/m2  RF_peak=%.4g" % (
        di_mean, t_max_eff, gc_eff,
        rf_peak if rf_peak is not None else float("nan")))
