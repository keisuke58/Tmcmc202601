...
from material_models import (
    ...
    compute_di as compute_di_scalar,
    E_MAX_PA, E_MIN_PA, DI_SCALE, DI_EXPONENT,
    PHI_PG_CRIT, HILL_M, W_PG, W_FN, V_CRIT,
    IDX_PG, IDX_FN,
)
...
def plot_3model_comparison(summary: list[dict]) -> str:
    ...
    ax_curve.axvline(PHI_PG_CRIT, color="red", ls="--", lw=1, alpha=0.7,
                     label=f"$\\varphi_{{crit}}={PHI_PG_CRIT}$")
    for s in summary:
        ax_curve.plot(s["phi_Pg_0d"], s["E_phi_pg"], "o", color=s["color"],
                      ms=10, zorder=5, markeredgecolor="black")
        ax_curve.annotate(s["label"].split()[0],
    ...
    w = 0.25
    bars_di  = ax_E.bar(x - w, E_di,  w, label="DI model",
                        color=[c for c in colors], alpha=0.4, edgecolor="black", hatch="///")
    bars_pg  = ax_E.bar(x,     E_pg,  w, label=r"$\varphi_{Pg}$ model",
                        color=[c for c in colors], alpha=0.85, edgecolor="black")
    bars_vir = ax_E.bar(x + w, E_vir, w, label="Virulence model",
                        color=[c for c in colors], alpha=0.6, edgecolor="black", hatch="\\\\\\")
    ...
    for bars, vals in [(bars_di, E_di), (bars_pg, E_pg), (bars_vir, E_vir)]:
        for bar, val in zip(bars, vals):
            ax_E.text(bar.get_x() + bar.get_width()/2, val + 15,
    ...
    ax_sigma.bar(x - w, sigma_di,  w, label="DI",
                 color=[c for c in colors], alpha=0.4, edgecolor="black", hatch="///")
    ax_sigma.bar(x,     sigma_pg,  w, label=r"$\varphi_{Pg}$",
                 color=[c for c in colors], alpha=0.85, edgecolor="black")
    ax_sigma.bar(x + w, sigma_vir, w, label="Virulence",
                 color=[c for c in colors], alpha=0.6, edgecolor="black", hatch="\\\\\\")
    ...
    ax_sigma.set_title(r"(c) Compressive prestress $\sigma_0 = -E \cdot \varepsilon_{growth}$ (tooth end)")
    ...
    ax_phi.bar(x - w/2, phi_pg_vals, w, label=r"$\varphi_{Pg}$ (P. gingivalis)",
               color="#d62728", alpha=0.85, edgecolor="black")
    ax_phi.bar(x + w/2, phi_fn_vals, w, label=r"$\varphi_{Fn}$ (F. nucleatum)",
               color="#9467bd", alpha=0.85, edgecolor="black")
    ax_phi.axhline(PHI_PG_CRIT, color="red", ls="--", lw=1, alpha=0.7,
    ...
    for i, (pg, fn) in enumerate(zip(phi_pg_vals, phi_fn_vals)):
        ax_phi.text(x[i] - w/2, pg + 0.01, f"{pg:.3f}",
                    ha="center", va="bottom", fontsize=7)
        ax_phi.text(x[i] + w/2, fn + 0.01, f"{fn:.3f}",
                    ha="center", va="bottom", fontsize=7)
...
def main():
    ...
    print("=" * 70)
    print(f"  Models: DI (s={DI_SCALE:.4f}), "
    ...
    for ckey, meta in CONDITIONS.items():
        ...
        print(f"  φ: So={phi[0]:.4f} An={phi[1]:.4f} Vd={phi[2]:.4f} "
              f"Fn={phi[3]:.4f} Pg={phi[4]:.4f}  ({elapsed:.1f}s)")

        # 3. 既存 1D CSV ロード
        orig = load_original_csv(ckey)
        if orig is None:
            print(f"  スキップ: 元 CSV なし。先に multiscale_coupling_1d.py を実行。")
            continue
    ...
    print(f"{'Condition':<25} {'DI_0D':>7} {'φ_Pg':>7} {'V':>7} "
          f"{'E_di':>7} {'E_φPg':>7} {'E_vir':>7}")
    print("-" * 78)
    for s in summary:
        print(f"  {s['condition_key']:<23} "
              f"{s['di_0d']:>7.4f} "
              f"{s['phi_Pg_0d']:>7.4f} "
              f"{s['V_0d']:>7.4f} "
              f"{s['E_di']:>7.1f} "
    ...
    if len(summary) >= 3:
        ...
        print(f"    DI model:        {comm_avg_di:.0f} / {dysb_avg_di:.0f} = {comm_avg_di/max(dysb_avg_di,1e-6):.1f}x")
        print(f"    φ_Pg model:      {comm_avg_pg:.0f} / {dysb_avg_pg:.0f} = {comm_avg_pg/max(dysb_avg_pg,1e-6):.1f}x")
        print(f"    Virulence model: {comm_avg_vir:.0f} / {dysb_avg_vir:.0f} = {comm_avg_vir/max(dysb_avg_vir,1e-6):.1f}x")
