...
def fig2_ci_bars(data):
    ...
    for si, (col, slabel) in enumerate([(0, "Substrate"), (1, "Surface")]):
        ...
        bars = ax.bar(x, p50, width=w, color=colors, alpha=0.8,
        ...
        for xi, v in enumerate(p50):
            ax.text(xi, v + (p95[xi] - p50[xi]) + 0.01,
...
def fig5_summary(data, spear):
    ...
    for pos, vals_list, marker in [(pos_s, vals_sub, "s"), (pos_su, vals_surf, "^")]:
        bp = ax_box.boxplot(vals_list, positions=pos, widths=0.55,
                            patch_artist=True,
                            medianprops=dict(color="black", lw=2),
    ...
    wd = 0.35
    ax_delta.bar(xd - wd/2, delta_sub,  wd, label="Substrate",
                 color=[COLORS[c] for c in CONDITIONS], alpha=0.8, edgecolor="k", lw=0.5)
    ax_delta.bar(xd + wd/2, delta_surf, wd, label="Surface",
                 color=[COLORS[c] for c in CONDITIONS], alpha=0.4, edgecolor="k", lw=0.5,
                 hatch="//")
    ax_delta.axhline(0, color="black", lw=0.8, ls="--")
    ax_delta.set_xticks(xd)
    ax_delta.set_xticklabels([LABELS[c].replace("\n"," ") for c in CONDITIONS],
                              fontsize=8, rotation=12, ha="right")
...
def main():
    ...
    for cond in CONDITIONS:
        arr = data[cond][:, 0] / 1e6
        print(f"  {cond:20s}  p05={np.percentile(arr, 5):.3f}"
