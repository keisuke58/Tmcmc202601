...
def fig1_pipeline():
    ...
    def box(x, y, w, h, label, sublabel="", color="#FFFFFF", textcolor="#1A2A4A", fontsize=13, radius=0.4):
        ...
        if sublabel:
            ax.text(x, y - 0.32, sublabel, ha="center", va="center",
                    fontsize=9, color=textcolor + "BB" if len(textcolor) > 6 else "#555577",
                    zorder=4, style="italic")

    def arrow(x1, y1, x2, y2, label="", color="#334155"):
        ax.annotate("", xy=(x2, y2), xytext=(x1, y1),
                    arrowprops=dict(arrowstyle="-|>", color=color, lw=2.5, mutation_scale=20), zorder=2)
        if label:
            mx, my = (x1 + x2) / 2, (y1 + y2) / 2
            ax.text(mx + 0.1, my, label, fontsize=8, color=color, va="center")

    ax.text(10, 10.5, "Oral Biofilm Parameter Estimation — Overall Pipeline",
    ...
    box(15.5, 4.0, 3.6, 2.4, "⑥ Output", "MAP estimate\n95% CI\nConvergence (Rhat, ESS)", "#2563EB", fontsize=12)
    box(11.0, 5.0, 3.2, 1.6, "Speedup", "Numba JIT\nTSM-ROM\nParallel (4–8×)", "#64748B", TEXT, fontsize=11)
    ...
    ax.annotate("", xy=(9.4, 6.2), xytext=(13.7, 6.2),
                arrowprops=dict(arrowstyle="-|>", color="#E05252", lw=2,
                                connectionstyle="arc3,rad=-0.4", mutation_scale=16), zorder=2)
    ax.text(11.5, 5.35, "iterate", fontsize=9, color="#E05252", ha="center")

    for i, (txt, c) in enumerate([
        ("● Input", COLORS["So"]), ("● Prob. model", COLORS["An"]),
        ("● ODE model", COLORS["Vd"]), ("● Algorithm", COLORS["Pg"]), ("● Output", COLORS["accent"])
    ]):
        ax.text(0.5 + i*3.9, 0.5, txt, fontsize=9, color=c, fontweight="bold")
    ax.text(10, 0.12, "Dysbiotic × HOBIC | 20 parameters | 8 stages × 150 particles",
...
def fig2_network():
    ...
    ax.set_ylim(-1.4, 1.6)
    ax.axis("off")
    ax.set_title("Inter-species interaction network", color=TEXT, fontsize=15, fontweight="bold", pad=12)
    ...
    edges = [
        (0, 1, 0.8, "+"), (1, 0, 0.5, "+"), (2, 3, 1.2, "+"), (3, 2, 0.6, "+"),
        (3, 4, 2.4, "+"), (2, 4, 3.56, "+"), (0, 2, 0.7, "w"), (1, 3, 0.4, "w"),
    ]
    ...
    for i, (name, full_name, color) in enumerate(zip(names, full, SCOLORS)):
        ...
        ax.text(x, y, name, ha="center", va="center", fontsize=12, fontweight="bold", color=BG, zorder=4)
        lx, ly = x * 1.55, y * 1.45 + (0.15 if i == 4 else 0)
        ax.text(lx, ly, full_name, ha="center", va="center", fontsize=8, color=color, zorder=4)

    ax.annotate("Hill gate\n(Fn-dependent)", xy=pos[3], xytext=(0.7, -0.5),
                arrowprops=dict(arrowstyle="->", color="#B8860B", lw=1.5),
                fontsize=8, color="#8B6914", zorder=5)
    for txt, c in [("strong facilitation", "#4EC9B0"), ("weak interaction", "#888888")]:
        ax.plot([], [], color=c, lw=2, label=txt)
    ax.legend(loc="upper right", facecolor=LEGEND_BG, edgecolor=LEGEND_EC, labelcolor=TEXT, fontsize=8)

    ax2 = axes[1]
    ax2.set_facecolor(BG)
    ax2.set_title("Interaction matrix A[i→j] (MAP estimate)", color=TEXT, fontsize=15, fontweight="bold", pad=12)
    A = np.array([
    ...
    cmap = LinearSegmentedColormap.from_list("bio", [BG, "#E8F4FC", "#2563EB", "#4EC9B0", "#DAA520"])
    ...
    for i in range(5):
        for j in range(5):
            if locked[i, j]:
                ...
                ax2.text(j, i, f"{v:.2f}", ha="center", va="center", fontsize=13, fontweight="bold", color=tc)
    cbar = fig.colorbar(im, ax=ax2, fraction=0.046, pad=0.04)
    cbar.set_label("Interaction strength", color=TEXT, fontsize=10)
    cbar.ax.yaxis.set_tick_params(color=AXES)
    plt.setp(cbar.ax.yaxis.get_ticklabels(), color=TEXT)
    for i in range(5):
        ax2.add_patch(plt.Rectangle((i - 0.5, i - 0.5), 1, 1, fill=False, edgecolor="#B8860B", lw=2.5))
    ax2.text(2, 5.3, "0 = fixed by biology  |  diagonal = self-regulation", ha="center", fontsize=9, color=TEXT_SEC)
...
def fig3_tmcmc():
    ...
    ax1.set_title("TMCMC — β schedule and particle evolution", color=TEXT, fontsize=13, fontweight="bold")
    ...
    for s_idx, beta in enumerate(beta_stages):
        ax1.text(s_idx * 0.5 + true_center[0], true_center[1] + 1.6, f"β={beta:.2f}",
                 ha="center", fontsize=7.5, color=plt.cm.plasma(beta), rotation=30)
    ...
    for x, y, txt, c in steps:
        ...
        ax2.add_patch(bbox)
        ax2.text(x, y, txt, ha="center", va="center", fontsize=8.5, color=c, fontweight="bold", zorder=4)
    ...
    ax3.legend(lines1 + lines2, labels1 + labels2, facecolor=LEGEND_BG,
               edgecolor=LEGEND_EC, labelcolor=TEXT, fontsize=8)
    ...
    fig.text(0.5, 0.98, "TMCMC algorithm — detail", ha="center", color=TEXT, fontsize=16, fontweight="bold")
...
def fig4_data_model():
    ...
    for label, color, cy, desc in blocks:
        ...
        ax3.add_patch(bbox)
        ax3.text(2.25, cy, label, ha="center", va="center", fontsize=10, fontweight="bold", color=color, zorder=4)
    ...
    locked_box = FancyBboxPatch((5.5, 8.3), 4.0, 1.2, boxstyle="round,pad=0.08",
                                 facecolor="#FFF0F0", edgecolor="#C44", lw=1.5)
    ax3.add_patch(locked_box)
    ax3.text(7.5, 8.9, "Locked parameters", ha="center", fontsize=9, color="#C44", fontweight="bold")
    ax3.text(7.5, 8.45, "Fixed to 0 by biology", ha="center", fontsize=8, color="#884444")
    hill_box = FancyBboxPatch((5.5, 6.8), 4.0, 1.2, boxstyle="round,pad=0.08",
                               facecolor="#FFFDE7", edgecolor="#B8860B", lw=1.5)
    ax3.add_patch(hill_box)
    ax3.text(7.5, 7.4, "Hill gate (fixed)", ha="center", fontsize=9, color="#8B6914", fontweight="bold")
    ax3.text(7.5, 6.95, "K=0.05, n=4 (Fn-dependent)", ha="center", fontsize=7.5, color="#666")

    fig.text(0.5, 0.98, "Data structure · ODE model · parameter space", ha="center", color=TEXT, fontsize=15, fontweight="bold")
    plt.tight_layout()
...
def fig5_positioning():
    ...
    for name, x, y, c, s in methods:
        ...
        ax1.text(x, y + 0.06, name, ha="center", va="center", fontsize=9,
                 color=c, fontweight="bold" if is_hl else "normal", zorder=5,
                 bbox=dict(boxstyle="round,pad=0.2", fc=BG, ec=c if is_hl else "none",
                           lw=1.5 if is_hl else 0, alpha=0.9) if is_hl else None)
    ...
    ax1.text(0.5, -0.08, "Limited data · high-dim · nonlinear → TMCMC fits well", ha="center", fontsize=9.5, color=TEXT_SEC)
    ...
    for i, (fname, desc) in enumerate(items):
        y = 8.5 - i * 1.6
        bbox = FancyBboxPatch((0.5, y - 0.5), 9.0, 1.0, boxstyle="round,pad=0.1",
                               facecolor=LEGEND_BG, edgecolor="#4EC9B0", lw=1, zorder=3)
        ax2.add_patch(bbox)
        ax2.text(1.0, y, fname, ha="left", va="center", fontsize=10, color="#2D8B6F", fontweight="bold", zorder=4)
        ax2.text(5.5, y, desc, ha="center", va="center", fontsize=9, color=TEXT_SEC, zorder=4)
    ax2.text(5, 0.5, "_runs/Dysbiotic_HOBIC_YYYYMMDD_XXXX/", ha="center", fontsize=9, color=TEXT_SEC)

    fig.text(0.5, 0.98, "Positioning & output structure", ha="center", color=TEXT, fontsize=15, fontweight="bold")
    plt.tight_layout()
...
def fig6_poster():
    ...
    fig.text(0.5, 0.965, "Quantifying Oral Biofilm Inter-Species Interactions via Bayesian Inference",
             ha="center", fontsize=22, fontweight="bold", color="white")
    fig.text(0.5, 0.945, "5-Species ODE Model · TMCMC · Dysbiotic × HOBIC · 20 Parameters · MAP + 95% CI",
             ha="center", fontsize=11, color="#8899AA")

    gs = GridSpec(3, 5, figure=fig, hspace=0.55, wspace=0.4, left=0.04, right=0.97, top=0.93, bottom=0.05)
    ...
    for i, (x, label, c, icon) in enumerate(pipeline_steps):
        box = FancyBboxPatch((x - 1.6, 0.6), 3.2, 2.8, boxstyle="round,pad=0.15",
        ...
        ax_pipe.text(x, 1.85, label, ha="center", va="center", fontsize=10, fontweight="bold", color=c, zorder=4)
        ax_pipe.text(x, 0.3, f"Step {i+1}", ha="center", fontsize=8, color=c, alpha=0.7)
        if i < len(pipeline_steps) - 1:
            ax_pipe.annotate("", xy=(x + 1.75, 2.0), xytext=(x + 1.6, 2.0),
                             arrowprops=dict(arrowstyle="-|>", color=AXES, lw=2.5, mutation_scale=16), zorder=2)
    ax_pipe.text(12, 3.75, "End-to-end inference flow", ha="center", fontsize=9, color=TEXT_SEC, style="italic")
    ...
    for i, j, w in net_edges:
        ...
        lw = 0.5 + w * 0.4
        ax_net.annotate("", xy=(xj * 0.82, yj * 0.82), xytext=(xi * 0.82, yi * 0.82),
                         arrowprops=dict(arrowstyle="-|>", color="#4EC9B0", lw=lw,
                                         connectionstyle="arc3,rad=0.2", mutation_scale=10, alpha=alpha))
    for i, (name, c) in enumerate(zip(["So", "An", "Vd", "Fn", "Pg"], SCOLORS)):
        x, y = pos_net[i]
        ax_net.add_patch(Circle((x, y), 0.25, color=c, zorder=3, ec="white", lw=1.5))
        ax_net.text(x, y, name, ha="center", va="center", fontsize=9, fontweight="bold", color="white", zorder=4)

    ax_rmse = fig.add_subplot(gs[1, 1:3])
    ax_rmse.set_facecolor("#080E1A")
    ax_rmse.set_title("RMSE Before → After (example run)", color="white", fontsize=11, fontweight="bold")
    ...
    from matplotlib.colors import LinearSegmentedColormap
    cmap2 = LinearSegmentedColormap.from_list("bio2", [BG, "#E8F4FC", "#2563EB", "#4EC9B0", "#DAA520"])
    ...
    for i in range(5):
        for j in range(5):
            v = A[i, j]
            tc = TEXT if v < 2.5 else BG
            ax_mat.text(j, i, f"{v:.1f}" if v > 0 else "0", ha="center", va="center", fontsize=9, color=tc, fontweight="bold")
    ...
    ax_conv.text(5, 2.0, "Uncertainty quantified: MAP + 95% CI · ESS · Rhat", ha="center", fontsize=10, color="#2D8B6F")
...
def fig7_combined_poster():
    ...
    # Title
    fig.text(0.5, 0.97, "From Bayesian Parameters to Tooth-Scale Mechanics",
             ha="center", fontsize=24, fontweight="bold", color="white")
    fig.text(0.5, 0.945, "TMCMC (5-species ODE) \u2192 DI field \u2192 3-tooth conformal biofilm FEM \u2192 stress & displacement",
             ha="center", fontsize=12, color="#8899AA")

    gs = GridSpec(4, 3, figure=fig, hspace=0.5, wspace=0.35, left=0.03, right=0.98, top=0.92, bottom=0.04)

    # ---- Row 0: Three-column flow ----
    ax_tmcmc = fig.add_subplot(gs[0, 0])
    ax_tmcmc.set_facecolor(BG)
    ax_tmcmc.set_title("TMCMC (Parameter estimation)", color="#2D8B6F", fontsize=13, fontweight="bold")
    ax_tmcmc.axis("off")
    ...
    for x, y, label, c in steps_t:
        b = FancyBboxPatch((x - 2.2, y - 0.6), 4.4, 1.2, boxstyle="round,pad=0.1",
                            facecolor=c + "22", edgecolor=c, lw=1.5, zorder=3)
        ax_tmcmc.add_patch(b)
        ax_tmcmc.text(x, y, label, ha="center", va="center", fontsize=9, color=c, fontweight="bold", zorder=4)
    for i in range(len(steps_t) - 1):
        ax_tmcmc.annotate("", xy=(5, steps_t[i + 1][1] + 0.6), xytext=(5, steps_t[i][1] - 0.6),
                          arrowprops=dict(arrowstyle="-|>", color=AXES, lw=1.5, mutation_scale=12))
    ax_tmcmc.text(5, 2.2, "Output: \u03b8_MAP per condition\n(4 conditions)", ha="center", fontsize=8, color=TEXT_SEC)
    ...
    for i, line in enumerate(cpl_items):
        y = 8.0 - i * 1.8
        ax_cpl.text(0.5, y, line, ha="left", va="center", fontsize=9, color=TEXT_SEC, family="monospace")
    ...
    for x, y, label, c in steps_f:
        b = FancyBboxPatch((x - 2.2, y - 0.5), 4.4, 1.0, boxstyle="round,pad=0.08",
                            facecolor=c + "22", edgecolor=c, lw=1.5, zorder=3)
        ax_fem.add_patch(b)
        ax_fem.text(x, y, label, ha="center", va="center", fontsize=8.5, color=c, fontweight="bold", zorder=4)
    for i in range(len(steps_f) - 1):
        ax_fem.annotate("", xy=(5, steps_f[i + 1][1] + 0.5), xytext=(5, steps_f[i][1] - 0.5),
                        arrowprops=dict(arrowstyle="-|>", color=AXES, lw=1.5, mutation_scale=12))
    ...
    ax_res.set_title("Key result (late-time, force control): displacement is the biomarker", color=TEXT, fontsize=12, fontweight="bold")
    ax_res.axis("off")
    ...
    for idx, (name, e_eff, u_med, color) in enumerate([
        ("DH-baseline (dysbiotic)\nE_eff = 0.50 MPa\n|U|_med \u2248 0.36 \u03bcm", 0.5, 0.358, "#E05252"),
        ("Commensal (e.g. static)\nE_eff \u2248 9.85 MPa\n|U|_med \u2248 0.018 \u03bcm", 9.85, 0.018, "#4EC9B0"),
    ]):
        x0 = 2 + idx * 12
        b = FancyBboxPatch((x0, 1.2), 10, 2.8, boxstyle="round,pad=0.15",
                            facecolor=color + "22", edgecolor=color, lw=2, zorder=3)
        ax_res.add_patch(b)
        ax_res.text(x0 + 5, 2.8, name, ha="center", va="center", fontsize=10, color=color, fontweight="bold")
    ax_res.text(14, 2.5, "\u2248 19.7\u00d7", ha="center", va="center", fontsize=28, fontweight="bold", color="#B8860B")
    ax_res.text(14, 1.4, "displacement ratio\n(soft vs stiff biofilm)", ha="center", fontsize=9, color=TEXT_SEC)
    ax_res.text(14, 0.5, "MISES identical under force BC; displacement differentiates conditions.", ha="center", fontsize=9, color=TEXT_SEC)
    ...
    for ri, row in enumerate(rows):
        ...
        for ci, cell in enumerate(row):
            ...
            ax_tbl.text(col_x[ci], y, cell, ha="left" if ci == 0 else "center", va="center", fontsize=9, color=color, fontweight=weight)
        if ri == 0:
    ...
    ax_bt.text(14, 0.3, "Report: biofilm_3tooth_report.pdf  |  Overview: overview_tmcmc_fem_en.md", ha="center", fontsize=8, color=TEXT_SEC)
...
def fig8_tmcmc_fem_flow():
    ...
    steps = [
        ...
        (11.2, "E_eff(DI)", "E_max(1\u2212r)^\u03b1 + E_min\u00b7r\nr = clip(DI/s_DI, 0, 1)", "#F4A142"),
    ...
    for i, (x, title, sub, c) in enumerate(steps):
        w, h = 1.8, 2.2
        box = FancyBboxPatch((x - w/2, 2.2), w, h, boxstyle="round,pad=0.08",
                              facecolor=c + "22", edgecolor=c, lw=2, zorder=3)
        ax.add_patch(box)
        ax.text(x, 3.7, title, ha="center", va="center", fontsize=11, fontweight="bold", color=c, zorder=4)
        ax.text(x, 3.0, sub, ha="center", va="center", fontsize=8, color=TEXT_SEC, zorder=4)
        if i < len(steps) - 1:
            ax.annotate("", xy=(x + w/2 + 0.25, 3.3), xytext=(x + w/2, 3.3),
                        arrowprops=dict(arrowstyle="-|>", color=AXES, lw=2, mutation_scale=18))
    ax.text(12, 5.3, "TMCMC \u2192 FEM coupling (tmcmc_to_fem_coupling.py)", ha="center", fontsize=14, fontweight="bold", color=TEXT)
    ax.text(12, 0.8, "Same geometry & load; only DI\u2192E_eff varies by condition \u2192 different displacement (force BC) or MISES (disp BC)", ha="center", fontsize=9, color=TEXT_SEC)
    plt.tight_layout()
...
def fig9_coupling_DI_Eeff():
    ...
    ax1.set_xlim(0, 10)
    ax1.set_ylim(0, 10)
    ax1.text(5, 9, "DI \u2192 E_eff mapping", ha="center", fontsize=14, fontweight="bold", color=TEXT)
    ax1.text(5, 7.8, r"$r = \mathrm{clip}(\mathrm{DI}\,/\,s_{\mathrm{DI}},\ 0,\ 1)$", ha="center", fontsize=12)
    ax1.text(5, 6.8, r"$E_{\mathrm{eff}}(\mathrm{DI}) = E_{\max}(1-r)^\alpha + E_{\min}\cdot r$", ha="center", fontsize=11)
    params = [
    ...
    ax1.text(5, 0.8, "Low DI \u2192 high E_eff (stiff)\nHigh DI \u2192 low E_eff (soft)", ha="center", fontsize=9, color=TEXT_SEC)
...
def fig10_coupling_four_conditions():
    ...
    conditions = [
        ("dh_baseline", "Dysbiotic\n(a_{35} high)", "DI \u2248 0.51 (late)", "E_eff \u2248 0.5 MPa", "#E05252"),
        ("commensal_static", "Commensal\n(balanced)", "DI \u2248 0", "E_eff \u2248 9.85 MPa", "#4EC9B0"),
        ("dysbiotic_static", "Dysbiotic\n(moderate)", "DI low (early)", "E_eff mid", "#F4A142"),
        ("commensal_hobic", "Commensal\n(HOBIC)", "DI \u2248 0", "E_eff \u2248 9.85 MPa", "#4E9AF1"),
    ]
    for i, (key, label, di_str, e_str, c) in enumerate(conditions):
        x = 2.2 + i * 4.0
        box = FancyBboxPatch((x - 1.5, 4.8), 3.0, 2.6, boxstyle="round,pad=0.1",
                              facecolor=c + "28", edgecolor=c, lw=1.5, zorder=3)
        ax.add_patch(box)
    ...
    ax.text(9, 6.0, "Same 3-tooth geometry\n1 MPa inward, Tie at slit", ha="center", fontsize=10, fontweight="bold", color=TEXT)
    box_mid = FancyBboxPatch((7, 5.0), 4, 1.6, boxstyle="round,pad=0.08", facecolor=LEGEND_BG, edgecolor=AXES, lw=1)
    ax.add_patch(box_mid)
    for i in range(4):
        x = 2.2 + i * 4.0
        ax.annotate("", xy=(8.2, 5.8), xytext=(x + 1.5, 4.8),
                    arrowprops=dict(arrowstyle="-|>", color=AXES, lw=1.2,
                                    connectionstyle="arc3,rad=0.2"))
    ax.annotate("", xy=(9, 4.2), xytext=(9, 5.0),
                arrowprops=dict(arrowstyle="-|>", color=AXES, lw=2))
    ax.text(9, 3.6, "Abaqus Static", ha="center", fontsize=10, fontweight="bold", color=TEXT)
    ax.annotate("", xy=(9, 2.4), xytext=(9, 3.2),
                arrowprops=dict(arrowstyle="-|>", color=AXES, lw=2))
    ax.text(9, 2.0, "ODB \u2192 MISES, |U|", ha="center", fontsize=10, color=TEXT_SEC)
    ax.text(9, 1.2, "Force BC: MISES same; |U| \u2248 19.7\u00d7 larger for DH-baseline (soft)", ha="center", fontsize=9, color="#2D8B6F")
    plt.tight_layout()
