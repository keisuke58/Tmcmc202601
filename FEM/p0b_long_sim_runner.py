...
CONDITIONS = [
    {"key": "dh_baseline",      "label": "DH-baseline",
     "theta_subdir": "dh_baseline"},
    {"key": "commensal_static", "label": "Commensal-static",
     "theta_subdir": "commensal_static"},
    {"key": "Dysbiotic_Static", "label": "Dysbiotic-static",
     "theta_subdir": "Dysbiotic_Static"},
    {"key": "Commensal_HOBIC",  "label": "Commensal-HOBIC",
     "theta_subdir": "Commensal_HOBIC"},
]
...
def step_sim(args):
    ...
    for cond in CONDITIONS:
        ...
        cmd = [
            "python3", _FEM3D,
            "--theta-json", theta_path,
            "--condition",  cond["key"],
            "--nx", str(args.nx), "--ny", str(args.ny), "--nz", str(args.nz),
            "--n-macro",     str(args.n_macro),
            "--n-react-sub", str(args.n_react_sub),
...
def step_fem(args):
    ...
    for cond in CONDITIONS:
        ...
        if not os.path.exists(snaps_path) and not args.dry_run:
            ...
            print(f"  Run: python3 p0b_long_sim_runner.py --step sim")
            continue
...
"""
        ...
        if rc != 0:
            print(f"  [error] DI export failed")
            continue
        ...
        if not args.no_abaqus:
            ...
            cmd_ext = [_ABAQUS, "python", _EXTRACT, odb_file,
                       "--out-elem", elem_csv, "--out-node", node_csv]
...
def main():
    ...
    ap.add_argument("--n-macro",     type=int, default=1000,
    ...
    ap.add_argument("--force",   action="store_true",
                    help="Re-run even if done.flag exists")
    ap.add_argument("--no-abaqus", action="store_true",
                    help="Skip Abaqus solve (only export DI + INP)")
    args = ap.parse_args()
