...
_PARAM_TEX_20 = [
    ...
    r"$a_{13}$", r"$a_{14}$", r"$a_{23}$", r"$a_{24}$",r"$a_{55}$",
    r"$b_5$",    r"$a_{15}$", r"$a_{25}$", r"$a_{35}$",r"$a_{45}$",
]
_PARAM_DESC_20 = [
    "S.or self-inh.", "S.or–A.na", "A.na self-inh.", "S.or growth", "A.na growth",
    "Vei self-inh.", "Vei–F.nu", "F.nu self-inh.", "Vei growth", "F.nu growth",
    "S.or–Vei", "S.or–F.nu", "A.na–Vei", "A.na–F.nu", "Pg self-inh.",
    "Pg growth", "S.or–Pg", "A.na–Pg", "Vei–Pg", "F.nu–Pg",
]
...
def plot_combined(all_data: dict, out_path: Path) -> None:
    ...
    for ax, (rho_dict, depth_name) in zip(
        ...
        for ci, cond in enumerate(conds):
            ...
            ax.bar(x + offs, rho, width=w * 0.9,
                   color=COND_COLOR.get(cond, "#888888"), alpha=0.8,
                   label=COND_LABEL.get(cond, cond))
    ...
    fig.suptitle("Parameter sensitivity: Spearman correlation with von Mises stress\n"
                 "(top 10 parameters by mean |ρ| at substrate)", fontsize=11)
