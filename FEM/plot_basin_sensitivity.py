...
def main():
    ...
    if "dh_baseline" in all_samples:
        ...
        ax_a.hist(di_vals, bins=bins, color="#d62728", alpha=0.7,
                  edgecolor="k", linewidth=0.5, density=True, label="Posterior samples")

        # MAP line
        ax_a.axvline(di_map, color="navy", linewidth=2.5, linestyle="--",
                     label=f"MAP = {di_map:.3f}", zorder=5)
        ...
        ax_a.text(0.95, 0.92, f"90% CI: [{ci[0]:.2f}, {ci[1]:.2f}]\n"
    ...
    if "dh_baseline" in all_samples:
        ...
        for j, (key, color, name) in enumerate(zip(phi_keys, SP_COLORS, SP_SHORT)):
            ...
            ax_b.bar(x_idx, vals, bottom=bottom, color=color, width=1.0,
        ...
        ax_b.set_ylim(0, 1)
        ax_b.legend(fontsize=8, loc="upper left", ncol=5,
        ...
        if n_low > 0 and n_low < n:
            ...
            ax_b.text(n_low / 2, -0.08, "Diverse\nbasin", ha="center",
                      fontsize=8, transform=ax_b.get_xaxis_transform())
            ax_b.text((n_low + n) / 2, -0.08, "Monodom.\nbasin", ha="center",
                      fontsize=8, transform=ax_b.get_xaxis_transform())
    ...
    for i, c in enumerate(ALL_CONDITIONS):
        ...
        if c in all_samples:
            ...
            ax_c.scatter(i + jitter, di_vals, color=meta["color"], alpha=0.6,
                         s=25, edgecolor="k", linewidth=0.3, zorder=3)

        # MAP marker
        if c in master:
            di_map = master[c]["di_0d_map"]
            ax_c.scatter(i, di_map, marker="D", color="navy", s=80, zorder=5,
            ...
            if n_filt > 0:
                label += f"\n{n_filt} filtered"
            ax_c.text(i, -0.08, label, ha="center", fontsize=7.5,
    ...
    # Legend annotations
    ax_c.text(0.02, 0.97, "Diverse basin (DI < 0.3)", transform=ax_c.transAxes,
              fontsize=8, va="top", color="green", weight="bold")
    ax_c.text(0.02, 0.07, "Mono-dominated basin (DI > 0.7)", transform=ax_c.transAxes,
              fontsize=8, va="bottom", color="red", weight="bold")
    ...
    for c in ALL_CONDITIONS:
        ...
        if c in all_samples:
            ...
            ax_d.scatter(di_vals, E_vals, color=meta["color"], alpha=0.5,
                         s=20, edgecolor="k", linewidth=0.3, label=meta["label"], zorder=3)

        if c in master:
            # MAP point
            di_map = master[c]["di_0d_map"]
            E_map = master[c]["E_di_map"]
            ax_d.scatter(di_map, E_map, marker="D", color=meta["color"],
                         s=100, edgecolor="navy", linewidth=2, zorder=5)

            # CI horizontal band
            ci = master[c]["di_0d_ci90"]
            E_ci = master[c]["E_di_ci90"]
            ax_d.fill_between([ci[0], ci[1]], [E_ci[0], E_ci[0]],
                              [E_ci[1], E_ci[1]],
                              color=meta["color"], alpha=0.1, zorder=1)

    # Literature reference zone
    ax_d.axhspan(550, 14000, alpha=0.04, color="blue")
    ax_d.axhspan(20, 380, alpha=0.04, color="orange")
    ax_d.text(0.05, 900, "Commensal range\n(Pattem 2018)", fontsize=7,
              color="blue", alpha=0.7, style="italic")
    ax_d.text(0.7, 50, "Dysbiotic range\n(Pattem 2018)", fontsize=7,
