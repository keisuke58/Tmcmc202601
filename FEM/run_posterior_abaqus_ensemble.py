...
def _run_fem(theta: np.ndarray, n_macro: int, n_react_sub: int,
             dt_h: float, condition: str) -> tuple[np.ndarray, np.ndarray]:
...
def _export_field_csv(snaps_phi: np.ndarray,
...
def run_sample(
    ...
    if done_flag.exists() and stress_json.exists():
        with stress_json.open() as f:
            d = json.load(f)
        print("  [resume] sample %04d  sub=%.3g  surf=%.3g" % (
    ...
    if fem_done.exists() and field_csv.exists():
        ...
        snaps_phi, mesh_x, mesh_y, mesh_z = _run_fem(
            theta, n_macro, n_react_sub, dt_h, cond)
    ...
    print("  [done] sample %04d  sub=%.4g Pa  surf=%.4g Pa  (%.1fs total)" % (
        enum_k, stress["substrate_smises"], stress["surface_smises"],
...
def aggregate(out_dir: Path) -> dict:
    ...
    for pct, tag in [(5, "p05"), (50, "p50"), (95, "p95")]:
        np.save(out_dir / ("stress_%s.npy" % tag),
                np.percentile(all_arr, pct, axis=0))
    meta = {
        "n_valid":       len(subs),
...
def plot_ensemble(results: dict[str, dict], out_path: Path) -> None:
    ...
    for ax, (depth_key, depth_label) in zip(
        ...
        for k, cond in enumerate(conds):
            ...
            bar = ax.bar(k, p50 / 1e6, color=colors[cond], alpha=0.8,
                         label=labels[cond], width=0.5)
            ax.errorbar(k, p50 / 1e6,
                        yerr=[[( p50 - p05) / 1e6], [(p95 - p50) / 1e6]],
                        fmt="none", color="black", capsize=6, linewidth=1.5)
            ax.text(k, p95 / 1e6 + 0.008,
                    "%.3f" % (p50 / 1e6),
...
def main() -> None:
    ...
    for cond in args.conditions:
        ...
        indices          = (np.arange(n_total) if n_total == n_use
                            else rng.choice(n_total, size=n_use, replace=False))

        meta_path = out_dir / "run_meta.json"
        with meta_path.open("w") as f:
            json.dump({
                "condition": cond, "n_samples": int(n_use),
                "n_macro": args.n_macro, "seed": args.seed,
        ...
        if not args.plot_only:
            ...
            for enum_k, idx in enumerate(indices):
                ...
                run_sample(enum_k, theta, cond, sample_dir,
                           args.n_macro, args.n_react_sub, args.dt_h)

            print("\n[cond done] %s  total %.1fm" % (
                cond, (time.perf_counter() - t_cond) / 60))
        ...
        if meta:
            ...
            print("  substrate: p50=%.4g  [%.4g, %.4g] Pa" % (
                meta["substrate_p50"], meta["substrate_p05"], meta["substrate_p95"]))
            print("  surface  : p50=%.4g  [%.4g, %.4g] Pa" % (
