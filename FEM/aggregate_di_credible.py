...
def process_condition(cond: str, run_abaqus: bool) -> dict | None:
    ...
    print("  DI quantiles: p05=%.4f  p50=%.4f  p95=%.4f" % (
    ...
    if run_abaqus:
        ...
        for qi, qtag in enumerate(QTAGS):
            ...
            if stress:
                ...
                print("done in %.1fs  sub=%.3g  surf=%.3g" % (
...
def _plot_spatial_uncertainty(
    ...
    fig.suptitle("B1: DI Spatial Credible Interval  –  %s  (z=mid slice)" % (
        COND_LABELS.get(cond, cond)), fontsize=11, fontweight="bold")
...
def _plot_depth_profile(
    ...
    ax.fill_between(x_u, p05_d, p95_d, alpha=0.25, color=col,
                    label="p05–p95 (θ uncertainty)")
    ax.plot(x_u, p50_d, color=col, lw=2, label="p50 median")
    ax.plot(x_u, sample_depth.min(axis=0), color=col, lw=0.7, ls="--", alpha=0.5)
    ax.plot(x_u, sample_depth.max(axis=0), color=col, lw=0.7, ls="--", alpha=0.5,
            label="sample min/max")

    ax.set_xlabel("Depth (x, normalised)", fontsize=10)
    ax.set_ylabel("Dysbiotic Index (DI)", fontsize=10)
    ax.set_title("B1: DI Depth Profile with θ Credible Band  –  %s" % (
        COND_LABELS.get(cond, cond)), fontsize=10)
...
def _plot_cross_condition(conditions: list[str]) -> None:
    ...
    for cond in conditions:
        ...
        for ax, (qi, qtag) in zip(axes, [(1, "Median (p50)"), (None, "p05–p95 band")]):
            if qi is not None:
                ...
                ax.fill_between(x_u, _dm(di_q[0]), _dm(di_q[2]),
                                alpha=0.15, color=col)
            else:
                ax.fill_between(x_u, _dm(di_q[0]), _dm(di_q[2]),
                                alpha=0.25, color=col, label=lab)
    ...
    fig.suptitle("B1: Cross-Condition DI Depth Profiles + θ Credible Bands",
    ...
    for ci, cond in enumerate(conditions):
        ...
        for si, (skey, slabel) in enumerate([
            ...
            if stress_all_p.exists():
                ...
                ax.bar(0, theta_vals.mean(),
                       color=col, alpha=0.7, label="θ-ensemble mean")
                ax.errorbar(0, theta_vals.mean(),
                            yerr=[[theta_vals.mean() - np.percentile(theta_vals, 5)],
                                  [np.percentile(theta_vals, 95) - theta_vals.mean()]],
                            fmt="none", color="black", capsize=8, lw=1.5,
                            label="θ p05–p95")
            ...
            if len(di_vals) == 3:
                ax.bar(1, di_vals[1],
                       color="orange", alpha=0.7, label="DI-CI p50")
                ax.errorbar(1, di_vals[1],
                            yerr=[[abs(di_vals[1] - di_vals[0])],
                                  [abs(di_vals[2] - di_vals[1])]],
                            fmt="none", color="darkorange", capsize=8, lw=1.5,
                            label="DI p05–p95")
...
def main() -> None:
    ...
    ap.add_argument("--no-abaqus",   action="store_true",
                    help="Skip Abaqus runs (field + plot only)")
