...
def fix_A3():
    ...
    for r in rows:
        v = r["variant"]
        variants[v] = (float(r["substrate_smises"]) / 1e6,
                       float(r["surface_smises"]) / 1e6)
    ...
    for si, (col_idx, slabel, ylim, ref_key) in enumerate([
        ...
        bars = ax.bar(x, subs, width=0.55, color=cols, alpha=0.85,
        ...
        for xi, (v, s) in enumerate(zip(order, subs)):
            ...
            ax.text(xi, s + (ylim[1] - ylim[0]) * 0.015,
                    f"{s:.3f} MPa\n({sign}{delta:.1f}%)",
                    ha="center", va="bottom", fontsize=9,
                    color="black", fontweight="bold" if v == "mild_weight" else "normal")

        # mild_weight に注目矢印
        mw_idx = order.index("mild_weight")
        mw_val = variants["mild_weight"][col_idx]
        dh_val = variants["dh_old"][col_idx]
        ax.annotate("", xy=(mw_idx, mw_val), xytext=(0, mw_val),
                    arrowprops=dict(arrowstyle="<->", color="#2ca02c", lw=1.8))
        ax.text(mw_idx / 2, (mw_val + dh_val) / 2,
                f"Δ={(mw_val-dh_val)/dh_val*100:+.1f}%",
                ha="center", va="bottom", fontsize=8.5, color="#2ca02c")
...
def fix_DI():
    ...
    for cond in CONDITIONS:
        ...
        ax_all_band.fill_between(d["x"], d["p05"], d["p95"],
                                  color=c, alpha=0.35, label=LABELS_FLAT[cond])
        ax_all_band.plot(d["x"], d["p50"], color=c, lw=1.2)

    for ax, title in [(ax_all_med, "Median DI (all cond.)"),
    ...
    for cond in conds_no_dh:
        ...
        ax_zoom_ci.fill_between(d["x"], d["p05"], d["p95"],
                                 color=c, alpha=0.4, label=LABELS_FLAT[cond])
        ax_zoom_ci.plot(d["x"], d["p50"], color=c, lw=1.5)

    # dh-baseline の信頼帯を薄く追加（参照用）
    d_dh = data["dh_baseline"]
    ax_zoom_med.plot(d_dh["x"], d_dh["p50"], color=COLORS["dh_baseline"],
                     lw=1.0, ls="--", alpha=0.4, label="dh-baseline (ref)")
    ax_zoom_ci.plot(d_dh["x"], d_dh["p50"], color=COLORS["dh_baseline"],
                    lw=0.8, ls="--", alpha=0.3)

    for ax, title in [(ax_zoom_med, "Zoomed: Median DI (excl. dh-baseline)"),
...
def fix_E():
    ...
    for si, (col, slabel, ylim) in enumerate([
        ...
        bp = ax.boxplot(vals_list, positions=range(len(CONDITIONS)),
                        ...
                        flierprops=dict(marker="o", ms=5, alpha=0.3,
                                        markeredgewidth=0.5))
        ...
        for xi, (vals, col_) in enumerate(zip(vals_list, colors)):
            ...
            out_range = ~in_range
            ax.scatter(xi + jitter[in_range],  vals[in_range],
                       color=col_, s=22, zorder=5, alpha=0.85)
            if out_range.any():
                n_out = out_range.sum()
                ax.text(xi, ylim[1] - (ylim[1]-ylim[0])*0.05,
                        f"▲{n_out}外", ha="center", fontsize=7.5,
                        color=col_, fontweight="bold")

        # p05/p50/p95 アノテーション
        for xi, vals in enumerate(vals_list):
            p50 = np.percentile(vals, 50)
            ax.text(xi + 0.22, p50, f"{p50:.3f}", va="center",
        ...
        for xi, vals in enumerate(vals_list):
            for pct, ls_ in [(5, ":"), (95, ":")]:
                ...
                if ylim[0] <= v <= ylim[1]:
                    ax.plot([xi - 0.22, xi + 0.22], [v, v],
                            color=colors[xi], lw=0.9, ls=ls_, alpha=0.7)
    ...
    PARAM_NICE = {
        "a11":"a₁₁(So·So)","a12":"a₁₂(So·An)","a22":"a₂₂(An·An)",
        "b1":"b₁(So grow)","b2":"b₂(An grow)","a33":"a₃₃(Vd·Vd)",
        "a34":"a₃₄(Vd·Fn)","a44":"a₄₄(Fn·Fn)","b3":"b₃(Vd grow)",
        "b4":"b₄(Fn grow)","a13":"a₁₃(So·Vd)","a14":"a₁₄(So·Fn)",
        "a23":"a₂₃(An·Vd)","a24":"a₂₄(An·Fn)","a55":"a₅₅(Pg·Pg)",
    ...
    parts = ax_vln.violinplot(vals_list, positions=range(len(CONDITIONS)),
                               showmedians=False, showextrema=False, widths=0.55)
    for body, col_ in zip(parts["bodies"], colors_l):
        body.set_facecolor(col_)
        body.set_alpha(0.38)
    bp = ax_vln.boxplot(vals_list, positions=range(len(CONDITIONS)),
    ...
    for xi, (vals, col_) in enumerate(zip(vals_list, colors_l)):
        ...
        n_out = (~in_r).sum()
        if n_out:
            ax_vln.text(xi, 0.91, f"▲{n_out}", ha="center", fontsize=7.5,
                        color=col_, fontweight="bold")
    ...
    ax_vln.legend(handles=[
        mpatches.Patch(color=COLORS[c], label=LABELS_FLAT[c], alpha=0.75)
        for c in CONDITIONS], fontsize=8, loc="upper right")

    # sensitivity top-8
    mean_abs_rho = np.mean(
        np.abs(np.array([spear[c]["substrate"] for c in CONDITIONS])), axis=0)
    ...
    ax_sens.set_yticks(ypos); ax_sens.set_yticklabels(top_labs, fontsize=8.5)
    ax_sens.set_xlabel("Mean |Spearman ρ|", fontsize=9)
    ax_sens.set_title("Param Sensitivity (substrate)", fontsize=10)
    ax_sens.grid(axis="x", alpha=0.3, ls="--"); ax_sens.set_xlim(0, 0.65)
    ...
    for ax, col, slabel in [(ax_ci_s, 0, "Substrate"), (ax_ci_u, 1, "Surface")]:
        ...
        for xi, (v, lo, hi) in enumerate(zip(p50, p05, p95)):
            ax.text(xi, hi + (ylim_ci[1]-ylim_ci[0])*0.012,
                    f"{v:.3f}\n[{lo:.3f}–{hi:.3f}]",
                    ha="center", va="bottom", fontsize=7.5)
        ax.set_xticks(x)
        ax.set_xticklabels([LABELS_FLAT[c] for c in CONDITIONS], fontsize=8.5,
                            rotation=10, ha="right")
        ax.set_ylabel("$S_M$ (MPa)", fontsize=9); ax.set_title(f"CI p5–p95 — {slabel}", fontsize=10)
        ax.grid(axis="y", alpha=0.3, ls="--"); ax.set_ylim(ylim_ci)
    ...
    xd = np.arange(len(CONDITIONS)); wd = 0.35
    ax_dlt.bar(xd - wd/2, d_sub,  wd, color=colors_l, alpha=0.85, edgecolor="k", lw=0.5)
    ax_dlt.bar(xd + wd/2, d_surf, wd, color=colors_l, alpha=0.4,  edgecolor="k", lw=0.5, hatch="//")
    ax_dlt.axhline(0, color="k", lw=0.8, ls="--")
    # 数値ラベル
    for xi, (ds, du) in enumerate(zip(d_sub, d_surf)):
        ax_dlt.text(xi - wd/2, ds + (0.15 if ds >= 0 else -0.4),
                    f"{ds:+.1f}%", ha="center", fontsize=7.5)
    ax_dlt.set_xticks(xd)
    ax_dlt.set_xticklabels([LABELS_FLAT[c] for c in CONDITIONS],
...
def fix_B3():
    ...
    with (czm_base / "czm_analytical.csv").open() as f:
        for r in csv_mod.DictReader(f):
            ...
            if c not in results_raw:
                results_raw[c] = {}
            results_raw[c][qt] = {k: float(v) if k not in ('condition','di_qtag') else v for k, v in r.items()
                                   if k not in ("condition", "di_qtag")}

    # posterior CZM
    DI_SCALE = 0.025778; DI_EXP = 2.0
    T0 = 1.0e6; GC0 = 10.0
    post_czm = {}
    for cond in CONDITIONS:
        ...
        if stack.ndim == 2 and stack.shape[0] != 20:
            stack = stack.T
        di_bot = np.array([
            float(stack[i].reshape(15,15,15)[:, :, :3].mean()) for i in range(20)
        ])
    ...
    bars = ax.bar(x, rf_p50, width=0.52, color=colors, alpha=0.85,
    ...
    ax.errorbar(x, rf_p50, yerr=[rf_lo, rf_hi],
    ...
    for xi, cond in enumerate(CONDITIONS):
        ...
        ax.scatter(xi + jit[in_r], rf_post[in_r],
                   color=COLORS[cond], s=22, alpha=0.55, zorder=5)
        n_out = (~in_r).sum()
        if n_out:
            ax.text(xi, ylim_rf[1] * 0.97, f"▲{n_out}", ha="center",
    ...
    for xi, (v, lo, hi) in enumerate(zip(rf_p50, rf_p05, rf_p95)):
        ...
        ax.text(xi, max(hi, v) + 1.5e4, f"{v/1e5:.2f}×10⁵ N\n({delta:+.1f}%)",
    ...
    ax.set_title("B3: Peak Pull Force per Condition\n(bars=p50, errors=p05–p95 DI CI, dots=20 samples)", fontsize=10)
    ax.grid(axis="y", alpha=0.3, ls="--")
    ...
    ax2.plot(di_curve, gc_curve, "k--", lw=1.5, alpha=0.7,
             label=r"$G_c=G_{c,0}(1-DI/s)^n$")
    ...
    for cond in CONDITIONS:
        ...
        ax2.scatter(di_post[in_r], gc_post[in_r], color=COLORS[cond],
                    s=22, alpha=0.45, zorder=3)
        for qtag, mk, ms in [("p05","v",9),("p50","o",12),("p95","^",9)]:
            r_ = results_raw[cond][qtag]
            if r_["di_mean"] <= DI_zoom:
                ax2.scatter(r_["di_mean"], r_["gc_eff"], color=COLORS[cond],
                            s=ms**2, marker=mk, edgecolors="k", lw=0.7, zorder=5)
        r50 = results_raw[cond]["p50"]
        ax2.annotate(LABELS_FLAT[cond],
                     (r50["di_mean"], r50["gc_eff"]),
                     textcoords="offset points", xytext=(5, 4), fontsize=8.5)

    if total_out > 0:
        ax2.text(0.98, 0.05, f"注: {total_out}サンプルが表示範囲外\n(DI > {DI_zoom:.3f})",
