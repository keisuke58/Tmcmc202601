...
for col, tooth in enumerate(TEETH):
    ...
    sc_ref = ax.scatter(
        n_x[idx], n_y[idx], n_z[idx],
        c=Umag[idx], cmap=CMAP_U, norm=norm_u,
        s=4, alpha=0.75, linewidths=0, depthshade=True)
    ax.set_title('%s  (n_nodes=%d)' % (tooth, mask.sum()), fontsize=10)
    set_3d_labels(ax)
cbar = fig.colorbar(sc_ref, ax=fig.axes, label='|U| displacement (mm)',
...
for col, tooth in enumerate(TEETH):
    ...
    idx  = idx[subsample(idx)]
    sc_ref = ax.scatter(
        e_cx[idx], e_cy[idx], e_cz[idx],
        c=mises[idx], cmap=CMAP_S, norm=norm_s,
        s=3, alpha=0.65, linewidths=0, depthshade=True)
    ax.set_title('%s  (n_elems=%d)' % (tooth, mask.sum()), fontsize=10)
    set_3d_labels(ax)
cbar = fig.colorbar(sc_ref, ax=fig.axes, label='von Mises (MPa)',
                    shrink=0.55, pad=0.05)
cbar.ax.tick_params(labelsize=8)
fig.suptitle('Biofilm von Mises stress – 1 MPa inward pressure (2nd–98th pct color scale)', fontsize=12)
plt.tight_layout()
...
for ax, tooth in zip(axes, TEETH):
    ...
    stats_txt = (
        ...
        'q75   = %.4f\n'
        'max   = %.4f'
    ) % (m_t.min(), np.percentile(m_t,25), med, mu,
         np.percentile(m_t,75), m_t.max())
...
for ax, tooth in zip(axes, TEETH):
    ...
    q25s  = np.array(q25s)
    q75s  = np.array(q75s)
    ax.fill_between(b_ids, q25s, q75s,
                    alpha=0.25, color=COLORS[tooth], label='IQR (25-75%)')
    ax.plot(b_ids, meds,  'o-', color=COLORS[tooth],
            linewidth=2, markersize=5, label='Median')
    ax.plot(b_ids, means, 's--', color=COLORS[tooth],
            linewidth=1.2, markersize=4, alpha=0.8, label='Mean')
...
for ax, tooth in zip(axes, TEETH):
    ...
    for j, (lbl, u) in enumerate([('INNER', u_inner), ('OUTER', u_outer)]):
        ax.text(j + 1, ax.get_ylim()[1] * 0.98 if ax.get_ylim()[1] > 0 else u.max() * 1.05,
                'n=%d\nmed=%.3e' % (len(u), np.median(u)),
                ha='center', va='top', fontsize=7)
fig.suptitle(
    'Displacement |U| at inner (ENCASTRE→0) and outer (free) surface nodes\n'
    'Inner = 0 by BC (ENCASTRE); outer shows actual deformation', fontsize=11)
plt.tight_layout()
...
bplot = ax.boxplot(
    data_per_tooth, labels=TEETH, patch_artist=True,
...
ax.set_title(
    'MISES stress per tooth – 1 MPa inward pressure\n(outliers hidden)', fontsize=11)
ax.grid(True, alpha=0.3, axis='y')
plt.tight_layout()
...
ax.axis('off')
col_labels = [
    'Tooth', 'Elements', 'Nodes',
    'U_max (mm)', 'U_outer_med (mm)',
    'MISES_min', 'MISES_mean', 'MISES_max',  '(MPa)'
]
rows = []
for tooth in TEETH:
    em      = (e_tooth == tooth)
...
for col, tooth in enumerate(['T30', 'T31']):
    ...
    ax.scatter(n_x[idx], n_y[idx], n_z[idx],
               c=Umag[idx], cmap=CMAP_U, norm=norm_u,
               s=3, alpha=0.5, linewidths=0, label='OUTER')

    # APPROX nodes – highlighted in orange, larger
    m_ap = mask_t & (n_region == 'APPROX')
    if m_ap.sum() > 0:
        idx_ap = np.where(m_ap)[0]
        ax.scatter(n_x[idx_ap], n_y[idx_ap], n_z[idx_ap],
                   c='orange', s=20, alpha=0.9, linewidths=0,
                   zorder=5, label='APPROX (slit %d)' % m_ap.sum())

    # INNER nodes – small grey
    m_in = mask_t & (n_region == 'INNER')
    idx_in = np.where(m_in)[0][subsample(np.where(m_in)[0], 2000)]
    ax.scatter(n_x[idx_in], n_y[idx_in], n_z[idx_in],
               c='lightgrey', s=1, alpha=0.3, linewidths=0)
...
bplot = ax.boxplot(data_list, tick_labels=tick_list, patch_artist=True,
                   notch=False, showfliers=True,
                   flierprops=dict(marker='.', markersize=1.5, alpha=0.3))
colors = ['#aaaacc', '#ff8800', '#5599ff',  # T30: INNER, APPROX, OUTER
          '#aaccaa', '#ff8800', '#44aa44']   # T31: INNER, APPROX, OUTER
...
vcolors = ['#9999dd', '#ff8800', '#4488ff',  # T30
           '#99bb99', '#ff8800', '#44aa44']   # T31
...
bplot = ax.boxplot(data_all, tick_labels=TEETH, patch_artist=True,
                   notch=False, showfliers=False,
                   medianprops=dict(color='k', linewidth=2))
for patch, tooth in zip(bplot['boxes'], TEETH):
    patch.set_facecolor(COLORS[tooth] + 'aa')
...
for i, (tooth, d) in enumerate(zip(TEETH, data_all)):
    ax.text(i + 1, float(np.percentile(d, 75)) * 1.01,
...
for i, (reg, d) in enumerate(region_data.items()):
    ax2.text(i + 1, float(np.percentile(d, 75)) * 1.01,
...
for k, y0 in enumerate(y_cuts):
    ...
    cz_s = e_cz[in_slice]
    ms_s = mises[in_slice]
    sc = ax.scatter(cx_s, cz_s, c=ms_s, cmap=CMAP_S,
                    norm=norm_s, s=6, alpha=0.75, linewidths=0)
    # Colour teeth differently with marker shape
    for tooth, marker in [('T23', 'o'), ('T30', 's'), ('T31', '^')]:
        mt = in_slice & (e_tooth == tooth)
        if mt.sum() > 0:
            ax.scatter(e_cx[mt], e_cz[mt], c=mises[mt], cmap=CMAP_S,
                       norm=norm_s, s=8, alpha=0.8, marker=marker,
                       linewidths=0, label=tooth)
    plt.colorbar(sc, ax=ax, label='MISES (MPa)', shrink=0.75)
    ax.set_xlabel('x (mm)', fontsize=8)
    ax.set_ylabel('z (mm)', fontsize=8)
    ax.set_title('Cross-section y = %.1f mm  (±%.1f mm, n=%d elements)' % (
        y0, half_width, in_slice.sum()), fontsize=9)
...
for col, tooth in enumerate(TEETH):
    ...
    sc = ax.scatter(e_cx[idx], e_cz[idx], c=sed[idx], cmap='inferno',
                    norm=norm_sed, s=4, alpha=0.75, linewidths=0)
    plt.colorbar(sc, ax=ax, label='SED (mJ/mm³)', shrink=0.75)
    ax.set_xlabel('x (mm)', fontsize=8)
    ax.set_ylabel('z (mm)', fontsize=8)
    ax.set_title('%s  SED median=%.4f  max=%.3f  mJ/mm³' % (
        tooth,
        float(np.median(sed[mask])),
        float(sed[mask].max())), fontsize=9)
    ax.set_aspect('equal', adjustable='datalim')
    ax.grid(True, alpha=0.2)
...
print("  MISES – min=%.4f  mean=%.4f  max=%.4f  MPa" % (
    float(mises.min()), float(mises.mean()), float(mises.max())))
print("\nPer-tooth MISES:")
for tooth in TEETH:
    m = mises[e_tooth == tooth]
    print("  %-4s  min=%.4f  mean=%.4f  max=%.4f  MPa  (n=%d)" % (
